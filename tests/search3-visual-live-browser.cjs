...
(async()=>{
 ...
 try{for(const width of [390,768,1280]){
  ...
  await page.locator('[data-action="offer"][data-key="tourvisor%3Avisual-tv-101"]').click();await page.waitForFunction(()=>document.querySelector('[data-action="confirm-tour"]')&&!document.querySelector('[data-action="confirm-tour"]').disabled);
  ...
  receipts.push({width,three_sources_one_hotel:true,calendar_database_observation:true,search_before_submit:0,total:133500.5,local_application:true,supplier_requests:0,lead_requests:0});await context.close();
