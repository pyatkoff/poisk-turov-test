<?php
require_once $argv[1].'/app/integrations/andromeda-client.php';
$calls=[];
...
require_once $argv[1].'/app/integrations/andromeda-search.php';
...
$dictionary=['townfrom'=>['payload'=>['TOWNFROM'=>[['id'=>1,'name'=>'Moscow']]]],'all'=>['payload'=>[
    ...
    'OPERATORS'=>[],
    'MEAL'=>[['id'=>5,'name'=>'AI','alias'=>'Все включено'],['id'=>7,'name'=>'UAI','alias'=>'Ультра все включено'],['id'=>1,'name'=>'BB','alias'=>'Завтрак'],['id'=>3,'name'=>'HB','alias'=>'Завтрак и ужин'],['id'=>4,'name'=>'FB','alias'=>'Трех разовое'],['id'=>8,'name'=>'OB','alias'=>'Без питания']],
...
if($point['HOTELS']!=='2000042763'||$point['OPERATORS']!=='5'||$point['MEAL']!=='5,7')throw new RuntimeException('HTTP criteria lost hotel/operator/meal filter');
$request['page']=2;
...
if($wire[1]['HOTELS']!=='2000042763'||$wire[1]['OPERATORS']!=='5'||$wire[1]['MEAL']!=='5,7')throw new RuntimeException('upstream hotel/meal filter absent');
...
if($full['status']!=='complete'||($pointSaved['store']['criteria']['HOTELS']??null)!=='2000042763'||($pointSaved['store']['criteria']['MEAL']??null)!=='5,7')throw new RuntimeException('hotel/meal criteria rejected by private store');
