...
php=full.source()
ok(full.EXPERIMENT in php and full.egypt.EXPERIMENT not in php,'fresh two-adult experiment bound')
...
ok("'adults'=>2" in php and "'ADULT'=>2" in php and "($value['adults'] ?? null) !== 2" in php,'two-adult party remains explicit across providers')
ok("'children'=>0" in php and "'childs'=>[]" in php and "'CHILD'=>0" in php,'two-adult control has no child payload')
...
ok("'three-price-egypt-two-adult-20260914-v8'" in php and "'generation'=>26091408" in php,'new operation/session generation bound')
...
tv=case('tourvisor',[offer('tourvisor',101,'114357','14357')])
report=full.build_report({'anex':anex,'andromeda':andromeda,'tourvisor':tv},'completed')
ok(report['spec']['adults']==2 and report['spec']['meal_family']=='ai' and report['observation_summary']['andromeda_pages_loaded']==2,
   'report retains two-adult AI scenario and complete pagination')
...
print(f'ANEX three-source Egypt two-adult comparator: {checks} checks passed; network=0')
