...
php=full.source()
ok(full.EXPERIMENT in php and full.egypt.EXPERIMENT not in php,'fresh solo experiment bound')
...
ok("'adults'=>1" in php and "'ADULT'=>1" in php and "($value['adults'] ?? null) !== 1" in php,'solo party reaches all provider and validation paths')
ok("'children'=>0" in php and "'childs'=>[]" in php and "'CHILD'=>0" in php,'solo adult control has no child payload')
ok("($value['meal_family'] ?? null) !== 'ai'" in php and "$mealFamily !== 'ai'" in php and "'meal_family'=>'ai'" in php,
   'AI family remains enforced at input, normalized offer and output')
ok("'meal'=>7" in php,'current Search3 AI selector remains unchanged for Andromeda')
ok("'three-price-egypt-solo-20260914-v7'" in php and "'generation'=>26091407" in php,'new operation/session generation bound')
...
def offer(provider,local,price,fuel=None):
    return {'provider':provider,'local_hotel_id':local,'external_hotel_id':str(local),'date':'2026-12-07','nights':10,
            'adults':1,'children':0,'meal_family':'ai','meal_label':'AI','room':'Standard Room','room_norm':'standard room',
            'placement':'SGL','placement_norm':'sgl','price':price,'currency':'RUB','fuel_charge':fuel,
...
andromeda=case('andromeda',[offer('andromeda',101,'70000')],
...
bad=case('andromeda',[offer('andromeda',101,'70000')],
...
anex=case('anex',[offer('anex',101,'70000')],{'received_offers':1,'mapped_received':1,'unmapped_received':0})
tv=case('tourvisor',[offer('tourvisor',101,'80768','10768')])
report=full.build_report({'anex':anex,'andromeda':andromeda,'tourvisor':tv},'completed')
ok(report['spec']['adults']==1 and report['spec']['meal_family']=='ai' and report['observation_summary']['andromeda_pages_loaded']==2,
   'report retains solo AI scenario and complete pagination')
...
ok(report['comparison']['triple_examples'][0]['basis']=='same_current_local_hotel_date_party_meal_and_exact_room',
   'comparison basis remains exact party/meal/room context')
...
print(f'ANEX three-source Egypt solo comparator: {checks} checks passed; network=0')
