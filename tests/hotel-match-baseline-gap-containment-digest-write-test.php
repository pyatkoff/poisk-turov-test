...
if(count(hmbgcw_set_lines($fixture))!==2||hmbgcw_set_sha($fixture)===hmbgcw_set_sha(array_reverse($fixture))===false)exit(3);
$s=(string)file_get_contents(__DIR__.'/../scripts/diagnostics/hotel_match_baseline_gap_containment_digest_write.php');foreach(['SERIALIZABLE','hmbgc_run($pdo,$auditManifest)','FOR UPDATE','current_safe_set_changed','locked_safe_set_changed','post_commit_readback_failed','resolver_readback_failed'] as $n)if(strpos($s,$n)===false)exit(4);if(substr_count($s,'INSERT INTO anex_hotel_search_mappings')!==1)exit(5);if(stripos($s,'UPDATE andromeda_hotel_identities')!==false||stripos($s,'DELETE FROM')!==false)exit(6);echo "MATCH containment digest writer source guards PASS; expected=94 supplier=0\n";
