...
function payload(offers){return{source:'anytour-db-first-results-v1',scopeVersion:1,scopeDigest:'c'.repeat(64),selectionAuthority:false,hotelCount:1,offerCount:offers.length,storedOfferCount:offers.length,withheldOfferCount:0,hotels:[{anytourHotelId:77,hotel:{id:77,catalog:'anytour',revision:1,name:'Own Hotel'},offers}]};}
...
(async()=>{
 ...
 env.root.V2Results.render([live],{empty:false});assert.equal(env.calls.at(-1).list[0].tours.length,1);
 ...
 let merged=env.calls.at(-1).list;assert.equal(merged.length,1);assert.equal(merged[0].tours.length,2,'DB cached offer is merged with already-rendered live offer');assert.ok(merged[0].tours.some(t=>t.cachedListing===true));
 ...
 env.root.dispatchEvent(new env.root.CustomEvent('v2:search-reset',{detail:{generation:2,dirty:true}}));env.root.V2SearchLifecycle.generation=2;env.root.V2SearchLifecycle.dirty=true;env.root.V2Results.render([live],{empty:true});assert.equal(env.calls.at(-1).list[0].tours.length,1,'dirty reset clears DB snapshot');

 let emptyFetches=0;
 env=fakeRoot('/_preview/search3-local-candidate/poisk-turov/',async()=>{emptyFetches++;return{ok:true,json:async()=>({ok:true,data:{...payload([]),hotelCount:0,offerCount:0,storedOfferCount:0,hotels:[]}})}});env.root.V2Results.render([live],{empty:false});const before=env.calls.length;env.root.dispatchEvent(new env.root.CustomEvent('v2:search-reset',{detail:{generation:1}}));await tick();await tick();assert.equal(emptyFetches,1);assert.equal(env.calls.length,before,'empty DB does not wipe skeleton/live results');

 console.log('SEARCH3_LOCAL_DB_PROVIDER_OK project=2 merge=2 empty_noop=1 route_isolated=1');
