...
// Multi-window resume: an exact unknown/no-replay day is a gap, not a reason
// to repeat it or hide later independent days.
$rangeCalls=[];
$range=AnyTourAnexLocalOfferCollectorV1::collectRange(
    ['action'=>'search','generation'=>99,'params'=>[]],
    '2026-10-01','2026-10-03',
    static function(array $request,int $index,array $window)use(&$rangeCalls):array{
        $rangeCalls[]=$window['from'];
        if($index===0)return [
            'source'=>'fixture','status'=>'unknown_no_replay','selection_authority'=>false,
        ];
        return [
            'source'=>'fixture','status'=>'complete','search_hotels'=>10+$index,
            'grouped_candidates'=>0,'expand_calls'=>0,'charter_concrete_candidates'=>0,
            'regular_concrete_candidates'=>1,'apd_batch_items'=>0,'apd_batch_calls'=>0,
            'apd_complete_offers'=>0,'final_price_ready_offers'=>0,'retryable_offers'=>0,
            'grouped_drained'=>true,'concrete_drained'=>true,'discovered_set_drained'=>true,
            'selection_authority'=>false,
        ];
    }
);
ck($range['status']==='incomplete'&&$range['windows_completed']===2&&count($range['windows'])===3,
    'range unknown continues later days without replay');
ck($rangeCalls===['2026-10-01','2026-10-02','2026-10-03'],'range unknown exact call order');

// Durable checkpoint state machine: started cannot be replayed; completed/supplier-error
// summaries are safely reusable; a different full-scope digest cannot reuse the id.
$cpRoot=sys_get_temp_dir().'/anytour-anex-range-checkpoint-'.bin2hex(random_bytes(8));
$scopeDigest=str_repeat('a',64);
try{
    $cp=new AnyTourAnexRangeCheckpointV1($cpRoot,'g2100000000',$scopeDigest);
    $start=$cp->begin('2026-10-01');
    ck($start===['action'=>'run'],'checkpoint first day starts');
    $unknown=$cp->begin('2026-10-01');
    ck(($unknown['action']??null)==='reuse'
        &&($unknown['result']['status']??null)==='unknown_no_replay'
        &&($unknown['result']['checkpoint_reused']??null)===true
        &&($unknown['result']['selection_authority']??null)===false,
        'checkpoint started becomes unknown no replay');

    $complete=new AnyTourAnexRangeCheckpointV1($cpRoot,'g2100000001',$scopeDigest);
    ck($complete->begin('2026-10-02')===['action'=>'run'],'checkpoint complete starts');
    $complete->finish('2026-10-02','complete',[
        'status'=>'complete','search_hotels'=>27,'final_price_ready_offers'=>19,
        'grouped_drained'=>true,'concrete_drained'=>true,'discovered_set_drained'=>true,
        'unsafe_supplier_id'=>'must-not-persist',
    ]);
    $completeRead=(new AnyTourAnexRangeCheckpointV1($cpRoot,'g2100000001',$scopeDigest))->begin('2026-10-02');
    ck(($completeRead['action']??null)==='reuse'
        &&($completeRead['result']['status']??null)==='complete'
        &&($completeRead['result']['search_hotels']??null)===27
        &&($completeRead['result']['final_price_ready_offers']??null)===19
        &&!array_key_exists('unsafe_supplier_id',$completeRead['result']),
        'checkpoint complete safe summary only');

    $error=new AnyTourAnexRangeCheckpointV1($cpRoot,'g2100000002',$scopeDigest);
    $error->begin('2026-10-03');
    $error->finish('2026-10-03','supplier_error',[
        'status'=>'supplier_error','error_code'=>'ANEX_HTTP_ERROR',
        'private'=>'must-not-persist',
    ]);
    $errorRead=(new AnyTourAnexRangeCheckpointV1($cpRoot,'g2100000002',$scopeDigest))->begin('2026-10-03');
    ck(($errorRead['result']['status']??null)==='supplier_error'
        &&($errorRead['result']['error_code']??null)==='ANEX_HTTP_ERROR'
        &&!array_key_exists('private',$errorRead['result']),
        'checkpoint supplier error terminal reuse');

    $scopeMismatch=false;
    try{new AnyTourAnexRangeCheckpointV1($cpRoot,'g2100000002',str_repeat('b',64));}
    catch(RuntimeException $e){$scopeMismatch=$e->getMessage()==='ANEX_RANGE_CHECKPOINT_SCOPE';}
    ck($scopeMismatch,'checkpoint scope mismatch fail closed');
}finally{
    if(is_dir($cpRoot)){
        $it=new RecursiveIteratorIterator(
            new RecursiveDirectoryIterator($cpRoot,FilesystemIterator::SKIP_DOTS),
...
echo "ANEX_RANGE_CHECKPOINT_OK unknown_no_replay=1 complete_reuse=1 supplier_error_reuse=1 scope_guard=1\\n";
