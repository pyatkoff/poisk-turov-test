...
$base=[
    'country_id'=>4,'country_name'=>'Турция','country_slug'=>'turciya',
...
$egypt=array_replace($base,[
    'candidate_type'=>'country','country_id'=>1,'country_name'=>'Египет','country_slug'=>'egipet',
    'observations_total'=>110,'observations_30d'=>70,'hotels_total'=>35,'hotels_30d'=>25,
]);
$antalya=array_replace($base,[
    'candidate_type'=>'resort','region_id'=>20,'region_name'=>'Анталья','region_slug'=>'antalya',
    'observations_total'=>75,'observations_30d'=>60,'hotels_total'=>18,'hotels_30d'=>16,
]);
$maldivesOctober=array_replace($base,[
    'candidate_type'=>'country_month','country_id'=>8,'country_name'=>'Мальдивы','country_slug'=>'maldivy',
...
$report=v2_seo_inventory_opportunity_report($rows,50,$bindings);
...
if(($report['controlled_route_match_count']??0)!==3) opp_fail('controlled_route_match_count');
...
if(($report['route_semantics']??'')!=='only_explicit_identity_registry_bindings_are_routes_catalog_slugs_are_hints_only') opp_fail('route_semantics');
...
if(($byKey[$turkeyKey]['review_path']??'')!=='/country/turkey/') opp_fail('country_route_binding');
if(($byKey[$turkeyKey]['catalog_path_hint']??'')!=='/country/turciya/') opp_fail('country_catalog_hint');
if(($byKey[$turkeyKey]['route_mapping_state']??'')!=='controlled_identity_registry_match') opp_fail('controlled_route_state');
if(($byKey[$turkeyKey]['path_exists_in_controlled_registry']??false)!==true) opp_fail('controlled_path_detection');

$egyptKey='country:country=1';
if(($byKey[$egyptKey]['review_path']??'')!=='/country/egypt/') opp_fail('egypt_route_binding');
if(($byKey[$egyptKey]['catalog_path_hint']??'')!=='/country/egipet/') opp_fail('egypt_catalog_hint');

$seasonalKey='country_month:country=8:period=2026-10';
if(!array_key_exists('review_path',$byKey[$seasonalKey])||$byKey[$seasonalKey]['review_path']!==null) opp_fail('unmapped_seasonal_route_not_null');
if(($byKey[$seasonalKey]['catalog_path_hint']??'')!=='/country/maldivy/october/') opp_fail('seasonal_catalog_hint');
if(($byKey[$seasonalKey]['route_mapping_state']??'')!=='unmapped_review_identity') opp_fail('seasonal_route_state');
if(($byKey[$seasonalKey]['path_period_semantics']??'')!=='year_specific_identity_requires_explicit_route_binding') opp_fail('seasonal_period_boundary');
...
$reverse=v2_seo_inventory_opportunity_report(array_reverse($rows),50,$bindings);
if(($reverse['evidence_sha256']??'')!==($report['evidence_sha256']??'')) opp_fail('nondeterministic_fingerprint');

$duplicate=$rows;
$duplicate[]=$turkey;
$dupeReport=v2_seo_inventory_opportunity_report($duplicate,50,$bindings);
...
echo "SEO_INVENTORY_OPPORTUNITY_SMOKE_OK candidates=4 controlled_routes=3 demand=unknown publication_candidates=0\n";
