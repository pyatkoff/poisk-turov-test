...
async function run(engine, width, height) {
  ...
  try {
    ...
    assert.equal(await direct.locator('#resultSummary').innerText(),'Найдено отелей: 1 · предложения нужно обновить','Rollover summary revokes the current-price claim with the offers');
    ...
    assert.equal(await direct.locator('#resultSummary').innerText(),'Найдено отелей: 1 · предложения нужно обновить','Expired profile-only summary does not claim a current-search price');
    ...
    assert.equal(await direct.locator('#results .direct-tour, #results .hotel-price').count(),0);
    assert.equal(await direct.locator('#resultSummary').innerText(),'Найдено отелей: 1 · предложения нужно обновить','Past-date profile summary remains truthful without offers');
