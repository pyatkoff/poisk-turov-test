...
putenv('MATCH_OPERATOR_ANCHOR_V5_TEST_LIBRARY=1');
require_once __DIR__.'/../scripts/diagnostics/hotel_match_operator_current_anchor_review_v5.php';
function t5(bool $ok,string $msg):void{if(!$ok)throw new RuntimeException($msg);}
t5(OCAR5_OP==='hotel-match-operator-current-anchor-review-1971-20260915-v5','operation id');
...
$r=ocar4_evaluate($pending,$anchors,$hotels);t5(count($r['safe'])===1&&($r['safe_by_operator']['operator_342']??0)===1,'helper policy unchanged');
echo "MATCH_OPERATOR_ANCHOR_V5_TEST_OK\n";
