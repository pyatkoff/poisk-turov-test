...
check(mod.EXPERIMENT=='anex_three_source_broad_price_20260912_v4');check(mod.SPEC['date']=='2026-10-27')

def row(provider,local,room,price,fuel=None):
    return {'provider':provider,'local_hotel_id':local,'external_hotel_id':str(local),'hotel_name':'Hotel','date':mod.SPEC['date'],'nights':8,'adults':1,'children':0,
        'meal_family':'ai','meal_key':'ai','meal_qualifiers':[],'meal_equivalence_verified':False,
        'meal_label':'AI','room':room,'room_norm':room.lower(),'placement':'SGL','placement_norm':'sgl','price':price,'currency':'RUB','fuel_charge':fuel,
...
check(report['triple_examples'][0]['key']['meal_key']=='ai')
