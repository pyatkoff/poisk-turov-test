...
$hot->execute([999,5,7,'AI','2026-09-17 00:40:00']); // no AnyTour bridge: excluded
$hot->execute([102,null,8,'UAI','2026-09-17 00:50:00']); // no operator identity: excluded
...
$obs->execute([999,5,12,'Other','2026-09-17 00:14:00']); // no bridge: excluded
$obs->execute([102,null,13,'No operator','2026-09-17 00:15:00']); // excluded

$mealId = (int)$db->query("SELECT id FROM anytour_meal_plans WHERE code='all-inclusive'")->fetchColumn();
...
$check(count($result['localMealPlans']) === 10, 'ten independent local meal definitions exposed for review');
$check($result['counts']['mealCandidates'] === 2, 'meal candidates scoped to bridged hotel and explicit operator');
$check($result['counts']['roomCandidates'] === 2, 'room candidates scoped to bridged hotel and explicit operator');
...
$check($meal5['hotelId'] === 1 && $meal5['scope']['hotelKey'] === '102', 'meal bound through current AnyTour source relation');
$check($meal5['sourceSha256'] === $sourceSha, 'exact source digest retained');
$check($meal5['reference'] === ['kind'=>'meal','keyKind'=>'code','externalKey'=>'7'], 'supplier meal id retained as exact code key');
$check($meal5['observedCount'] === 2 && $meal5['lastSeenAt'] === '2026-09-17 00:20:00', 'meal evidence frequency/freshness retained');
$check($meal5['labelConflict'] === true && $meal5['distinctLabels'] === 2, 'conflicting labels surfaced rather than normalized');
$check($meal5['decisionState'] === 'rejected', 'existing negative decision surfaced and not overwritten');
...
$check($roomCode['sampleLabel'] === 'Deluxe Sea View' && $roomCode['observedCount'] === 2, 'room label is evidence only and exact count retained');
$check(is_array($roomLabel) && $roomLabel['reference']['externalKey'] === 'Promo Room', 'room label used only when supplier id is absent');
$check($roomLabel['decisionState'] === 'unmapped', 'unreviewed room remains unmapped');
$check($result['counts']['unmappedMeals'] === 1 && $result['counts']['unmappedRooms'] === 2, 'unmapped queue counts are explicit');
...
$check($mealId > 0, 'fixture local meal target exists but was not auto-used');
