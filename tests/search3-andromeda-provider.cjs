...
(async()=>{
  ...
  await runtimeWindow.AnyTourAndromedaProvider.openDetail(retained.offerRef);
  await runtimeWindow.AnyTourAndromedaProvider.openDetail(retained.offerRef);
  assert.equal(detailCalls,1,'close/reopen uses retained detail without another request');
  assert.equal(renders.at(-1).items[0].tours.length,3,'details retain all offers');
  ...
  assert.equal(failed.retry,true);assert.equal(failed.status,'error');
  assert.doesNotMatch(failed.message,/истёк/);
  await runtimeWindow.AnyTourAndromedaProvider.openDetail(other.offerRef);
  runtimeWindow.fetch=async()=>({ok:true,json:async()=>({ok:true,data:{provider:'andromeda',offer_context:context,price:{amount:100,currency:'RUB'}}})});
  await runtimeWindow.AnyTourAndromedaProvider.openDetail(other.offerRef);
