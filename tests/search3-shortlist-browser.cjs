...
async function checkDisplayIdentity(browser, width) {
  ...
  try {
    ...
    assert.deepEqual(await shortlist.locator('.search3-shortlist-item__facts > div').filter({ has: page.locator('dt', { hasText: /^Номер$/ }) }).locator('dd').allTextContents(), ['Стандартный номер', 'Стандартный номер', 'Стандартный номер'], 'comparison displays one reviewed Russian room label while saved originals remain intact');
