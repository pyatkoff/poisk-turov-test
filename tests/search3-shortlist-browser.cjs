...
async function discloseMatchingOffers(page) {
  ...
  assert.match(minimumFacts, /12\.09\.2026/, 'collapsed shortlist entry identifies the matching minimum departure');
  assert.match(minimumFacts, /Ночей\s+9/, 'minimum nights remain exact');
  assert.match(minimumFacts, /Всё включено/, 'minimum meal follows the matching AI offer');
  assert.doesNotMatch(await card.locator('.hotel-tours').innerText(), /offer-standard|Biblioglobus|Tourvisor|STANDARD|FAMILY|DBL/, 'collapsed shortlist entry does not expose an offer ID, room, placement or provider');
...
async function checkJourney(browser, width) {
  ...
  try {
    ...
    await meal.selectOption('meal:label:без питания');
    await openComparison(page, width);
    const filteredSaved = page.locator('.search3-shortlist-item[data-offer-id="offer-standard"] .search3-shortlist-select');
    assert.equal(await filteredSaved.textContent(), 'Проверить тур', 'a uniquely loaded offer hidden only by local filters stays actionable');
    assert.equal(await filteredSaved.isEnabled(), true, 'local filtering never turns the exact current-search offer into a stale snapshot');
    assert.deepEqual(await page.evaluate(() => window.__shortlistCalls), [], 'classifying a locally filtered offer performs no supplier or detail request');
    await filteredSaved.click();
    await page.waitForFunction(() => window.V2TourController.currentTour?.id === 'offer-standard');
...
async function checkSearchRecovery(browser, width) {
  ...
  try {
    ...
    await page.waitForFunction(() => {const form=document.getElementById('tourSearch');if(!form)return false;const target=document.querySelector(form.dataset.search3Parameters==='compact'?'.search-parameter-dialog[open] [data-from]':'#tourSearch [name=dateFrom]');return target&&document.activeElement===target&&target.getAttribute('aria-invalid')==='true';});
...
async function checkDisplayIdentity(browser, width) {
  ...
  try {
    ...
    assert.deepEqual(await shortlist.locator('.search3-shortlist-item__facts > div').filter({ has: page.locator('dt', { hasText: /^Питание(?: · отличается)?$/ }) }).locator('dd').allTextContents(), ['AI', 'Всё включено', 'All Inclusive'], 'comparison preserves exact meal labels while saved originals remain intact');
    assert.deepEqual(await shortlist.locator('.search3-shortlist-item__facts > div').filter({ has: page.locator('dt', { hasText: /^Номер(?: · отличается)?$/ }) }).locator('dd').allTextContents(), ['STANDARD', 'Standard room', 'Стандартный номер'], 'comparison preserves exact room labels while saved originals remain intact');
