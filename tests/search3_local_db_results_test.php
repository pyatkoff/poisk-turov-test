...
function dto_fixture(string $provider,int $legacy,string $salt,string $price,string $checkin='2026-10-05',int $nights=7):array{
 ...
 $providerHotelRef=$provider==='andromeda'?'andromeda_catalog:'.$salt:'hotel:'.$provider.':'.$salt;
 ...
 'local_hotel_id'=>$legacy,'identity'=>['search_ref_digest'=>hash('sha256','search:'.$provider.':'.$salt),'offer_ref_digest'=>hash('sha256','offer:'.$provider.':'.$salt),'provider_hotel_ref_digest'=>hash('sha256',$providerHotelRef)],
...
$pdo->exec("CREATE TABLE andromeda_hotel_identities(id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,supplier_namespace VARCHAR(64) NOT NULL,external_hotel_id VARCHAR(120) NOT NULL,local_hotel_id BIGINT UNSIGNED NULL,decision_status VARCHAR(32) NOT NULL,KEY ix_external(external_hotel_id),KEY ix_tuple(supplier_namespace,external_hotel_id,decision_status)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
...
$pdo->prepare("INSERT INTO andromeda_hotel_identities(supplier_namespace,external_hotel_id,local_hotel_id,decision_status) VALUES('andromeda_catalog','sam',202,'accepted')")->execute();
