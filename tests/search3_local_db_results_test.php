...
need($result['providerOfferCounts']===['anex'=>1,'andromeda'=>0,'tourvisor'=>1],'provider count map includes zero-count known provider');
...
need($revoked['providerOfferCounts']===['anex'=>0,'andromeda'=>0,'tourvisor'=>0],'revoked scope keeps stable zero provider map');
...
need($restored['providerOfferCounts']===['anex'=>1,'andromeda'=>0,'tourvisor'=>1],'restored provider counts stable');
...
need($none['providerOfferCounts']===['anex'=>0,'andromeda'=>0,'tourvisor'=>0],'empty scope has explicit zero provider map');
need(json_encode($none['providerOfferCounts'],JSON_THROW_ON_ERROR)==='{"anex":0,"andromeda":0,"tourvisor":0}','empty provider counts encode as JSON object, never array');
