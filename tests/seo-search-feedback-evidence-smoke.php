...
$zeroClicks=$rows[0];
$zeroClicks['source_ref']='fixture://gsc/zero-clicks';
$zeroClicks['metrics']=['impressions'=>1000,'clicks'=>0,'avg_position'=>9.1,'ctr'=>0.0];
$zeroClicksIntake=v2_seo_search_feedback_intake([$zeroClicks],$now);
if(($zeroClicksIntake['state']??'')!=='search_feedback_intake_ready')feedback_fail('zero_clicks_valid_ctr_blocked');

$badCtrConsistency=$rows[0];
$badCtrConsistency['source_ref']='fixture://gsc/inconsistent-ctr';
$badCtrConsistency['metrics']['ctr']=0.01;
$blocked=v2_seo_search_feedback_intake([$badCtrConsistency],$now);
if(($blocked['state']??'')!=='search_feedback_intake_blocked')feedback_fail('inconsistent_ctr_not_blocked');
$badCtrErrors=$blocked['rows'][0]['errors']??[];
if(!in_array('ctr_inconsistent_with_clicks_impressions',$badCtrErrors,true))feedback_fail('inconsistent_ctr_error_missing');
...
echo "SEO_SEARCH_FEEDBACK_EVIDENCE_OK launched=10 observedFixture=2 missingUnknown=8 seasonal=2 zeroImpressionsUnknown=1 zeroClicks=1 ctrConsistency=1 autoRecommendation=0 autoDeindex=0 hotelTours=0\n";
