...
function feedback_fail(string $message): void
{
    fwrite(STDERR,"SEO_SEARCH_FEEDBACK_EVIDENCE_FAIL:$message\n");
    exit(1);
}
...
$rows=[
    [
        ...
        'source_ref'=>'fixture://gsc/export-1',
    ...
    [
        ...
        'metrics'=>['impressions'=>500,'clicks'=>35,'avg_position'=>8.2,'ctr'=>0.07],
    ],
];
...
foreach(['automatic_recommendation_allowed','automatic_deindex_allowed','publication_allowed','indexation_change_allowed','sitemap_change_allowed','canonical_change_allowed','route_change_allowed','hotel_tours_indexation_allowed'] as $flag){
    if(($intake[$flag]??true)!==false) feedback_fail('boundary_'.$flag);
}
...
foreach(['search_contract_changes','tourvisor_contract_changes','pricing_contract_changes','lead_contract_changes','metrika_contract_changes'] as $flag){
    if(($intake[$flag]??true)!==false) feedback_fail('contract_'.$flag);
}

$hotel=$rows[0];
$hotel['path']='/country/turkey/hotel/aegean-park-1601/';
$blocked=v2_seo_search_feedback_intake([$hotel],$now);
if(($blocked['state']??'')!=='search_feedback_intake_blocked') feedback_fail('hotel_scope_not_blocked');

$stale=$rows[0];
$stale['collected_at_epoch']=$now-8*86400;
$stale['period_end_epoch']=$stale['collected_at_epoch']-3600;
$stale['period_start_epoch']=$stale['period_end_epoch']-7*86400;
$blocked=v2_seo_search_feedback_intake([$stale],$now);
if(($blocked['state']??'')!=='search_feedback_intake_blocked') feedback_fail('stale_not_blocked');

$badCtr=$rows[0];
$badCtr['metrics']['ctr']=1.2;
$blocked=v2_seo_search_feedback_intake([$badCtr],$now);
