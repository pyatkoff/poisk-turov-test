...
require_once __DIR__.'/../scripts/catalog/anytour_offer_live_acceptance.php';
...
accept_add_offer($offer,1,102,'tourvisor',$scopeTv,$tvNew,110000,2,$tvIdentity); // running refresh: must stay hidden
accept_add_offer($offer,2,106,'anex',$scopeAnex,$anexToken,130000,3,str_repeat('e',64));
accept_add_offer($offer,4,999,'andromeda',$scopeAnd,$andToken,140000,4,str_repeat('f',64)); // no current bridge: withheld
...
echo "ANYTOUR_OFFER_LIVE_ACCEPTANCE_TEST_OK stored=4 visible=2 running_hidden=1 bridge_withheld=1 writes=0 supplier_calls=0\n";
