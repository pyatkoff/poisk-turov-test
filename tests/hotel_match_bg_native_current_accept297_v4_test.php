...
require getenv('MATCH_SCRIPT_PATH') ?: __DIR__.'/../scripts/diagnostics/hotel_match_bg_native_current_accept297_v4.php';
...
$input=$argv[1]??''; $source=loadj($input.'/alias-output.json');
$supported=array_values(array_filter($source['rows'],fn($r)=>($r['alias_evidence_supported']??false)===true));
need(count($supported)===297,'real_supported_count');
need(count(array_filter($supported,fn($r)=>count($r['input_row']['canonical_evidence'])===1))===207,'single_anchor_source_count');
need(count(array_filter($supported,fn($r)=>count($r['input_row']['canonical_evidence'])>1))===90,'multi_anchor_source_count');
$s=seeds($input);need(count($s)===297,'real_cohort_count');
...
$root=sys_get_temp_dir().'/match-bg-native-accept297-'.bin2hex(random_bytes(6));mkdir($root,0700);$results=[];
foreach(['success','manual','source_rejected','target_occupied','canonical_multi','anchor_changed','below_threshold','insert_error','commit_unknown','commit_false','post_corrupt','unknown_guard'] as $case) {
    $i=$identities;$h=$hotels;$m=$manual;$expectedCount=297;
    ...
    if($case==='source_rejected'||$case==='below_threshold'){
        $n=$case==='below_threshold'?200:1;$expectedCount-= $n;
...
$out=['state'=>'offline_tests_passed','candidate_count'=>count($s),'full_predicate_matches'=>$good,'source_single_anchor'=>207,'source_multi_anchor_current_hold_candidates'=>90,'raw_bg_namespace_only'=>true,'operator_115_writes'=>0,'transaction_cases'=>$results,'exclusive_no_replay'=>true,'fixture_only'=>true,'live_database_writes'=>0,'fixture_output'=>$root];
