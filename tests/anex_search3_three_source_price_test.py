...
report=mod.compare(results); check(report['same_subject_across_completed_cases'] is True); check(report['aligned_three_source_tour_count']==1)
check(report['aligned_three_source_examples'][0]['identical_supplier_package_verified'] is False); check(report['aligned_three_source_examples'][0]['fuel_inclusion_verified'] is False)
...
failure=mod.transport_failure(SSHBatchError('SECRET HOST STDERR')); check(failure['status']=='transport_unconfirmed'); check('SECRET' not in str(failure))
