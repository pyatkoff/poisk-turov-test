...
form.querySelector = selector => ({ '[data-home-children]': childCount, '[data-home-child-ages]': childBox, '.at-home-search__more': more, 'button[type="submit"]': submit })[selector];
form.reportValidity = () => !country.required || (!country.disabled && country.value !== '');

class FormDataStub {
  constructor() { this.entries = [['from', departure.value], ...(!country.disabled ? [['country', country.value]] : []), ...Object.entries(values)]; }
...
const fetch = url => {
  ...
  if (action === 'departures') return Promise.resolve({ ok: true, json: async () => [{ id: 1, name: 'Москва' }, { id: 2, name: 'Казань' }] });
  return new Promise(resolve => pendingCountries.push(items => resolve({ ok: true, json: async () => items })));
};
const document = {
...
(async () => {
  ...
  pendingCountries[0]([{ id: 4, name: 'Турция' }]); await tick(); await tick();
