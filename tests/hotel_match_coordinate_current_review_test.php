...
mcc_t(($r['route']??'')==='auto_accept_candidate','exact coordinate/name must accept');
mcc_t((int)($r['target']??0)===1,'exact target');
mcc_t(($r['reason']??'')==='coordinate_supported_exact_name','exact reason');

$r=mcc_candidate(['Alpha Beach'],[['latitude'=>36.0000,'longitude'=>30.0000]],4,$hotels,$forms,$grid);
mcc_t(($r['route']??'')==='auto_accept_candidate','strong name isolated coordinate must accept');
mcc_t((int)($r['target']??0)===1,'strong target');
