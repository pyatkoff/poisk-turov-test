...
$json=json_decode(implode("\n",$out),true);if(!is_array($json)||($json['hold_count']??0)!==2)seasonal_dossier_cli_smoke_fail('missing_identity_hold');
$checked=$now-30;$expires=$now+3600;$freshness=$expires-$checked;
$inventory=v2_seo_seasonal_identity_inventory([
 ['page_type'=>'resort_month','page_key'=>'resort_month:1:4:20:2026-09','country_id'=>4,'region_id'=>20,'departure_id'=>1,'departure_year'=>2026,'departure_month'=>9,'offer_count'=>3,'freshness_seconds'=>$freshness,'evidence_checked_at_epoch'=>$checked,'expires_at_epoch'=>$expires,'observed_at'=>gmdate('c',$checked),'expires_at'=>gmdate('c',$expires)],
 ['page_type'=>'month','page_key'=>'month:1:8:2026-09','country_id'=>8,'region_id'=>null,'departure_id'=>1,'departure_year'=>2026,'departure_month'=>9,'offer_count'=>2,'freshness_seconds'=>$freshness,'evidence_checked_at_epoch'=>$checked,'expires_at_epoch'=>$expires,'observed_at'=>gmdate('c',$checked),'expires_at'=>gmdate('c',$expires)],
],$now);
...
$json=json_decode(implode("\n",$out),true);if(!is_array($json)||($json['go_review_count']??0)!==2||($json['publication_candidates']??null)!==[])seasonal_dossier_cli_smoke_fail('go_output');
echo "SEO_SEASONAL_DOSSIER_CLI_OK holdWithoutInventory=2 goWithInventory=2 publication=0\n";
