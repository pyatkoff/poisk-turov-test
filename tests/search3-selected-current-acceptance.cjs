...
async function checkLongFlightPrice(page, width) {
  ...
  assert.equal(facts.cells.find(cell => cell.label.text === 'Номер').value.text, item.roomType, 'long supplier room fact stays complete without an unproven translation');
...
async function run(browser, width) {
  ...
  try {
    ...
    assert.deepEqual(detail.facts.map(item => item.value), ['Москва', '05.10.2026', '9', '2 взр. + 1 дет.', 'Всё включено',
      tour.roomType, 'DBL + CHD', 'Библио-Глобус', 'Чартер', 'без доплаты'], 'selected tour retains exact offer facts and reuses the canonical operator identity used by results and Compare');
