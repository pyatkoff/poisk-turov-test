...
$country = [
    'type'=>'country','path'=>'/country/turkey/',
    'data'=>['search_state'=>['country'=>4]],
];
$resorts = [
...
$base = static fn(array $x): array => array_merge([
    'state'=>'fresh_review_identity','page_type'=>'month','country_id'=>4,'region_id'=>null,
    'departure_id'=>1,'year'=>2026,'month'=>10,'freshness_seconds'=>7200,
...
$out=v2_seo_seasonal_family_binding($country,$resorts,$inventory);
...
foreach(['unregistered_region_identity','country_identity_mismatch','duplicate_page_key'] as $required){
    if(!in_array($required,$errors,true))seasonal_binding_fail($required);
}

$badResorts=$resorts; $badResorts[]=['type'=>'resort','path'=>'/country/turkey/fake/','data'=>['search_state'=>['country'=>4,'region'=>77]]];
try { v2_seo_seasonal_family_binding($country,$badResorts,$inventory); seasonal_binding_fail('duplicate_region_not_rejected'); }
catch(InvalidArgumentException $e){}
$unsafe=$inventory; $unsafe['publication_allowed']=true;
try { v2_seo_seasonal_family_binding($country,$resorts,$unsafe); seasonal_binding_fail('unsafe_inventory_not_rejected'); }
catch(InvalidArgumentException $e){}

echo "SEO_SEASONAL_BINDING_OK bound=2 blocked=3 registeredRegions=2 publication=0\n";
