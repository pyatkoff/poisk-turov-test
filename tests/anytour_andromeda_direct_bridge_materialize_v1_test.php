...
require_once __DIR__ . '/../scripts/catalog/anytour_andromeda_direct_bridge_materialize_v1.php';
...
$owns=[];
foreach([[101,'Alpha',1],[202,'Inactive',0],[303,'Existing',1]] as[$local,$name,$active]){
...
// Existing valid direct binding must be skipped by the operation planner.
$existing=AnyTourProviderIdentityBridgeV1::materializeAcceptedAndromeda($db,[['supplier_namespace'=>'operator_315','external_hotel_id'=>'8001']],new DateTimeImmutable('2026-09-17T15:00:00Z'));
op_need($existing['created']===1&&$existing['verified']===1,'fixture-existing-direct');
$before=anytour_andromeda_direct_count($db);op_need($before===1,'before-one-direct');
$planned=anytour_andromeda_direct_candidates($db,1000);
op_need(count($planned)===1,'only-one-eligible');
op_need($planned[0]['supplier_namespace']==='andromeda_catalog'&&$planned[0]['external_hotel_id']==='7001','eligible-exact-tuple');
op_need($planned[0]['local_hotel_id']===101&&$planned[0]['anytour_hotel_id']===$owns[101],'eligible-exact-target');
...
op_need($result['directBefore']===1&&$result['directAfter']===2,'exact-count-delta');
...
$digest=AnyTourProviderIdentityBridgeV1::providerRefDigest('andromeda_catalog:7001');
...
op_need(is_array($saved)&&(int)$saved['anytour_hotel_id']===$owns[101]&&$saved['acquired_via']==='match_accepted_bridge','direct-row-target');

// Re-running the pure operation function finds no new cohort; the live runner itself is NO-REPLAY by operation directory/journal.
...
// Current MATCH revocation makes the direct row unusable; no fallback through legacy is allowed for a present direct digest.
$db->exec("UPDATE andromeda_hotel_identities SET decision_status='pending' WHERE external_hotel_id='7001'");
$visible=AnyTourProviderIdentityBridgeV1::filterOfferRows($db,[['provider'=>'andromeda','provider_hotel_ref_digest'=>$digest,'legacy_hotel_id'=>101,'anytour_hotel_id'=>$owns[101]]]);
op_need($visible===[],'revocation-fail-closed');

echo "ANYTOUR_ANDROMEDA_DIRECT_BRIDGE_OPERATION_OK selected=1 created=1 existing_skipped=1 inactive_hidden=1 ambiguous_hidden=1 revoked_hidden=1\n";
