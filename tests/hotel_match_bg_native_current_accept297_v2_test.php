...
require getenv('MATCH_SCRIPT_PATH') ?: __DIR__.'/../scripts/diagnostics/hotel_match_bg_native_current_accept297_v2.php';
...
$input=$argv[1]??''; $s=seeds($input);need(count($s)===297,'real_cohort_count');
...
foreach(['success','manual','source_rejected','target_occupied','anchor_changed','below_threshold','insert_error','commit_unknown','commit_false','post_corrupt','unknown_guard'] as $case) {
...
$out=['state'=>'offline_tests_passed','candidate_count'=>count($s),'full_predicate_matches'=>$good,'raw_bg_namespace_only'=>true,'operator_115_writes'=>0,'transaction_cases'=>$results,'exclusive_no_replay'=>true,'fixture_only'=>true,'live_database_writes'=>0,'fixture_output'=>$root];
