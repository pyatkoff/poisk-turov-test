...
$assert(anex_concrete_fuel_input($spec)===$spec,'exact known-inventory scenario');
$bad=$spec;$bad['date']='2026-10-19';
try{anex_concrete_fuel_input($bad);$assert(false,'sealed empty date accepted');}catch(RuntimeException $e){$assert($e->getMessage()==='CONCRETE_FUEL_INVALID_INPUT','old case not replayed');}
...
foreach(['->bron(','bron_ticket','broninit(','->calc(','AnyTourAnexAdditionalPricesClient','ANEX_B2B_TOKEN','INSERT INTO anex_hotel_search_mappings','UPDATE anex_hotel_search_mappings'] as $forbidden)$assert(strpos($php,$forbidden)===false,'forbidden '.$forbidden);
$assert(strpos($php,'->expand(')!==false&&strpos($php,'->flights(')!==false,'existing concrete runtime consumers used');
$assert(strpos($php,"ANEX_CONCRETE_FUEL_PROGRAM = '2637'")!==false,'program2637 pinned');
