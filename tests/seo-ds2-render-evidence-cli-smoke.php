...
foreach(['destination','hotel_tours'] as $family)foreach([375,430,768,1024,1440] as $width){
    $row=[
        'family'=>$family,
...
$rows[5]['noindex_ok']=true;$rows[0]['secondary_search_handoff_contract_ok']=false;$tmp=tempnam(sys_get_temp_dir(),'ds2-render-');file_put_contents($tmp,json_encode(['rows'=>$rows],JSON_THROW_ON_ERROR));$out=[];$code=0;exec('php '.escapeshellarg($cli).' --evidence='.escapeshellarg($tmp).' --now-epoch='.$now.' --require-ready 2>&1',$out,$code);@unlink($tmp);if($code!==3)fail_ds2_cli('secondary_handoff_boundary');
echo "SEO_DS2_RENDER_EVIDENCE_CLI_OK captures=10 checks=10 hotelTours=noindex_review_only\n";
