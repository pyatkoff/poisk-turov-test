...
async function checkToolbarLayout(page, width, previous) {
  ...
  if (width > 600) {
    ...
    assert.ok(closed.sort.y >= closed.edit.y + closed.edit.height && Math.abs(closed.summary.y - closed.sort.y) < 2, `mobile sort and filters share the readable second row: ${JSON.stringify(closed)}`);
