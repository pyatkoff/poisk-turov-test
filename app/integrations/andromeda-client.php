...
final class AnyTourAndromedaClient
{
    ...
    /**
     * Explicit capture of one booking package; this does not call the booking method.
     * claiminc is an opaque private ID, resolved by the caller from its saved offer.
     * The caller must reserve the operation durably before invoking this method.
     * This instance prevents a second attempt, including after an unknown outcome.
     * Return value is private raw evidence, NOT a browser response or verified quote.
     * See https://dokuwiki.samo.ru/doku.php?id=andromeda:bron
     */
    public function package(string $claiminc): array
    {
        ...
        // Local input budget, not a supplier ID format or limit. Never trim/cast/decode IDs.
        ...
        // Do not equate catalogKey with PRICES.id, select a price, or call calc/get_flights.
        // Their relationship and the public allowlist require a verified captured package.
        return $reply;
    }
