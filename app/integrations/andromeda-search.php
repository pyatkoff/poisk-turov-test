...
final class AnyTourAndromedaSearch {
    ...
    public function __construct(array &$privateState, callable $save, bool $enabled = false) {
    ...
    public function start(array $criteria, string $searchRef, int $generation, int $now,
        ...
        if (!self::supports($criteria)) throw new InvalidArgumentException('ANDROMEDA_SEARCH_UNSUPPORTED');
    ...
    public function resume(string $searchRef, int $generation, int $now): array {
        ...
        $s=$this->state;
        if (!is_array($s['store']??null) || !self::supports($s['criteria']??[])
