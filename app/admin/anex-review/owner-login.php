...
final class AnexReviewOwnerLogin
{
    ...
    public function __construct(string $configFile, string $documentRoot)
    {
        ...
        if (!$root || !$file || $file !== $configFile || is_link($file) || !is_file($file)
            ...
            || (fileperms($file) & 0777) !== 0600) throw new RuntimeException('owner_config_unavailable', 503);
        ...
        $this->auth = new AnexReviewOwnerAuth($config['private_directory'], $root);
...
function anex_owner_login_html(string $csrf, string $nonce, string $error = '', bool $authenticated = false): string
{
