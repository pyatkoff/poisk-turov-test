...
async function footerCheck(page){const state=await page.evaluate(()=>{const f=document.querySelector('.ds2-site-footer');if(!f)return null;const c=getComputedStyle(f);return{background:c.backgroundColor,color:c.color}});report.footer=state;if(!state)throw new Error('mobile footer missing');if(state.background==='rgba(0, 0, 0, 0)'||state.background==='rgb(255, 255, 255)')throw new Error(`mobile footer is not dark: ${state.background}`)}
...
(async()=>{
 ...
 try{
  ...
  await settleImages(page,'#results .hotel-card img');await snap(page,'m02-results','#resultsTools');
  ...
  if(!await visible(page,'#selectedTour .tour-flights',30000))throw new Error('mobile flights missing');await snap(page,'m04-flights','#selectedTour .tour-flights');
  if(!await clickVisible(page,'#selectedTour .search3-flight-continue button'))throw new Error('mobile flight continue missing');if(!await visible(page,'#selectedTour.search3-final-review',10000))throw new Error('mobile final review missing');await snap(page,'m05-final-review','#selectedTour');
