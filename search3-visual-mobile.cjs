...
(async()=>{
 ...
 try{
  ...
  report.leadSelectionSummary=await page.locator('#selectedTour .lead-selection-summary').innerText().catch(()=>'');if(!report.leadSelectionSummary||/рейсом\d|умолчаниюТуда|уточняютсяСамая/.test(report.leadSelectionSummary))throw new Error(`mobile lead selection summary is not readable: ${report.leadSelectionSummary}`);
