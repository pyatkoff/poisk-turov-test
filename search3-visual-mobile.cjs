...
(async()=>{
 ...
 try{
  ...
  report.resultCardContract=await page.evaluate(()=>{const card=document.querySelector('#results .hotel-card');if(!card)return null;return{highlightCount:card.querySelectorAll('.search3-hotel-highlight').length,width:card.getBoundingClientRect().width,viewport:innerWidth,documentWidth:document.documentElement.scrollWidth};});
  ...
  await snap(page,'m02-results','#results .hotel-card');
