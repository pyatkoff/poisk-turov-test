...
(async()=>{
 ...
 try{
  ...
  if(!await visible(page,'#selectedTour .tour-flights',30000))throw new Error('flights missing');await snap(page,'05-flights','#selectedTour .tour-flights');
  if(!await visible(page,'#selectedTour .search3-flight-continue button',6000))throw new Error('flight continue render missing');if(!await clickVisible(page,'#selectedTour .search3-flight-continue button'))throw new Error('flight continue missing');if(!await visible(page,'#selectedTour.search3-final-review',10000))throw new Error('final review missing');await snap(page,'06-final-review','#selectedTour');
