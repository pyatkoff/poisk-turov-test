...
function lead_bool($value):bool{return filter_var($value,FILTER_VALIDATE_BOOLEAN);}
...
function lead_same_origin():bool{$origin=trim((string)($_SERVER['HTTP_ORIGIN']??''));if($origin==='')return true;$host=strtolower((string)($_SERVER['HTTP_HOST']??''));$originHost=strtolower((string)(parse_url($origin,PHP_URL_HOST)??''));return $host!==''&&$originHost===$host;}
...
function lead_build(array $data):array{
    ...
    if($lead['searchId'])$details[]='searchId: '.$lead['searchId'];if($lead['hotel'])$details[]='Отель: '.$lead['hotel'];if($lead['region'])$details[]='Регион: '.$lead['region'];if($lead['meal'])$details[]='Питание: '.$lead['meal'];if($lead['roomType'])$details[]='Номер: '.$lead['roomType'];if($lead['placement'])$details[]='Размещение: '.$lead['placement'];if($lead['operator'])$details[]='Оператор: '.$lead['operator'];if($lead['price'])$details[]='Цена тура: '.$lead['price'].' '.$lead['currency'];if($lead['flight'])$details[]='Выбранный перелёт: '.$lead['flight'];if($lead['flightPrice'])$details[]='Цена варианта перелёта: '.$lead['flightPrice'].' '.$lead['currency'];if($lead['flightFuel'])$details[]='Топливный сбор: '.$lead['flightFuel'].' '.$lead['currency'];if($lead['comment'])$details[]='Комментарий пользователя: '.$lead['comment'];if($lead['page'])$details[]='Страница: '.$lead['page'];
