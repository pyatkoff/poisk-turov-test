...
window.addEventListener('v2:results-rendered',function(e){if(applying)return;var items=e&&e.detail&&Array.isArray(e.detail.items)?e.detail.items:[];if(lastApplied.length&&sameRefs(items,lastApplied)){updateCount(items.length);return;}cancelPriceApply();source=items.slice();lastApplied=[];renderRail(false);if(source.length&&activeCount())apply();else announce(source.length);});
