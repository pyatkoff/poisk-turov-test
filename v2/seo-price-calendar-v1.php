...
function v2_seo_render_price_calendar(array $calendar, array $searchState, string $title = 'Цены по датам вылета'): string
{
    ...
    foreach ((array)($calendar['series'] ?? []) as $point) {
        ...
        $items[] = '<a class="sp-price-day'.($best?' sp-price-day--best':'').'" href="'.sp_e($href).'">'
            ...
            .'<strong>'.sp_e($price).'</strong>'
            .'<small>'.($hotelCount > 0 ? sp_e('найдено в ' . $hotelCount . ' отелях') : 'свежая цена').'</small>'
            .($best?'<span class="sp-price-day__badge">лучшая</span>':'')
            .'</a>';
    }
    if ($items === []) return '';
    ...
    return '<section class="sp-card sp-price-calendar" data-seo-price-calendar>'
        ...
        .'<div class="sp-price-calendar__grid">'.implode('', $items).'</div>'
        .'<p class="sp-price-calendar__note">Это минимальные цены, которые AnyTour находил в свежих наблюдениях. Отсутствие цены означает отсутствие свежего наблюдения, а не отсутствие туров. Стоимость и доступность перепроверяются перед заявкой.</p>'
        .'</section>';
}
