...
function ensure(){const r=root();if(!r||r.hidden)return null;let s=r.querySelector('.search3-booking-stepper');if(s)return s;s=document.createElement('nav');s.className='search3-booking-stepper';s.setAttribute('aria-label','Этапы оформления тура');s.innerHTML='<button type="button" class="search3-booking-step is-active" data-step="hotel" aria-current="step"><span>1</span><b>Отель</b></button><button type="button" class="search3-booking-step" data-step="room"><span>2</span><b>Номер и питание</b></button><button type="button" class="search3-booking-step" data-step="flight"><span>3</span><b>Рейсы</b></button><button type="button" class="search3-booking-step" data-step="tourists"><span>4</span><b>Туристы</b></button><button type="button" class="search3-booking-step" data-step="confirm"><span>5</span><b>Подтверждение</b></button>';const back=r.querySelector('.back-results');if(back&&back.nextSibling)r.insertBefore(s,back.nextSibling);else r.prepend(s);return s}
...
function target(step){const r=root();if(!r)return null;if(step==='room')return r.querySelector('.facts,.hotel-desc')||r;if(step==='flight')return r.querySelector('.tour-flights')||r;if(step==='tourists'||step==='confirm')return r.querySelector('.search3-lead-shell,.lead-form')||r;return r.querySelector('.selected-head,.selected-picture')||r}
