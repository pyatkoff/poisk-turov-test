...
function v2_seo_generated_resort_launch_enabled(): bool
{
    return v2_seo_core_resort_launch_manifest()!==[];
}
...
function v2_seo_generated_resort_review_record(string $path): array
{
    ...
    $record=$rows[$path];
    $public=v2_seo_generated_resort_launch_enabled()&&in_array($path,v2_seo_core_resort_launch_paths(),true);
    $record['status']=$public?'approved':'review';
    $record['publication_allowed']=$public;$record['indexation_allowed']=$public;$record['sitemap_allowed']=$public;$record['route_launch_allowed']=$public;
    return $record;
}
...
function v2_seo_generated_resort_month_review_record(string $basePath,int $monthNo,?int $nowEpoch=null): array
{
    ...
    $h1=$baseH1.' в '.$month['prep'];$period=sprintf('%04d-%02d',$year,$monthNo);$public=v2_seo_generated_resort_launch_enabled()&&in_array($path,v2_seo_core_resort_launch_paths(),true);
    return ['id'=>'seasonal.generated.'.$countryId.'.'.$regionId.'.'.$monthNo.'.v1','status'=>$public?'approved':'review','path'=>$path,'type'=>'seasonal','data'=>[
        ...
        'publication_allowed'=>$public,'indexation_allowed'=>$public,'sitemap_allowed'=>$public,'route_launch_allowed'=>$public];
}
