...
const seen=new Map(),hotels=new Map();
...
function hotelMeta(h){h=h||{};return{hotelId:h.id,name:h.name,price:h.price,country:h.country&&h.country.name,region:h.region&&h.region.name};}
...
window.addEventListener('v2:results-rendered',function(e){const items=e.detail&&Array.isArray(e.detail.items)?e.detail.items:[];items.forEach(function(h){const id=cleanId(h&&h.id);if(id)hotels.set(id,hotelMeta(h));});});
...
window.addEventListener('v2:analytics',function(e){const d=e.detail||{};if(d.name!=='hotel_open')return;const p=d.params||{},id=cleanId(p.hotelId),meta=hotels.get(id)||{};pushDetail(Object.assign({},meta,{hotelId:id}));});
