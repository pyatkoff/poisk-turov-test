<?php
/** V2-only lead adapter. Prepares and, when explicitly enabled, writes a lead directly to the existing AnyTour Bitrix lead iblock. */
...
function lead_build(array $data):array{
    ...
    $lead=[
        ...
        'yclid'=>lead_text($data['yclid']??'',100),
        'yaclient'=>lead_text($data['yaclient']??'',100),
        'utm_source'=>lead_text($data['utm_source']??'',160),
        'utm_medium'=>lead_text($data['utm_medium']??'',160),
        'utm_campaign'=>lead_text($data['utm_campaign']??'',160),
        'utm_content'=>lead_text($data['utm_content']??'',160),
        'utm_term'=>lead_text($data['utm_term']??'',160),
    ];
    ...
    $details=[
        'Имя: '.$lead['name'],
        'Телефон: '.$lead['phone'],
        'Город вылета: '.$lead['departure'],
        'Страна: '.$lead['country'],
        'Туристы: '.$people,
        'Даты вылета: '.$lead['date'],
        'Количество ночей: '.(string)$lead['nights'],
        'V2 Tourvisor tourId: '.$lead['tourId'],
    ];
    ...
    if($lead['price'])$details[]='Цена: '.$lead['price'].' '.$lead['currency'];
    ...
    $properties=[
        ...
        'NAME'=>$lead['name'],
        'PHONE'=>$lead['phone'],
        'COMMENTS'=>implode('; ',$details),
        'DEPARTURE'=>$lead['departure'],
        'PEOPLE'=>$people,
        'COUNTRY'=>$lead['country'],
        'STATUS'=>V2_LEAD_STATUS_ID,
        'SOURCE'=>V2_LEAD_SOURCE_ID,
        'YA_CLIENT'=>$lead['yaclient'],
        'YA_CLID'=>$lead['yclid'],
        'YA_UTM_SOURCE'=>$lead['utm_source'],
        'YA_UTM_MEDIUM'=>$lead['utm_medium'],
        'YA_UTM_CAMPAIGN'=>$lead['utm_campaign'],
        'YA_UTM_CONTENT'=>$lead['utm_content'],
        'YA_UTM_TERM'=>$lead['utm_term'],
    ];
    ...
    return [
        ...
        'element'=>[
            'IBLOCK_ID'=>V2_LEAD_IBLOCK_ID,
            'IBLOCK_SECTION_ID'=>V2_LEAD_SECTION_ID,
            'PROPERTY_VALUES'=>$properties,
            'NAME'=>'Заявка от '.$createdAt,
            'ACTIVE'=>'Y',
        ],
    ];
}
...
if($_SERVER['REQUEST_METHOD']==='GET'){
    if(isset($_GET['selftest'])){
        $sample=lead_build([
            'name'=>'V2 Selftest','phone'=>'8 (999) 123-45-67','tourId'=>'tv-test-1','searchId'=>'42',
            'hotel'=>'Test Hotel','country'=>'Турция','region'=>'Анталья','departure'=>'Москва','date'=>'15.09.2026',
            'nights'=>7,'adults'=>2,'childs'=>0,'meal'=>'AI','roomType'=>'Standard','placement'=>'DBL',
            'operator'=>'Test Operator','price'=>150000,'comment'=>'direct writer self-test'
        ]);
        $p=$sample['properties']??[];
        $checks=[
            'validPhone'=>(($sample['lead']['phone']??'')==='+79991234567'),
            'iblock'=>(($sample['element']['IBLOCK_ID']??0)===V2_LEAD_IBLOCK_ID),
            'section'=>(($sample['element']['IBLOCK_SECTION_ID']??0)===V2_LEAD_SECTION_ID),
            'status'=>(($p['STATUS']??0)===V2_LEAD_STATUS_ID),
            'source'=>(($p['SOURCE']??0)===26),
            'tourDetails'=>(strpos((string)($p['COMMENTS']??''),'tv-test-1')!==false&&strpos((string)($p['COMMENTS']??''),'Test Hotel')!==false),
        ];
    ...
    lead_out(['ok'=>true,'adapter'=>'v2-direct-bitrix-lead','mode'=>'dry-run','writes'=>false,'config'=>['iblock'=>4,'section'=>12,'status'=>9,'source'=>26],'selftest'=>'?selftest=1']);
}
...
$built=lead_build($data);
if(!empty($built['errors']))lead_out(['ok'=>false,'error'=>'Validation failed','fields'=>$built['errors']],422);

$commit=((string)($_GET['commit']??'')==='1');
if(!$commit){
    lead_out(['ok'=>true,'mode'=>'dry-run','writes'=>false,'lead'=>$built['lead'],'config'=>['iblock'=>4,'section'=>12,'status'=>9,'source'=>26],'element'=>$built['element']]);
}

$boot=lead_bootstrap();
if(empty($boot['ok']))lead_out(['ok'=>false,'error'=>$boot['error']??'Bitrix bootstrap failed'],500);
$el=new \CIBlockElement();
$leadId=$el->Add($built['element']);
