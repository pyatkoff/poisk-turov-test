...
function parse(data){
 ...
 for(const group of data.hotels){
  ...
  const offers=[];for(const stored of group.offers){
   ...
   const tour=offerTour(stored),legacyHotelId=positiveId(stored&&stored.legacyHotelId);if(!tour||!legacyHotelId)return null;
...
const api=Object.freeze({localCandidate,endpoint,offerTour,parse,apply,version:3});root.AnyTourLocalDbProviderV1=api;
