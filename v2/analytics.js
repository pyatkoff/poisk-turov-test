...
const SAFE_KEYS=new Set(['searchId','durationMs','resultsCount','progress','minPrice','sort','tourId','hotelId','hotel','region','country','price','currency','flightIndex','flightNumber','isDefault','continuedResultsCount','phase','errorCode','source']);
...
window.addEventListener('v2:search-progress',e=>{const d=e.detail||{};if(Number(d.progress||0)===100)return;});
...
window.addEventListener('v2:results-rendered',e=>{const d=e.detail||{};if(d.sort)track('results_rendered',{resultsCount:Array.isArray(d.items)?d.items.length:0,sort:String(d.sort)});});
...
document.addEventListener('click',e=>{const b=e.target&&e.target.closest&&e.target.closest('.hotel-info-toggle');if(!b)return;const card=b.closest('.hotel-card');track('hotel_details_opened',{hotelId:String(card&&card.dataset.hotelId||'')});},true);
window.V2Analytics={track,safeParams,version:1,metrikaCounter:metrikaCounter()};
})();