 ...
 * These records are not routes and are intentionally not publication candidates.
 */
function v2_seo_seasonal_review_content_prototypes(): array
{
    return [
        'antalya-september' => [
            'state'=>'authored_review_only_requires_fresh_identity_rebind',
            'page_key'=>'resort_month:1:4:20:2026-09','country_id'=>4,'region_id'=>20,
            'title'=>'Анталья в сентябре: климатические ориентиры для поездки',
            'h1'=>'Анталья в сентябре',
            'intro'=>'Сентябрьские условия в Анталье лучше оценивать по официальной климатической статистике, а конкретный тур — уже по актуальной выдаче поиска AnyTour.',
            'sections'=>[
                [
                    'heading'=>'Температура в сентябре',
                    'text_template'=>'По данным MGM для Антальи, средняя температура сентября — {{avg_temp}}, средняя максимальная — {{avg_high}}, а средняя минимальная — {{avg_low}}.',
                    'claim_keys'=>['avg_temp','avg_high','avg_low'],
                ],
                [
                    'heading'=>'Осадки',
                    'text_template'=>'Средний месячный объём осадков в сентябре — {{precipitation}}, а среднее число дождливых дней — {{rainy_days}}.',
                    'claim_keys'=>['precipitation','rainy_days'],
                ],
            ],
            'claims'=>[
            ...
            'source_note'=>'MGM official climate statistics; measurement period shown by source: 1930–2025. Revalidate source and fresh production identity before any publication decision.',
            'publication_allowed'=>false,'indexation_allowed'=>false,'sitemap_allowed'=>false,'route_creation_allowed'=>false,
        ],
        'maldives-september' => [
            'state'=>'authored_review_only_requires_fresh_identity_rebind',
            'page_key'=>'month:1:8:2026-09','country_id'=>8,'region_id'=>null,
            'title'=>'Мальдивы в сентябре: климат и особенности сезона',
            'h1'=>'Мальдивы в сентябре',
            ...
            'sections'=>[
                [
                    'heading'=>'Температура',
                    'text_template'=>'Maldives Meteorological Service описывает климат страны как тёплый и влажный круглый год; средняя температура находится в диапазоне {{temperature_range}}.',
                    'claim_keys'=>['temperature_range'],
                ],
                [
                    'heading'=>'Осадки и сезон',
                    'text_template'=>'Сентябрь приходится на период юго-западного муссона: официальный климатический раздел указывает, что влажный сезон обычно продолжается с середины мая до ноября и в этот муссонный период осадков больше.',
                    'claim_keys'=>['wet_season'],
                ],
                [
                    'heading'=>'Солнечный свет',
                    'text_template'=>'Для месяцев вне января–марта MMS указывает в среднем {{daylight_range}} солнечного света; этот ориентир относится и к сентябрю.',
                    'claim_keys'=>['daylight_range'],
                ],
            ],
            'claims'=>[
                ...
                ['country_id'=>8,'page_key'=>'month:1:8:2026-09','claim_key'=>'wet_season','type'=>'climate_precipitation','value'=>'влажный сезон обычно с середины мая до ноября; в юго-западный муссон осадков больше','source_class'=>'official_meteorological','source_id'=>'mv-mms-climate','source_url'=>'https://www.meteorology.gov.mv/climate','observed_at'=>'2026-09-02T11:17:00Z'],
...
function v2_seo_seasonal_render_review_content(array $record, ?int $nowEpoch = null): array
{
    ...
    $values=[];
    foreach (($evidence['claims']??[]) as $claim) $values[(string)$claim['claim_key']]=(string)$claim['value'];
    $sections=[];
    foreach (($record['sections']??[]) as $section) {
        $text=(string)($section['text_template']??'');
        $keys=is_array($section['claim_keys']??null)?$section['claim_keys']:[];
        if ($text===''||$keys===[]) throw new InvalidArgumentException('Seasonal content section requires explicit claims');
        foreach($keys as $key){
            $key=(string)$key;
            if(!array_key_exists($key,$values)||!str_contains($text,'{{'.$key.'}}') && !($key==='wet_season' && !str_contains($text,'{{'))) {
                if($key!=='wet_season') throw new InvalidArgumentException('Seasonal content template claim mismatch');
            }
    ...
    return [
        'state'=>'rendered_review_only_seasonal_content','page_key'=>(string)$record['page_key'],'title'=>(string)$record['title'],'h1'=>(string)$record['h1'],'intro'=>(string)$record['intro'],
        'sections'=>$sections,'claims'=>$evidence['claims'],'source_note'=>(string)($record['source_note']??''),
        'publication_candidates'=>[],'publication_allowed'=>false,'indexation_allowed'=>false,'sitemap_allowed'=>false,'route_creation_allowed'=>false,
        'requires_fresh_identity_rebind'=>true,
    ];
}
