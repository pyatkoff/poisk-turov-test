...
function v2_seo_production_identity_expected_rows(): array
{
    ...
    $rows[]=[
        ...
        'type'=>'hotel_tours',
        'http_status'=>200,
        'robots_prefix'=>'noindex,follow',
        'canonical'=>'https://anytoour.ru'.$hotel,
 ...
 * Fetcher returns ['status'=>int,'body'=>string] for a full URL.
 */
function v2_seo_collect_production_identity(callable $fetch, ?int $nowEpoch=null): array
{
    ...
    foreach($expected as $row){
        $url='https://anytoour.ru'.$row['path'];
        $res=(array)$fetch($url);
        ...
        $pages[]=[
            ...
            'robots'=>$robots,
            'canonical'=>$canonical,
            'sitemap_member'=>str_contains($sitemapBody,'https://anytoour.ru'.$row['path']),
        ];
    }
    ...
    $validated['transport_sitemap_status']=(int)($sitemap['status']??0);
    $validated['source']='live_http_collector';
    $validated['publication_allowed']=false;
    $validated['hotel_tours_publication_allowed']=false;
    $validated['hotel_tours_indexation_allowed']=false;
    return $validated;
}
