...
async function observedFetch(input,init){const action=actionFrom(input),response=await baseFetch(input,init);if(action){response.clone().json().then(data=>emit(action,data,response)).catch(()=>{});}return response;}
...
const runtime={api,on,state,setSearchId,build,fetch:observedFetch,version:3,defaultTimeout:DEFAULT_TIMEOUT};
window.V2Runtime=runtime;window.V2RuntimeV3=runtime;
})();