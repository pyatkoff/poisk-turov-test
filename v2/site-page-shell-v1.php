...
function sp_asset(string $file): string {
  $name=basename($file);
  if($name!==$file||!preg_match('/^[a-zA-Z0-9._-]+$/',$name)) throw new InvalidArgumentException('Invalid standalone asset name');
  return '/'.rawurlencode($name).'?v='.rawurlencode(v2_asset_content_version(__DIR__.'/'.$name));
}
