...
function v2_seo_controlled_launch_catalog(): array
{
    ...
    $special=[
        '/country/turkey/antalya/september/'=>v2_seo_content_pilot_antalya_september(),
        '/country/maldives/september/'=>v2_seo_content_pilot_maldives_september(),
    ];
    foreach(v2_seo_core_month_content_records() as $record){
        $path=(string)($record['path']??'');
        $records[]=$special[$path]??$record;
        unset($special[$path]);
    }
