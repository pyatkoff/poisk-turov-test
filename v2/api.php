...
function jwt(): string {
    ...
    if (defined('TOURVISOR_JWT') && trim((string)TOURVISOR_JWT) !== '') return trim((string)TOURVISOR_JWT);

    $conf = $_SERVER['DOCUMENT_ROOT'] . '/site_conf.php';
    if (is_file($conf)) {
        $params = [];
        require $conf;
        if (!empty($params['TOURVISOR_JWT'])) return trim((string)$params['TOURVISOR_JWT']);
        if (defined('TOURVISOR_JWT') && trim((string)TOURVISOR_JWT) !== '') return trim((string)TOURVISOR_JWT);
    }
    return '';
}
...
function tv_get(string $path, array $params = []): array {
    $token = jwt();
    if ($token === '') out(['ok' => false, 'error' => 'TOURVISOR_JWT is not configured for V2'], 500);
    ...
    if ($errno) out(['ok' => false, 'error' => 'Tourvisor connection error', 'detail' => $error], 502);
    $decoded = json_decode((string)$body, true);
    if ($code < 200 || $code >= 300) {
        out(['ok' => false, 'error' => 'Tourvisor HTTP ' . $code, 'data' => $decoded ?? $body], $code >= 400 && $code < 600 ? $code : 502);
    }
