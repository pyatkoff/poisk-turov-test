...
The Site remains at v147 as the visual reference. This publication did not connect live search or calendar DB. Stage 2 must reuse the canonical adapter and lifecycle with the approved TV + direct ANEX + SAMO/Andromeda-only inventory contract; LOCAL stored offers remain calendar/history/SEO-only. Latest related functional candidate at this checkpoint: draft #3921 `70fa3d001f4a8ad6f82c9db06683e2731f44419b` (supersedes #3915), 7/7 CI green per its handoff, not merged/published. Preserve that owner's app/data/lifecycle work. Do not silently substitute its branch for the published source or imply that provider-specific applications are connected.
...
Next: connect the canonical `prototype-search/data.js` and lifecycle after the active TV/SAMO/ANEX-only source packet is checked/integrated; keep LOCAL observations for calendar/SEO and canonical catalogue for identities/content. Do not transplant the Site `live-data.js` or replace the active functional app/data owners. Provider-specific quote/flight/application authority must remain exact; unresolved provider applications cannot become bookable through UI copy.
