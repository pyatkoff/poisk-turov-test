...
function v2_seo_search_feedback_evidence(array $record, ?int $nowEpoch=null): array
{
    ...
    $fingerprint=hash('sha256',json_encode([
        ...
        'collected_at_epoch'=>$collectedAt,'period_start_epoch'=>$periodStart,'period_end_epoch'=>$periodEnd,'metrics'=>$normalized,
...
/** Partial intake: missing launched pages mean UNKNOWN feedback, never zero. */
function v2_seo_search_feedback_intake(array $rows, ?int $nowEpoch=null): array
{
    $nowEpoch??=time();
    $launchPaths=v2_seo_controlled_launch_paths();
    $launchScope='controlled_country_resort_seasonal_v3';
    $byPath=[];$errors=[];
    foreach($rows as $i=>$raw){
        if(!is_array($raw)){$errors[]='invalid_row_'.$i;continue;}
        $normalized=v2_seo_search_feedback_evidence($raw,$nowEpoch);$path=(string)($normalized['path']??'');
    ...
    $observed=array_keys($byPath);sort($observed,SORT_STRING);$missing=array_values(array_diff($launchPaths,$observed));sort($missing,SORT_STRING);ksort($byPath,SORT_STRING);
    ...
    return [
        ...
        'indexation_change_allowed'=>false,'sitemap_change_allowed'=>false,'canonical_change_allowed'=>false,'route_change_allowed'=>false,
        'hotel_tours_indexation_allowed'=>false,'search_contract_changes'=>false,'tourvisor_contract_changes'=>false,
        'pricing_contract_changes'=>false,'lead_contract_changes'=>false,'metrika_contract_changes'=>false,
    ];
}
