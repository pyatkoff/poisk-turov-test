...
function cp_render(array $page): void
{
    ...
    <main class="sp-main sp-country-page">
      ...
      <?php if ($offers): ?><section class="sp-card sp-offer-snapshot" data-country-offer-snapshot><h2>Актуальные туры</h2><p>Предложения собраны из свежих ценовых наблюдений AnyTour. Стоимость и доступность перепроверяются в поиске перед заявкой.</p><div class="sp-offer-list"><?php foreach ($offers as $offer): $hotel=trim((string)($offer['hotelName']??''))?:'Отель'; $departure=trim((string)($offer['departureName']??'')); $date=v2_seo_offer_date_label((string)($offer['departureDate']??'')); $nights=(int)($offer['nights']??0); $price=v2_seo_offer_price_label((float)($offer['price']??0),(string)($offer['currency']??'RUB')); $offerSearchState=$searchState; $departureId=(int)($offer['departureId']??0); if($departureId>0)$offerSearchState['from']=$departureId; $offerHref=v2_seo_search_handoff_url('/poisk-turov/',$offerSearchState); ?><article class="sp-offer-item"><h3><?=sp_e($hotel)?></h3><div class="sp-offer-meta"><?php if($departure!==''): ?><span class="sp-offer-fact">Вылет из <?=sp_e($departure)?></span><?php endif; ?><span class="sp-offer-fact"><?=sp_e($date)?></span><span class="sp-offer-fact"><?=sp_e((string)$nights)?> ночей</span></div><div class="sp-offer-bottom"><div class="sp-offer-price"><span>от</span><strong><?=sp_e($price)?></strong></div><a class="sp-secondary sp-offer-action" href="<?=sp_e($offerHref)?>">Посмотреть туры</a></div></article><?php endforeach; ?></div></section><?php endif; ?>
