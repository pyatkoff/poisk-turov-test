...
function ensureStylesheet(href,attr,onReady){
  var existing=document.querySelector('link['+attr+']');
  if(existing){if(onReady){if(existing.sheet)onReady();else existing.addEventListener('load',onReady,{once:true});}return;}
  var css=document.createElement('link');css.rel='stylesheet';css.href=href;css.setAttribute(attr,'1');
  if(onReady)css.addEventListener('load',onReady,{once:true});
...
function renderUpgradedHeader(){
  ...
  var personalHref=personal&&personal.getAttribute('href')?personal.getAttribute('href'):'';
  var personalHtml=personalHref?'<a class="at-global-header__personal" href="'+personalHref+'" target="_blank" rel="noopener">Личный кабинет</a>':'';
...
function upgradeHeader(){
  var old=document.querySelector('.at-site-header');if(!old||document.querySelector('.at-global-header'))return;
  ensureStylesheet(runtimePath('site-header-v2.css'),'data-at-header-v2-css',renderUpgradedHeader);
}
upgradeHeader();
