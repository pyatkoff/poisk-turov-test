...
function renderRealOffer(){
 ...
 $('#modal-footer').hidden=false;$('#modal-footer').innerHTML=`<div class="footer-total"><span>За всех туристов</span><strong>${o.pricePending?'Цена уточняется':money(o.total)}</strong></div>${unavailable?`<button class="primary" data-action="refresh-hotel" data-id="${h.id}">Обновить предложения</button>`:o.quoteError?`<button class="primary" data-action="offer" data-key="${esc(o.key)}">Повторить проверку</button>`:`<button class="primary" data-action="confirm-tour" ${o.loading||o.flightsLoading||o.pricePending||!o.tour?'disabled':''}>Выбрать этот тур ${icon('arrow')}</button>`}`;
}
