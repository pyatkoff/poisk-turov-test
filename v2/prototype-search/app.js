...
function recommendedHotelScore(h){return ratingValue(h)+(h.beach!==null&&h.beach<=150?.2:0)+(popularity?.boost(h)||0);}
function recommendedHotelRank(h){const rank=popularity?.rank(h);return Number.isInteger(rank)?rank:Number.MAX_SAFE_INTEGER;}
function results(){return hotels.map(h=>({hotel:h,offers:hotelOffers(h)})).filter(r=>r.offers.length).sort((a,b)=>state.sort==='price'?a.offers[0].total-b.offers[0].total:state.sort==='rating'?ratingValue(b.hotel)-ratingValue(a.hotel):recommendedHotelScore(b.hotel)-recommendedHotelScore(a.hotel)||recommendedHotelRank(a.hotel)-recommendedHotelRank(b.hotel)||a.offers[0].total-b.offers[0].total);}
...
function cardHTML({hotel:h,offers}){const o=offers[0],opened=state.openHotel===h.id,photoIndex=state.photoIndexes[h.id]||0,popularityBadge=popularity?.badge(h)||'';return `<article class="hotel-card" id="hotel-${h.id}" data-hotel-id="${h.id}"><div class="hotel-main">
 ...
 <div class="hotel-info"><div class="hotel-info-top"><div>${popularityBadge?`<span class="hotel-popularity-badge" title="Входит в TOP500 продаваемых отелей">${popularityBadge==='Хит продаж'?'🔥 ':''}${esc(popularityBadge)}</span>`:''}<div class="hotel-stars" aria-label="${h.stars} звёзд">${'★'.repeat(h.stars)}</div><h3><button data-action="hotel-details" data-id="${h.id}">${esc(h.name)}${icon('arrow')}</button></h3><div class="hotel-location">${icon('pin')} ${esc(h.resort)}, ${esc(countryNames[h.country]||'')}</div></div><div class="rating-block"><strong>${ratingText(h)}<small>/ 5</small></strong><span>${h.rating===null?'Нет оценки':ratingValue(h)>=4.5?'Отлично':'Оценка гостей'}</span><small>Оценка гостей</small></div></div><div class="hotel-facts"><span>${esc(hotelHighlights(h))}</span></div></div></div>
