...
const countryNames={};
const mealNames={};
const mealName=id=>mealNames[String(id)]||'Неизвестный тип питания';
function syncMealNames(){Object.keys(mealNames).forEach(k=>delete mealNames[k]);data.catalog.meals.forEach(p=>{mealNames[String(p.id)]=p.nameRu;});}
const countriesTo={};
const operators=[];
...
function renderSummary(){const s=state.search,f=state.filters;$('#applied-search').innerHTML=`<div class="applied-route"><span class="summary-icon">${icon('plane')}</span><div><strong>${esc(s.origin)}<span>→</span>${esc(destinationLabel(appliedDestination()))}</strong><p>${state.selectedDate?dateText(state.selectedDate):rangeText(s.from,s.to)} · ${durationText()} · ${guestsText()}</p></div></div><div class="applied-extras"><span>${f.stars.length?f.stars.join(' / ')+' ★':'Любая категория'}</span><span>${f.meals.length?f.meals.map(id=>esc(mealName(id))).join(', '):'Любое питание'}</span><span>${budgetText()}</span></div><button class="secondary" data-action="edit-search">${icon('sliders')} Изменить поиск</button>`;}
...
function hotelOffers(h,options={}){
 ...
 return (h.offers||[]).filter(o=>o.search.origin===s.origin&&o.search.country===s.country&&o.adults===s.adults&&JSON.stringify([...o.ages].sort())===JSON.stringify([...s.ages].sort())&&o.day>=from&&o.day<=to&&o.nights>=s.minNights&&o.nights<=s.maxNights&&(!f.meals.length||f.meals.includes(String(o.mealPlanId)))&&o.total>=f.min&&(f.max===600000||o.total<=f.max)&&(!f.operators.length||f.operators.includes(o.operator))&&(!f.flight.length||f.flight.includes(o.flight))).sort((a,b)=>a.total-b.total||a.day.localeCompare(b.day));
}
...
function updateSearchUI(){
 ...
 const meals=state.filters.meals.map(mealName).join(' · ');$('#meal-label').textContent=state.filters.meals.length>1?state.filters.meals.length+' варианта':meals||'Любое';$('#quick-meal').setAttribute('aria-label','Питание: '+(meals||'любое'));$('#quick-meal').title=meals||'Любое питание';
...
function filterChipData(model=appliedFilterModel()){
 ...
 for(const key of ['stars','meals','resorts','operators','flight'])f[key].forEach(value=>chips.push({key,value,label:key==='stars'?value+' ★':key==='flight'?(value==='charter'?'Чартер':'Регулярный рейс'):key==='meals'?mealName(value):value}));
...
function renderFilters(){const model=editingFilterModel(),f=model.filters,hs=hotels.filter(h=>h.country===state.search.country);$('#filters').innerHTML=`
 ...
 ${Object.keys(mealNames).length?`<div class="filter-group"><h4>Питание</h4>${checkRows('meals',[...new Set([...Object.keys(mealNames),...f.meals])].map(m=>[m,mealName(m)]))}</div>`:''}
...
function openMeals(restored){
 mealDraft=restored||[...state.filters.meals];showModal('meals','Какое питание включить?','ПИТАНИЕ',`<p class="modal-intro">Можно выбрать несколько вариантов.</p>${data.catalog.mealCatalogueError?`<p class="modal-intro" role="status">${esc(data.catalog.mealCatalogueError)} <button class="text-button" data-action="retry-meal-catalog">Повторить загрузку</button></p>`:''}<div class="meal-options">${[['','Любое питание'],...[...new Set([...Object.keys(mealNames),...mealDraft])].map(m=>[m,mealName(m)])].map(([v,label])=>`<label class="meal-option"><input type="checkbox" data-meal-choice value="${esc(v)}" ${v?mealDraft.includes(v)?'checked':'':!mealDraft.length?'checked':''}><span><strong>${esc(label)}</strong></span></label>`).join('')}</div>`);
...
async function openOffer(key,restored=null,chooseFlight=false){
 ...
 try{const tour=await data.quote(initial);if(run!==selectionGeneration||!$('#modal').open||modalType!=='offer'||selectedOffer?.key!==key)return;selectedOffer={...initial,tour,total:data.amount(tour.price)||initial.total,room:data.text(tour.roomType)||initial.room,mealPlanId:data.mealPlanId(tour.meal),meal:data.meal(tour.meal)||initial.meal,loading:false,flightsLoading:true};renderRealOffer();await loadRealFlights(run);if(chooseFlight&&run===selectionGeneration&&selectedOffer?.variants?.length)openFlightPicker();}
...
function renderRealOffer(){
 ...
 ${unavailable?`<p class="saved-tour-notice">Сохранённое предложение. Найдём актуальные туры в этот отель на эту дату и ${nightsText(o.nights)}, для ${guestsText(o)}. Цена, номер, питание и туроператор могут отличаться — вы выберете подходящий вариант.</p>`:''}
...
function refreshHotel(id){
 const o=selectedOffer,h=hotels.find(h=>h.id===id);
 ...
 try{data.params(next,h.legacyIds,state.filters);}catch(error){toast(error.message);return;}
 closeModal();state.search=next;draft=structuredClone(next);draftDestination=null;state.filters.hotelId=id;state.selectedDate=null;state.openHotel=id;
 updateSearchUI();updateURL();renderFilters();runSearch({hotelIds:h.legacyIds,exactRefresh:true});
 $('#search-status').scrollIntoView({behavior:scrollBehavior(),block:'start'});
}
function openLeadPreview(){
 const o=selectedOffer;if(!o?.tour)return;if(o.pricePending){toast('Цена перелёта пока не подтверждена. Выберите другой вариант.');return;}savedSelection=o;updateNav();
...
const offerGroupKey=o=>encodeURIComponent(o.room+'|'+(o.mealId?'local:'+o.mealId:o.mealPlanId?'plan:'+o.mealPlanId:o.meal));
...
function renderOfferList(reset=false){
 const h=hotels.find(h=>h.id===offerView.id),all=hotelOffers(h),filtered=all.filter(o=>(!offerView.flight||o.flight===offerView.flight)&&(!offerView.room||o.room===offerView.room)&&(!offerView.meal||String(o.mealPlanId)===offerView.meal));
...
function runSearch(options={}){
 ...
 data.search(state.search,event=>{
  ...
  if(event.type==='meal-catalog')syncMealNames();
...
document.addEventListener('click',e=>{const b=e.target.closest('[data-action]');if(!b||b.disabled)return;actionTrigger=b;queueMicrotask(()=>{if(actionTrigger===b)actionTrigger=null;});const action=b.dataset.action,id=+b.dataset.id;
 ...
 case 'retry-meal-catalog':{const current=[...mealDraft];data.loadMealCatalogue().then(()=>{syncMealNames();if(modalType==='meals')openMeals(current);}).catch(error=>toast(error.message));break;}
