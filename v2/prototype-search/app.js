...
function renderFilters(){const model=editingFilterModel(),f=model.filters,hs=hotels.filter(h=>h.country===state.search.country);$('#filters').innerHTML=`
 ...
 ${hs.some(h=>h.resort)?`<div class="filter-group"><h4>Курорт</h4>${checkRows('resorts',[...new Set(hs.map(h=>h.resort).filter(Boolean))].map(r=>[r,r]))}</div>`:''}
 ${operators.length?`<div class="filter-group"><h4>Туроператор</h4>${checkRows('operators',operators.map(o=>[o,o]))}</div>`:''}
 <div class="filter-hint">${icon('info')}<span>${!state.hasSearched&&!state.onlyFavorites?'Условия применятся после нажатия «Найти туры». Курорты и туроператоры появятся в выдаче.':'Фильтры применяются к найденным предложениям. Актуальная цена и сборы уточняются при выборе.'}</span></div>`;
