...
const refreshOfferNotice=o=>o.cached?`Сохранённое предложение доступно 24 часа с момента проверки. Повторный поиск использует тот же отель, дату, ${nightsText(o.nights)}, состав туристов, номер, питание, туроператора и тип перелёта. Другой тур не будет подставлен вместо выбранного.`:'Предложение найдено в текущем поиске. Перед выбором нужно проверить цену, наличие и варианты перелёта для этих условий.';
const cachedPriceNote=o=>o.raw?.listingPriceState==='final_verified'?'Ранее подтверждённая цена · наличие проверим':o.raw?.listingPriceState==='final_ready_estimate'?'Расчётная цена · подтвердим при выборе':'Сохранённая цена · требует проверки';
const priceNote=o=>o.cached?cachedPriceNote(o):needsRefresh(o)?'Цена из текущего поиска · требует подтверждения':'Цена предложения · сборы уточняются';
...
function results(){return hotels.map(h=>({hotel:h,offers:hotelOffers(h)})).filter(r=>r.offers.length).sort((a,b)=>state.sort==='price'?a.offers[0].total-b.offers[0].total:state.sort==='rating'?b.hotel.rating-a.hotel.rating:recommendedHotelScore(b.hotel)-recommendedHotelScore(a.hotel)||recommendedHotelRank(a.hotel)-recommendedHotelRank(b.hotel)||a.offers[0].total-b.offers[0].total);}
...
function cardHTML({hotel:h,offers}){const o=offers[0],opened=state.openHotel===h.id,photoIndex=state.photoIndexes[h.id]||0,popularityBadge=popularity?.badge(h)||'';return `<article class="hotel-card" id="hotel-${h.id}" data-hotel-id="${h.id}"><div class="hotel-main">
 <div class="hotel-photos ${h.photos.length?'':'photo-unavailable'}"><div class="hotel-image-wrap"><button class="hotel-image-button" data-action="gallery" data-id="${h.id}" aria-label="Открыть фотографии ${esc(h.name)}"><img class="hotel-image" src="${esc(photoUrl(h,photoIndex))}" alt="Фото отеля ${esc(h.name)}" loading="lazy" width="700" height="500"><span class="photo-count">${icon('image')} <span class="photo-index">${h.photos.length?photoIndex+1:0}</span> / ${h.photos.length}</span></button><button class="favorite-button ${state.favorites.includes(h.id)?'active':''}" data-action="favorite" data-id="${h.id}" aria-pressed="${state.favorites.includes(h.id)}" aria-label="${state.favorites.includes(h.id)?'Убрать из избранного':'В избранное'}: ${esc(h.name)}">${icon('heart')}</button><button class="compare-photo-button ${state.compare.includes(h.id)?'active':''}" data-action="toggle-compare" data-id="${h.id}" aria-pressed="${state.compare.includes(h.id)}" aria-label="${state.compare.includes(h.id)?'Убрать из сравнения':'Сравнить'}: ${esc(h.name)}" title="${state.compare.includes(h.id)?'В сравнении':'Сравнить отель'}">${icon(state.compare.includes(h.id)?'check':'compare')}</button><button class="card-photo-arrow prev" data-action="card-photo" data-id="${h.id}" data-dir="-1" aria-label="Предыдущее фото ${esc(h.name)}">${icon('back')}</button><button class="card-photo-arrow next" data-action="card-photo" data-id="${h.id}" data-dir="1" aria-label="Следующее фото ${esc(h.name)}">${icon('arrow')}</button></div><div class="card-thumbs">${h.photos.slice(0,12).map((p,i)=>`<button class="card-thumb ${photoIndex===i?'active':''}" data-action="card-photo-index" data-id="${h.id}" data-value="${i}" aria-label="Показать фото ${i+1} отеля ${esc(h.name)}" aria-pressed="${photoIndex===i}"><img src="${esc(p)}" alt="" loading="lazy" width="150" height="100"></button>`).join('')}</div></div>
...
function renderRealOffer(){
 ...
 <div class="tour-hero"><img src="${esc(photoUrl(h))}" alt="Фото ${esc(h.name)}"><div><div class="hotel-stars">${'★'.repeat(h.stars)}</div><h3>${esc(h.name)}</h3><p>${esc(h.resort)}, ${esc(countryNames[h.country]||'')}</p></div>${operatorBadge(o.operator)}</div>
  ${unavailable?`<p class="saved-tour-notice">${esc(refreshOfferNotice(o))}</p>`:''}
 ...
 <aside class="tour-price-details" aria-label="Состав и стоимость тура"><div class="price-breakdown"><h3>Стоимость тура</h3><p class="price-party">За ${guestsText(o)} · ${nightsText(o.nights)}</p><div class="price-line"><span>Топливный сбор</span><strong>${fuelText(o)}</strong></div><div class="price-line total"><span>${o.flightChoiceId!==null?'С выбранным перелётом':'Цена предложения'}</span><strong id="detail-total">${o.pricePending?'Уточняется':money(o.total)}</strong></div><p class="price-assurance">${icon('info')} ${o.loading?'Проверяем актуальность…':unavailable?priceNote(o):'Условия цены и наличие подтверждаются перед оформлением'}</p></div></aside></div>`,true);
 $('#modal-footer').hidden=false;$('#modal-footer').innerHTML=`<div class="footer-total"><span>За всех туристов</span><strong>${o.pricePending?'Цена уточняется':money(o.total)}</strong></div>${unavailable?`<button class="primary" data-action="refresh-hotel" data-id="${h.id}">${refreshOfferActionLabel(o)}</button>`:o.quoteError?`<button class="primary" data-action="offer" data-key="${esc(o.key)}">Повторить проверку</button>`:`<button class="primary" data-action="confirm-tour" ${o.loading||o.flightsLoading||o.pricePending||!o.tour?'disabled':''}>Выбрать этот тур ${icon('arrow')}</button>`}`;
}
...
const rehydrationText=value=>String(value||'').toLocaleLowerCase('ru-RU').replace(/\s+/g,' ').trim();
function sameRehydratedTour(candidate,target){
 if(!candidate||!target||candidate.cached||candidate.provider!==target.provider||candidate.hotelId!==target.hotelId
   ||candidate.day!==target.day||candidate.nights!==target.nights||candidate.adults!==target.adults
   ||JSON.stringify(candidate.ages||[])!==JSON.stringify(target.ages||[]))return false;
 if(rehydrationText(candidate.operator)!==rehydrationText(target.operator))return false;
 const room=rehydrationText(target.room),meal=rehydrationText(target.mealRaw||target.meal);
 if(room&&!/уточняется/.test(room)&&rehydrationText(candidate.room)!==room)return false;
 if(meal&&!/уточняется/.test(meal)){
  const liveMeals=[candidate.mealRaw,candidate.meal].map(rehydrationText).filter(Boolean);
  if(!liveMeals.includes(meal))return false;
 }
 if(['regular','charter'].includes(target.flight)&&candidate.flight!==target.flight)return false;
 return true;
}
function openRehydratedVariants(cached,offers){
 const h=selectedTourHotel(cached);if(!h||!offers.length)return;
 rehydrationDraft={cached,offers:[...offers]};
 const provider=cached.provider==='anex'?'ANEX':'Andromeda';
 showModal('rehydrated-variants','Предложение обновилось',provider+' · АКТУАЛЬНЫЕ ВАРИАНТЫ',`
  <p class="modal-intro">Сохранённый вариант больше не совпал один в один. Ниже только свежие предложения того же поставщика для этого отеля, даты, длительности и состава туристов.</p>
  <div class="favorite-list">${offers.map(o=>`<article class="favorite-item"><div class="favorite-info"><strong>${dateText(o.day)} · ${nightsText(o.nights)}</strong><h3>${esc(o.room)}</h3><p>${esc(mealLabel(o))} · ${esc(o.operator)}</p></div><div class="favorite-summary"><strong class="saved-price">${money(o.total)}</strong><span>${flightLabel(o)}</span></div><div class="favorite-actions"><button class="primary" data-action="rehydrated-offer" data-key="${esc(o.key)}">Проверить этот вариант ${icon('arrow')}</button></div></article>`).join('')}</div>`,true);
 $('#modal-footer').hidden=false;$('#modal-footer').innerHTML='<button class="secondary" data-action="modal-back">К сохранённому туру</button>';
}
function chooseRehydratedOffer(key){
 const draft=rehydrationDraft,offer=draft?.offers.find(item=>item.key===key);if(!offer)return;
 rehydrationDraft=null;selectedOffer={...offer};renderRealOffer();refreshHotel(offer.hotelId);
}
function genericExactRefresh(o,h){
 ...
 const exactFilters=defaultFilters();exactFilters.hotelId=h.id;if(o.meal&&!/уточняется/i.test(o.meal))exactFilters.meals=[o.meal];if(o.operator&&!/уточняется/i.test(o.operator))exactFilters.operators=[o.operator];if(['regular','charter'].includes(o.flight))exactFilters.flight=[o.flight];
 try{data.params(next,h.legacyIds,exactFilters);}catch(error){toast(error.message);return;}
 closeModal();state.search=next;draft=structuredClone(next);draftDestination=null;state.filters=exactFilters;state.selectedDate=null;state.openHotel=h.id;
...
async function refreshHotel(id){
 ...
 if(o.cached&&['anex','andromeda'].includes(o.provider)&&o.raw?.rehydration){
  const run=++selectionGeneration;selectedOffer={...o,loading:true,quoteError:''};renderRealOffer();
  try{
   const refreshed=await data.rehydrateCached(o);
   if(run!==selectionGeneration||!$('#modal').open||modalType!=='offer'||selectedOffer?.key!==o.key)return;
   if(refreshed.state==='unsupported'){selectedOffer={...o,loading:false};genericExactRefresh(o,h);return;}
   const offers=Array.isArray(refreshed.offers)?refreshed.offers:[];
   const exact=offers.find(item=>sameRehydratedTour(item,o));
   if(exact){
    selectedOffer={...exact,loading:false};renderRealOffer();
    await refreshHotel(id);return;
   }
   selectedOffer={...o,loading:false};
   if(offers.length){openRehydratedVariants(o,offers);return;}
   genericExactRefresh(o,h);
  }catch(error){
...
const comparisonVariantLabel=o=>o.room+' · '+o.meal+' · '+o.operator;
...
function renderOfferList(reset=false){
 ...
 $('#all-offers-list').innerHTML=groups.length?groups.map(({key,offers})=>{
  ...
  const rows=offers.slice(0,limit).map(o=>{const reference=Math.min(...all.filter(x=>x.day===o.day&&x.nights===o.nights).map(x=>x.total)),delta=o.total-reference;return `<div class="offer grouped-offer" data-offer-key="${o.key}"><div class="offer-departure"><strong>${dateText(o.day)} → ${dateText(o.returnDay)}</strong><small>${nightsText(o.nights)} · ${guestsText(o)}</small><small>Город вылета: ${esc(o.origin)}</small></div><div class="offer-flight-details"><span class="flight-tag ${o.flight}">${flightLabel(o)}</span>${operatorBadge(o.operator)}<small>${offerMetaNote(o)}</small></div><div class="offer-price"><strong>${money(o.total)}</strong>${delta?`<small class="offer-delta">+${money(delta)} к минимуму на эти даты и ${nightsText(o.nights)}</small>`:'<small class="offer-lowest">Минимум на эти даты и ночи</small>'}<button class="primary" data-action="offer" data-key="${o.key}">${offerActionLabel(o)} ${icon('arrow')}</button><button class="text-button compare-tour-link" data-action="compare-tour" data-key="${o.key}">Сравнить на эти даты</button></div></div>`}).join('');
...
function renderSearchStatus(items,total){
 ...
 more.hidden=!state.hasSearched||r.pending||providerPending||!r.canContinue;
...
document.addEventListener('click',e=>{const b=e.target.closest('[data-action]');if(!b||b.disabled)return;actionTrigger=b;queueMicrotask(()=>{if(actionTrigger===b)actionTrigger=null;});const action=b.dataset.action,id=+b.dataset.id;
 ...
 case 'offer':openOffer(b.dataset.key);break;case 'rehydrated-offer':chooseRehydratedOffer(b.dataset.key);break;case 'confirm-tour':confirmTour();break;case 'apply-andromeda-flights':applyAndromedaFlightChoice();break;case 'anex-additional-prices':applyAnexAdditionalPrices();break;case 'accept-price':if(verifiedOffer){const accepted=verifiedOffer;verifiedOffer=null;completeTour(accepted);}break;
