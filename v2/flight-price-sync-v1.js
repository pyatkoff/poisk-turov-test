...
function sync(tour,flight,index){const basePrice=Number(tour&&tour.price||0),flightPrice=valueOfPrice(flight&&flight.price),fuel=valueOfPrice(flight&&flight.fuelCharge);clarifyVariantPrices();if(!flightPrice)return;const delta=flightPrice - basePrice;let label='Стоимость с выбранным рейсом';if(basePrice&&delta>0)label+=' · +'+money(delta)+' ₽';else if(basePrice&&delta<0)label+=' · '+money(delta)+' ₽';render(flightPrice,label);renderFuel(flight);leadSelectionNote('Менеджер получит выбранный тур и выбранный рейс');window.dispatchEvent(new CustomEvent('v2:tour-price-updated',{detail:{tour:tour||null,flight:flight||null,index:Number(index||0),basePrice,price:flightPrice,delta,fuelCharge:fuel}}));}
