...
function create(refresh){
 ...
 let epoch=0,raw=[],options={},links=new Map(),profiles=new Map(),anchors=new Map(),storedOffers=new Map(),missing=new Set(),failed=new Set(),pending=new Set(),workers=new Set();
 function reset(){epoch++;workers.forEach(task=>task.controller.abort());workers=new Set();pending=new Set();links=new Map();profiles=new Map();anchors=new Map();storedOffers=new Map();missing=new Set();failed=new Set();raw=[];options={};}
 ...
 function clearOffers(source){const key=String(source||'');if(!key)throw new TypeError('Offer source required');storedOffers.forEach((bucket,own)=>{bucket.forEach((record,offerKey)=>{if(record.source===key)bucket.delete(offerKey);});if(!bucket.size)storedOffers.delete(own);});}
 ...
 const api={read(list,opts){raw=list.slice();options=Object.assign({},opts);pump();return project();},source:()=>raw,options:()=>options,details:h=>profiles.get(id(h&&h.anytourHotelId))||null,status,reset,upsertHotel:putProfile,upsertOffer,clearOffers,refresh:()=>refresh()};activeOwner=api;return api;
}
