...
function updateSummary(count,list){const visible=Array.isArray(list)?list:sourceItems,total=totalTours(visible);if(heading)heading.textContent='Найдено '+count+' '+hotelWord(count);if(summary)summary.textContent=total?total+' '+tourWord(total):'Актуальные варианты';}
function apply(){if(!mq.matches||!sourceItems.length)return;const r=renderer();if(!r||typeof r.render!=='function')return;const filtered=sourceItems.map(filteredHotel).filter(Boolean);applying=true;window.__DS2ResultsRailApplying=true;try{r.render(filtered,{keepResultsShell:true});}finally{window.__DS2ResultsRailApplying=false;applying=false;}updateSummary(filtered.length,filtered);updateRailCount(filtered.length);}
