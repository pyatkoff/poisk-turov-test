...
function init(){
  ...
  [['stars','Категория отеля','search3-stars'],['food','Питание','search3-meal'],['price_till','Бюджет на тур','search3-budget'],['hotel','Конкретный отель','search3-hotel'],['operator','Туроператор','search3-operator']].forEach(function(x){var f=field(form,x[0]);if(!f)return;cleanField(f);var s=f.querySelector(':scope > span');if(s)s.textContent=x[1];f.classList.add(x[2]);grid.appendChild(f);});
  var budget=form.elements.price_till;if(budget)budget.placeholder='Без ограничений';
  var hotel=form.elements.hotel;if(hotel)hotel.setAttribute('aria-label','Конкретный отель');var hotelInput=grid.querySelector('.search3-hotel input[type=text]');if(hotelInput)hotelInput.placeholder='Любой отель';
  var operator=form.elements.operator;if(operator&&operator.options&&operator.options.length)operator.options[0].textContent='Любой туроператор';
  ...
  setTimeout(function(){['region','stars','food','operator'].forEach(function(name){var f=field(form,name);if(!f)return;cleanField(f);if(name==='region'){var s=f.querySelector(':scope > span');if(s)s.textContent='Курорт / регион';f.classList.add('search3-region');if(f.parentNode!==main){var dateBoxNow=main.querySelector('.search3-dates');main.insertBefore(f,dateBoxNow||main.children[2]||null);}}else{if(name==='stars'){var st=f.querySelector(':scope > span');if(st)st.textContent='Категория отеля';}if(name==='food'){var ft=f.querySelector(':scope > span');if(ft)ft.textContent='Питание';}if(name==='operator'){var op=f.querySelector(':scope > span');if(op)op.textContent='Туроператор';if(f.querySelector('select')&&f.querySelector('select').options.length)f.querySelector('select').options[0].textContent='Любой туроператор';}if(f.parentNode!==grid)grid.appendChild(f);}});var hi=grid.querySelector('.search3-hotel input[type=text]');if(hi)hi.placeholder='Любой отель';},100);
}
