 ...
 *
 * This does not create routes, copy, publication candidates or sitemap entries.
 * It only proves that an exact month/resort-month identity has a verified
 * editorial parent and therefore cannot invent a country/region mapping.
 */
function v2_seo_seasonal_family_binding(array $countryRecord, array $resortRecords, array $identityInventory): array
{
    if (($countryRecord['type'] ?? '') !== 'country') {
        throw new InvalidArgumentException('Seasonal binding requires one country record');
    }
    ...
    if (($identityInventory['state'] ?? '') !== 'review_only_seasonal_identity_inventory') {
        throw new InvalidArgumentException('Seasonal binding requires review-only identity inventory');
    }
    ...
    foreach ($resortRecords as $record) {
        if (!is_array($record) || ($record['type'] ?? '') !== 'resort') {
            throw new InvalidArgumentException('Seasonal binding accepts resort records only');
        }
        $path = v2_seo_registry_path($record['path'] ?? '');
        if (!str_starts_with($path, rtrim($countryPath, '/') . '/')) {
            throw new InvalidArgumentException('Seasonal resort is outside country parent: ' . $path);
        }
        ...
        if ($recordCountryId !== $countryId || $regionId <= 0) {
            throw new InvalidArgumentException('Seasonal resort identity mismatch: ' . $path);
        }
        if (isset($regionParents[$regionId])) {
            throw new InvalidArgumentException('Duplicate seasonal resort region identity: ' . $regionId);
        }
    ...
    foreach (($identityInventory['identities'] ?? []) as $index => $identity) {
        if (!is_array($identity)) {
            $blocked[] = ['index'=>$index,'errors'=>['invalid_identity']];
            continue;
        }
        $errors = [];
        ...
        if ($key === '') $errors[] = 'missing_page_key';
        elseif (isset($seenKeys[$key])) $errors[] = 'duplicate_page_key';
        ...
        if (($identity['publication_allowed'] ?? true) !== false || ($identity['copy_allowed'] ?? true) !== false) {
            $errors[] = 'identity_publication_boundary_crossed';
        }
        ...
        if ($errors !== []) {
            $blocked[] = ['index'=>$index,'page_key'=>$key,'errors'=>$errors];
            continue;
        }

        $bound[] = [
            'state'=>'review_only_family_bound_identity',
            'page_key'=>$key,
            'page_type'=>$pageType,
    ...
    return [
        'state'=>'review_only_seasonal_family_binding',
        'country_id'=>$countryId,
        'country_path'=>$countryPath,
        'registered_region_count'=>count($regionParents),
