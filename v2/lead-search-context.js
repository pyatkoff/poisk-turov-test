...
function textValue(v){if(v===null||v===undefined)return'';if(Array.isArray(v))return v.map(textValue).filter(Boolean).join(', ');if(typeof v==='object'){for(const key of['russianName','fullRussianName','name','title','value','text','label','description']){const text=textValue(v[key]);if(text)return text;}return'';}return String(v).trim();}
function enrichPayload(payload,s){const out=Object.assign({},payload||{}),ages=childAgesFrom(s);if(ages.length){out.childAges=ages;out.childs=ages.length;}if(s&&Number.isInteger(Number(s.adults))&&Number(s.adults)>=1&&Number(s.adults)<=6)out.adults=Number(s.adults);if(Object.prototype.hasOwnProperty.call(out,'roomType'))out.roomType=textValue(out.roomType);if(Object.prototype.hasOwnProperty.call(out,'placement'))out.placement=textValue(out.placement);return out;}
...
window.V2LeadSearchContext={snapshot,childAgesFrom,textValue,enrichPayload,isLeadRequest,version:2};
})();
