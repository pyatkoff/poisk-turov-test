<?php
/** Report Russian Tourvisor departure cities with charter countries and direct+charter overlap. CLI only, no DB writes. */
...
$total=0; $active=[]; $inactive=[]; $charterPairs=0; $directCharterPairs=0; $directCharterCities=0;
foreach ($departures as $departure) {
    if (!is_array($departure)) continue;
    $id=(int)($departure['id'] ?? 0); $name=charter_departure_name($departure);
    if ($id<=0 || $name==='') continue;
    $total++;
    $charter=charter_country_ids($id,false);
    $directCharter=charter_country_ids($id,true);
    $charterCount=count($charter); $directCount=count($directCharter);
    $charterPairs += $charterCount; $directCharterPairs += $directCount;
    if ($directCount>0) $directCharterCities++;
    $row=['id'=>$id,'name'=>$name,'charter'=>$charterCount,'directCharter'=>$directCount];
...
foreach ($active as $row) fwrite(STDOUT,"CHARTER_ACTIVE id={$row['id']} name={$row['name']} countries={$row['charter']} direct_charter_countries={$row['directCharter']}\n");
foreach ($inactive as $row) fwrite(STDOUT,"CHARTER_INACTIVE id={$row['id']} name={$row['name']}\n");
