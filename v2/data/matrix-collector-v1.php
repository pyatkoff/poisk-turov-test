...
function matrix_priority_coverage(PDO $pdo): array {
    $ids=array_values(array_unique(array_map('intval',v2_priority_hotel_ids())));$set=array_fill_keys($ids,true);$csv=implode(',',$ids);
    $catalogRows=$pdo->query("SELECT id,is_active,country_id,country_name FROM catalog_hotels WHERE id IN (".$csv.")")->fetchAll(PDO::FETCH_ASSOC)?:[];$present=[];$active=[];$countries=[];
    foreach($catalogRows as $r){$id=(int)$r['id'];$present[$id]=true;if((int)$r['is_active']===1)$active[$id]=true;$country=(string)($r['country_name']?:$r['country_id']);if(!isset($countries[$country]))$countries[$country]=['present'=>0,'active'=>0];$countries[$country]['present']++;if((int)$r['is_active']===1)$countries[$country]['active']++;}
    $searched=[];$attempts=$pdo->query("SELECT hotel_ids_json,status FROM tour_matrix_collection_attempts WHERE criterion='hotel_batch' AND status IN ('success','empty') AND hotel_ids_json IS NOT NULL")->fetchAll(PDO::FETCH_ASSOC)?:[];
    foreach($attempts as $r){$batch=json_decode((string)$r['hotel_ids_json'],true);if(!is_array($batch))continue;foreach($batch as $id){$id=(int)$id;if(isset($set[$id]))$searched[$id]=true;}}
    ...
    return ['total'=>count($ids),'present'=>count($present),'active'=>count($active),'searched'=>count($searched),'observed'=>count($observed),'missing'=>count($ids)-count($present),'countries'=>$countries];
}
function matrix_log_priority_coverage(PDO $pdo): void {
    $x=matrix_priority_coverage($pdo);echo "TOP500_COVERAGE total={$x['total']} catalog_present={$x['present']} active={$x['active']} searched={$x['searched']} observed={$x['observed']} missing={$x['missing']}\n";
    ksort($x['countries']);foreach($x['countries'] as $name=>$c){echo "TOP500_COUNTRY country=".str_replace(' ','_',trim((string)$name))." present={$c['present']} active={$c['active']}\n";}
}
    ...
    foreach($pairs as $pair){
        ...
        foreach($windows as $w){
            foreach($batches as $bi=>$batch){
                ...
                $c[]=['criterion'=>'hotel_batch','target_key'=>$key,'batch_family'=>$family,'coverage'=>$coverage[$family]??0,'window_order'=>$w['order'],'departure_id'=>$dep,'country_id'=>$country,'country_name'=>$pair['country_name'],'region_id'=>null,'hotel_ids'=>$batch,'priority'=>$priority,'date_from'=>$w['from'],'date_to'=>$w['to'],'last'=>$last['hotel_batch|'.$key]??'0000-00-00 00:00:00'];
            }
            foreach($regions as $r){$rid=(int)$r['id'];$key=$dep.':'.$country.':r'.$rid.':'.$w['month'].':p'.$w['part'];$c[]=['criterion'=>'resort','target_key'=>$key,'coverage'=>0,'window_order'=>$w['order'],'departure_id'=>$dep,'country_id'=>$country,'country_name'=>$pair['country_name'],'region_id'=>$rid,'hotel_ids'=>[],'priority'=>false,'date_from'=>$w['from'],'date_to'=>$w['to'],'last'=>$last['resort|'.$key]??'0000-00-00 00:00:00'];}
            $key=$dep.':'.$country.':'.$w['month'].':p'.$w['part'];$c[]=['criterion'=>'month','target_key'=>$key,'coverage'=>0,'window_order'=>$w['order'],'departure_id'=>$dep,'country_id'=>$country,'country_name'=>$pair['country_name'],'region_id'=>null,'hotel_ids'=>[],'priority'=>false,'date_from'=>$w['from'],'date_to'=>$w['to'],'last'=>$last['month|'.$key]??'0000-00-00 00:00:00'];
        }
    }
...
$budget=matrix_int($argv,'budget',9,1,30);$months=matrix_int($argv,'months',12,1,18);$pollAttempts=matrix_int($argv,'poll-attempts',20,1,30);$pollSeconds=matrix_int($argv,'poll-seconds',2,1,10);$pdo=v2_data_db();matrix_log_priority_coverage($pdo);$targets=matrix_pick_balanced(matrix_candidates($pdo,$months),$budget);
...
foreach($targets as $t){
    ...
    $attempt=matrix_attempt_start($pdo,$t);$sid=null;echo "MATRIX_START criterion={$t['criterion']} priority=".(!empty($t['priority'])?'top500':'normal')." coverage=".(int)($t['coverage']??0)." country={$t['country_id']}:".str_replace(' ','_',trim((string)$t['country_name']))." key={$t['target_key']} hotels=".count($t['hotel_ids'])." from={$t['date_from']} to={$t['date_to']}\n";
    try{
...
matrix_log_priority_coverage($pdo);echo "MATRIX_COLLECTOR_DONE targets=".count($targets)." ok={$ok} failed={$failed} written={$total}\n";if($failed===count($targets))exit(1);
