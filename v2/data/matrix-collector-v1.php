...
function matrix_int(array $argv, string $name, int $default, int $min, int $max): int {
    $raw = matrix_arg($argv,$name,null); if ($raw===null || $raw==='') $raw=(string)$default;
    $n=filter_var($raw,FILTER_VALIDATE_INT); return $n===false?$default:max($min,min($max,(int)$n));
}
function matrix_search_id(array $x): ?int {
    foreach (['searchId','id'] as $k) { $v=filter_var($x[$k]??null,FILTER_VALIDATE_INT); if($v!==false && (int)$v>0)return(int)$v; }
    return null;
}
function matrix_complete(array $x): bool {
    return (int)($x['progress']??0)>=100 || strtolower(trim((string)($x['status']??'')))==='complete';
}
function matrix_rows(array $payload): array {
    if (array_is_list($payload)) return $payload;
    foreach(['hotels','items','results'] as $k) if(is_array($payload[$k]??null)) return $payload[$k];
    return [];
}
function matrix_fetch_results(int $searchId): array {
    $last=null;
    foreach([10000,5000,2000,1000,500,100] as $limit){
        try { return matrix_rows(v2_data_tv_get('/tours/search/'.$searchId,['limit'=>$limit])); }
        catch(Throwable $e){$last=$e;}
    }
    throw new RuntimeException('matrix results unavailable',0,$last);
}
function matrix_month_windows(int $months): array {
    $base=(new DateTimeImmutable('first day of next month'))->setTime(0,0);
    $out=[];
    for($i=0;$i<$months;$i++){
        $start=$base->modify('+'.$i.' months'); $end=$start->modify('last day of this month');
        $cursor=$start; $part=1;
        while($cursor<=$end){
            $sliceEnd=min($cursor->modify('+20 days'),$end);
            $out[]=['month'=>$start->format('Y-m'),'part'=>$part,'from'=>$cursor->format('Y-m-d'),'to'=>$sliceEnd->format('Y-m-d')];
            $cursor=$sliceEnd->modify('+1 day'); $part++;
        }
    }
    return $out;
}
function matrix_pairs(PDO $pdo,int $limit=30): array {
    $sql="SELECT dc.departure_id,dc.country_id,d.name departure_name,c.name country_name
      ...
      ORDER BY dc.departure_id,dc.country_id LIMIT ".(int)$limit;
    return $pdo->query($sql)->fetchAll(PDO::FETCH_ASSOC) ?: [];
}
function matrix_last_attempts(PDO $pdo): array {
...
function matrix_candidates(PDO $pdo,int $months): array {
    ...
    usort($c,static function($a,$b){
        ...
        $rank=['hotel_batch'=>0,'resort'=>1,'month'=>2]; return $rank[$a['criterion']]<=>$rank[$b['criterion']];
    });
    return $c;
}
...
function matrix_attempt_start(PDO $pdo,array $t): int {
    $s=$pdo->prepare("INSERT INTO tour_matrix_collection_attempts
      (criterion,target_key,departure_id,country_id,region_id,hotel_ids_json,date_from,date_to,nights_from,nights_to,status,started_at)
      VALUES (:criterion,:target_key,:departure,:country,:region,:hotels,:df,:dt,5,14,'started',:started)");
    $s->execute(['criterion'=>$t['criterion'],'target_key'=>$t['target_key'],'departure'=>$t['departure_id'],'country'=>$t['country_id'],'region'=>$t['region_id'],'hotels'=>$t['hotel_ids']?json_encode($t['hotel_ids']):null,'df'=>$t['date_from'],'dt'=>$t['date_to'],'started'=>(new DateTimeImmutable())->format('Y-m-d H:i:s')]);
    return (int)$pdo->lastInsertId();
}
function matrix_attempt_finish(PDO $pdo,int $id,string $status,?int $searchId,int $rows,int $written,?string $error=null): void {
    $s=$pdo->prepare("UPDATE tour_matrix_collection_attempts SET status=:status,search_id=:sid,rows_received=:rows,observations_written=:written,error_text=:error,finished_at=:finished WHERE id=:id");
    $s->execute(['status'=>$status,'sid'=>$searchId,'rows'=>max(0,$rows),'written'=>max(0,$written),'error'=>$error?mb_substr($error,0,1000):null,'finished'=>(new DateTimeImmutable())->format('Y-m-d H:i:s'),'id'=>$id]);
}

$budget=matrix_int($argv,'budget',9,1,30); $months=matrix_int($argv,'months',12,1,18); $pollAttempts=matrix_int($argv,'poll-attempts',20,1,30); $pollSeconds=matrix_int($argv,'poll-seconds',2,1,10);
$pdo=v2_data_db(); $targets=matrix_pick_balanced(matrix_candidates($pdo,$months),$budget);
if(!$targets){echo "MATRIX_COLLECTOR_NO_TARGETS\n";exit;}
$total=0;$ok=0;$failed=0;
foreach($targets as $t){
    $search=['departureId'=>$t['departure_id'],'countryId'=>$t['country_id'],'dateFrom'=>$t['date_from'],'dateTo'=>$t['date_to'],'nightsFrom'=>5,'nightsTo'=>14,'adults'=>2,'currency'=>'RUB','onlyCharter'=>false,'onlyDirect'=>false];
    ...
    try{
        ...
        if(!$complete){matrix_attempt_finish($pdo,$attempt,'timeout',$sid,0,0,'bounded poll timeout');$failed++;continue;}
        $rows=matrix_fetch_results($sid);
        $trusted=[]; foreach($rows as $h){if(!is_array($h))continue;$hc=is_array($h['country']??null)?(int)($h['country']['id']??0):0;if($hc>0&&$hc!==(int)$t['country_id'])continue;$trusted[]=$h;}
        if(!$trusted){matrix_attempt_finish($pdo,$attempt,'empty',$sid,0,0);$ok++;continue;}
        $r=v2_data_observe_search_results($trusted,['searchId'=>$sid,'departureId'=>$t['departure_id'],'countryId'=>$t['country_id'],'adults'=>2,'currency'=>'RUB','source'=>'scheduled_monitor','maxHotels'=>5000,'maxTours'=>50000]);
        $written=(int)($r['written']??0); $total+=$written; matrix_attempt_finish($pdo,$attempt,'success',$sid,count($trusted),$written);$ok++;
        echo "MATRIX_OK criterion={$t['criterion']} search={$sid} hotels=".count($trusted)." written={$written}\n";
