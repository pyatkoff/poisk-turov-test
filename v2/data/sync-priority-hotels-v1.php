...
function ph_upsert(PDO $pdo,array $row,string $now): bool {
    ...
    $countryName=trim((string)($country['name']??($row['countryName']??'')));
    if($countryId<=0||$countryName==='') return false;
...
$ok=0;$notFound=0;$invalid=0;$errors=0;
foreach($batch as $id){
    try {
        $row=ph_pick_hotel(v2_data_tv_get('/hotels/'.$id),$id);
        if(ph_upsert($pdo,$row,$now)){ $ok++; echo "PRIORITY_HOTEL_SYNC_OK hotel=$id\n"; }
        else { $invalid++; echo "PRIORITY_HOTEL_SYNC_INVALID hotel=$id\n"; }
    } catch(Throwable $e){
        $message=$e->getMessage();
        if(str_contains($message,'HTTP 404')){ $notFound++; echo "PRIORITY_HOTEL_SYNC_404 hotel=$id\n"; }
        else { $errors++; fwrite(STDERR,"PRIORITY_HOTEL_SYNC_ERROR hotel=$id ".mb_substr($message,0,300)."\n"); }
    }
    usleep(150000);
}
$presentAfter=$pdo->query("SELECT id FROM catalog_hotels WHERE id IN ($csv) AND is_active=1")->fetchAll(PDO::FETCH_COLUMN)?:[];
$remaining=max(0,count($ids)-count(array_unique(array_map('intval',$presentAfter))));
echo "PRIORITY_HOTEL_SYNC_SUMMARY requested=".count($batch)." synced=$ok not_found=$notFound invalid=$invalid errors=$errors remaining=$remaining\n";
if($errors>0) exit(2);
