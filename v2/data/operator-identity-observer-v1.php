...
function v2_operator_identity_ensure_schema(PDO $pdo): bool
{
    try {
        $exists = $pdo->query("SHOW TABLES LIKE 'tour_operator_identity_observations'");
        if ($exists && $exists->fetchColumn() !== false) return true;
        $migration = __DIR__ . '/migrations/20260913-tour-operator-identity.sql';
        $sql = is_file($migration) ? file_get_contents($migration) : false;
        if (!is_string($sql) || trim($sql) === '') return false;
        $pdo->exec($sql);
        $verify = $pdo->query("SHOW TABLES LIKE 'tour_operator_identity_observations'");
        return $verify && $verify->fetchColumn() !== false;
    } catch (Throwable $e) {
        return false;
    }
}
...
function v2_data_observe_operator_identities(array $hotels, array $context): array
{
    ...
    try {
        ...
        $pdo = v2_data_db();
        if (!v2_operator_identity_ensure_schema($pdo)) {
            return ['seen'=>count($rows),'written'=>0,'reason'=>'identity_schema_unavailable'];
        }
