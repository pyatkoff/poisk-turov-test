...
const localNames=new Set(['stars','rating','price_from','price_till','region','subregion']);
...
function current(){const f=new FormData(form);return{stars:Number(f.get('stars')||0),rating:ratingThreshold(f.get('rating')),priceFrom:Number(f.get('price_from')||0),priceTo:Number(f.get('price_till')||0),region:String(f.get('region')||''),subregion:String(f.get('subregion')||'')};}
function sourceBounds(){const lc=lifecycle(),s=lc&&lc.snapshot||{};return{stars:Number(s.hotelCategory||0),rating:ratingThreshold(s.hotelRating),priceFrom:Number(s.priceFrom||0),priceTo:Number(s.priceTo||0),region:first(s.regionIds),subregion:first(s.subregionIds)};}
...
function tourMatches(t,filters){const price=Number(t&&t.price||0);if(filters.priceFrom&&(!price||price<filters.priceFrom))return false;if(filters.priceTo&&(!price||price>filters.priceTo))return false;return true;}
function filteredHotel(h,filters){if(!hotelMatches(h,filters))return null;const tours=Array.isArray(h&&h.tours)?h.tours:[],needsTourFilter=!!(filters.priceFrom||filters.priceTo);if(!needsTourFilter)return h;if(!tours.length)return null;const kept=tours.filter(t=>tourMatches(t,filters));if(!kept.length)return null;const prices=kept.map(t=>Number(t&&t.price||0)).filter(v=>Number.isFinite(v)&&v>0);return Object.assign({},h,{tours:kept,price:prices.length?Math.min(...prices):h.price});}
...
function captureRendered(event){if(applying||window.__DS2ResultsRailApplying)return;const items=event&&event.detail&&Array.isArray(event.detail.items)?event.detail.items:[];sourceItems=items.slice();lastRenderedCount=items.length;if(!items.length)return;const filters=current();if(Object.values(filters).some(Boolean)&&canUseLocal(filters))apply();}
...
window.V2ResultsLocalFiltersV1={apply,current,sourceBounds,canUseLocal,matches,filteredHotel,isLocalTarget,get sourceItems(){return sourceItems.slice();},get shown(){return lastRenderedCount;},names:Array.from(localNames),version:2};
})();