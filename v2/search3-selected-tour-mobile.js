...
function sync(){var visible=!selected.hidden&&getComputedStyle(selected).display!=='none'&&selected.children.length>0;document.body.classList.toggle('search3-selected-open',visible);bar.hidden=!visible;if(!visible)return;var source=selected.querySelector('.selected-price');var target=bar.querySelector('[data-s3-selected-price]');if(source&&target){var text=String(source.textContent||'').replace(/^Стоимость\s*/i,'').trim();target.textContent=text||'—';}}
function submitLead(){var form=selected.querySelector('.lead-form');if(!form)return;var submit=form.querySelector('button[type=submit],input[type=submit]');if(typeof form.requestSubmit==='function'){try{form.requestSubmit(submit||undefined);return;}catch(_e){}}if(submit){submit.click();return;}form.scrollIntoView({behavior:'smooth',block:'start'});var focus=form.querySelector('input:not([type=hidden]),textarea,select');if(focus)focus.focus();}
document.addEventListener('click',function(e){var btn=e.target&&e.target.closest&&e.target.closest('[data-s3-selected-lead]');if(!btn)return;e.preventDefault();submitLead();});
