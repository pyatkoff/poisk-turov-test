...
function decorateLead(scope){const form=scope&&scope.querySelector('.lead-form');if(!form)return;form.classList.add('checkout-lead');if(!form.querySelector('.checkout-help')){const note=document.createElement('div');note.className='checkout-help';note.textContent='Менеджер получит параметры выбранного тура. Если выберете рейс выше, его данные и пересчитанная стоимость тоже попадут в заявку.';const submit=form.querySelector('button[type="submit"]');if(submit)submit.insertAdjacentElement('beforebegin',note);else form.appendChild(note);}}
