...
// Admission uses the existing server-validated, offer-bound surcharge receipt.
// Never infer inclusion from an operator, a positive price or a listing reference.
function fuelIncluded(tour){
 const fact=tour&&tour.search_surcharge,base=tour&&tour.base_search_price,price=tour&&tour.price;
 const money=value=>value&&typeof value.amount==='string'&&/^(?:0|[1-9][0-9]{0,11})(?:\.[0-9]{1,2})?$/.test(value.amount)&&value.currency==='RUB';
 const same=(a,b)=>money(a)&&money(b)&&a.amount===b.amount;
 return !!(fact&&fact.schema_version===1&&fact.provider==='andromeda'&&fact.state==='estimated'
  &&fact.arithmetic_applied===true&&fact.final_price_verified===false&&fact.surcharge_scope==='party'
  &&same(base,fact.search_price)&&amount(base)>0&&same(price,fact.search_price_with_surcharge)
  &&fact.search_price_with_surcharge.source==='derived_search_estimate'&&money(fact.party_surcharge)
  &&['andromeda_get_flights_transport','andromeda_get_flights_transport_converted'].includes(fact.party_surcharge.source));
}
function normalizeTour(tour,hotel){const offerContext=context(tour&&tour.offer_context);if(!tour||tour.provider!=='andromeda'||!fuelIncluded(tour)||!offerContext||offerContext.offer_ref!==tour.offer_ref)return null;const price=amount(tour.price),currency=String(tour.price&&tour.price.currency||'');if(!price||currency!=='RUB'||!/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/.test(tour.checkin||'')||!Number.isInteger(tour.nights)||tour.nights<1||tour.nights>60)return null;const content=hotel&&hotel.andromeda_content||{},image=safeUrl(content.image_url),providerCode=operatorHotelCodeFromImage(image);return{id:'andromeda:'+tour.offer_ref,provider:'andromeda',offerRef:tour.offer_ref,offerContext,listingPriceRef:listingReference(tour.listing_price_ref),selectionEnabled:false,quoteRequired:true,fuelIncluded:true,price,currency:'RUB',date:tour.checkin.split('-').reverse().join('.'),nights:tour.nights,meal:{name:String(tour.meal||'').slice(0,160)},roomType:String(tour.room||'').slice(0,300),placement:String(tour.placement||'').slice(0,160),operator:{name:String(tour.operator||'').slice(0,180)},providerHotelUrl:safeUrl(content.hotel_url),providerImageUrl:image,providerHotelCode:providerCode};}
...
const canonicalOwner=canonicalLocalCandidate()&&root.Search3CanonicalProfilesV1.current&&root.Search3CanonicalProfilesV1.current();
...
function restoreToggleFocus(toggleKey){if(!toggleKey||typeof document.querySelectorAll!=='function')return;const toggle=Array.from(document.querySelectorAll('.tour-more-toggle')).find(node=>node.dataset.hotelId===toggleKey);if(toggle)toggle.focus({preventScroll:true});}
function canonicalSync(options){
 if(!canonicalOwner||typeof canonicalOwner.upsertLegacyOffer!=='function'||typeof canonicalOwner.setLegacyHotelState!=='function')return false;
 const focused=document.activeElement,toggleKey=focused&&focused.matches&&focused.matches('.tour-more-toggle')?focused.dataset.hotelId:'',accepted=detailTours();
 canonicalOwner.clearOffers('andromeda');
 providerItems().forEach(input=>{const hotel=normalizeHotel(input);if(!hotel||hotel.mappingStatus!=='resolved'||!/^[1-9][0-9]*$/.test(String(hotel.id)))return;const key=String(hotel.id),expansion=expansions.get(key),expansionState=expansion?{status:expansion.status,count:expansion.tours.filter(t=>normalizeTour(t,input)).length}:{status:'idle',count:0};canonicalOwner.setLegacyHotelState(key,expansionState,{source:'andromeda'});hotel.tours.forEach(tour=>{const entry=accepted.get(tour.offerRef),state=details.get(tour.offerRef),decorated=entry&&entry.key===contextKey(tour.offerContext)?Object.assign({},tour,{providerDetail:Object.assign({eligible:true,open:false,status:'idle'},state||{})}):tour;canonicalOwner.upsertLegacyOffer(key,decorated,{source:'andromeda'});});});
 canonicalOwner.refresh(active&&!active.done?{empty:false}:options||null);restoreToggleFocus(toggleKey);return true;
}
function legacyRender(options){const focused=document.activeElement,toggleKey=focused&&focused.matches&&focused.matches('.tour-more-toggle')?focused.dataset.hotelId:'';rendering=true;try{const provider=providerItems(),accepted=detailTours(),items=merge(baseItems,provider).map(h=>{const eligible=h&&((h.provider==='andromeda')||(Array.isArray(h.providers)&&h.providers.includes('andromeda')));if(!eligible)return h;const key=String(h.id),expansion=expansions.get(key),tours=(h.tours||[]).map(t=>{const entry=accepted.get(t.offerRef);if(t.provider!=='andromeda'||!entry||entry.key!==contextKey(t.offerContext))return t;const state=details.get(t.offerRef);return Object.assign({},t,{providerDetail:Object.assign({eligible:true,open:false,status:'idle'},state||{})});});return Object.assign({},h,{tours,andromedaExpansion:expansion?{status:expansion.status,count:expansion.tours.filter(t=>normalizeTour(t,h)).length}:{status:'idle',count:0}});});const result=originalRender(items,options||lastOptions);restoreToggleFocus(toggleKey);return result;}finally{rendering=false;}}
function render(options){return canonicalOwner?canonicalSync(options):legacyRender(options);}
if(!canonicalOwner)renderer.render=function(list,options){if(rendering)return originalRender(list,options);baseItems=Array.isArray(list)?list.filter(hotel=>hotel&&hotel.provider!=='andromeda'):[];lastOptions=Object.assign({},options||{});return legacyRender(active&&!active.done?Object.assign({},lastOptions,{empty:false}):lastOptions);};
...
async function expandHotel(key){
 ...
 const seed=hotel&&(hotel.tours||[]).filter(t=>normalizeTour(t,hotel)).map(t=>context(t.offer_context)).find(c=>c&&!c.hotel_scope);if(!seed||!/^[1-9][0-9]*$/.test(key))return;
...
async function loadHotel(run,key,state){
 ...
 try{let page=1,total=1;do{
  ...
  if(!response.ok||!payload.ok||!data||data.provider!=='andromeda'||data.generation!==run.generation||data.page!==page||data.grouped!==false||!Array.isArray(data.hotels)||data.hotels.some(h=>!validHotel(h)||String(h.local_id)!==key))throw new Error('invalid_hotel_page');
  const refs=new Set(state.tours.map(t=>t.offer_ref));data.hotels.forEach(h=>h.tours.filter(t=>normalizeTour(t,h)).forEach(t=>{if(!refs.has(t.offer_ref)){state.tours.push(t);refs.add(t.offer_ref);}}));
