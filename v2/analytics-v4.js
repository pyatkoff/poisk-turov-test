(function(){'use strict';
if(window.V2Analytics&&Number(window.V2Analytics.version||0)>=4)return;
const startedAt=new Map(),cfg=window.V2_CONFIG||{};
const SAFE_KEYS=new Set(['searchId','durationMs','resultsCount','sort','tourId','hotelId','hotel','region','country','price','currency','flightIndex','flightNumber','isDefault','flightPrice','priceDelta','previousResultsCount','addedResultsCount','phase','errorCode','leadId','deduplicated']);
...
function priceValue(v){if(v&&typeof v==='object'&&v.value!==undefined)return Number(v.value||0);return Number(v||0);}
ensureMetrika();
...
window.addEventListener('v2:search-complete',e=>{const d=e.detail||{},id=Number(d.searchId||0),start=startedAt.get(id),items=Array.isArray(d.items)?d.items:[];track('search_complete',{searchId:id,durationMs:start?Math.round(performance.now()-start):0,resultsCount:items.length});if(id)startedAt.delete(id);});
...
window.addEventListener('v2:search-continued',e=>{const d=e.detail||{};track('search_continued',{searchId:Number(d.searchId||0),previousResultsCount:Number(d.previousResultsCount||0),addedResultsCount:Number(d.addedResultsCount||0)});});
...
window.addEventListener('v2:flight-selected',e=>{const d=e.detail||{},f=d.flight||{},forward=Array.isArray(f.forward)?f.forward:[],base=Number(d.tour&&d.tour.price||0),fp=priceValue(f.price);track('flight_selected',{tourId:String(d.tour&&d.tour.id||''),flightIndex:Number(d.index||0),flightNumber:String(forward[0]&&forward[0].number||''),isDefault:!!f.isDefault,flightPrice:fp,priceDelta:base&&fp?fp-base:0});});
...
window.addEventListener('v2:lead-submitted',e=>{const d=e.detail||{};track('lead_submitted',{searchId:Number(d.searchId||0),tourId:String(d.tourId||''),leadId:Number(d.leadId||0),deduplicated:!!d.deduplicated});});
...
document.addEventListener('click',e=>{const b=e.target&&e.target.closest&&e.target.closest('.hotel-info-toggle');if(!b||String(b.textContent||'').trim()!=='Об отеле')return;const card=b.closest('.hotel-card');track('hotel_open',{hotelId:String(card&&card.dataset.hotelId||'')});},true);
window.V2Analytics={track,safeParams,ensureMetrika,version:4,metrikaCounter:metrikaCounter()};
})();
