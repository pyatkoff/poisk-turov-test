...
function v2_seo_production_identity_registry_validate(
    ...
    foreach((is_array($evidence['pages']??null)?$evidence['pages']:[]) as $i=>$page){
        ...
        $seen[$path]=true;
        if(!isset($expected[$path])){ $errors[]='identity_path_outside_registry:'.$path; continue; }
        $want=$expected[$path];
    ...
    $expectedPaths=array_keys($expected); sort($expectedPaths,SORT_STRING);
    $actualPaths=array_keys($seen); sort($actualPaths,SORT_STRING);
    if($actualPaths!==$expectedPaths)$errors[]='identity_path_set_mismatch';
    ...
    $counts=['country'=>0,'resort'=>0,'hotel_tours'=>0];
    foreach($pages as $page) if(isset($counts[$page['type']])) $counts[$page['type']]++;

    return [
        'state'=>$errors===[]?'production_identity_registry_valid':'production_identity_registry_invalid',
        'integrity_ok'=>$errors===[],
        'domain'=>$domain,
        'observed_at_epoch'=>$observedAt,
        'max_age_seconds'=>$maxAgeSeconds,
        ...
        'type_counts'=>$counts,
        'pages'=>$pages,
        'identity_registry_sha256'=>$fingerprint,
        'errors'=>$errors,
