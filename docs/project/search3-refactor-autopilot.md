...
Текущие pins и продолжение находятся только в `AUTOPILOT_STATE.json.current_task`.
Конечный остаток приёмки описан в активном product plan; исторические «следующие»
действия ниже не являются исполняемой очередью.
...
Историческое наблюдение `м&#178;` закрыто каноническим entity decoder и source regression;
не повторять исправление. Следующее действие читать из свежего `current_task`.
