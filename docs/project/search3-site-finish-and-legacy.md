...
5. Desktop acceptance through the actual search to the visible lead form passed. Complete physical iPhone/Safari, long-value and unavailable-flight/recovery checks. Preview does not establish receipt of a real lead.
...
Integration fixtures exercise the real readers with an in-memory SQLite database: past cheap offers, stale snapshots, empty results, city filters, valid today/future offers and limiting. Date tests include Moscow midnight with a different host timezone and leap/year boundaries. No live search/API/lead calls. Implementation `1fddaf5e4e1c1285cb768050352c3d0a1ab0cdc4` passed core CI `33934082041` and visual CI `33934082023`; no public artifact or production change yet.
...
Exact published version: source `6a20f3dd4145cd9a3fd08bc8964d66725af43e5c`, tree `51ce6288dd83c31e4853de6454a3399ed1dcdf34`, build `33937670389`, artifact `9960716789`, deployment `33937843736`, deployment draft #1343. The artifact contains 715 files; its ZIP, inner archive, manifest and payload-checksum hashes are recorded in `AUTOPILOT_STATE.json`. Live checks confirmed ages `0/17`, completed real catalogue search with 80 hotels / 399 tours, opened an hotel, a concrete tour and its available flight, continued to the tour summary and displayed the lead form. No lead was submitted. Preview Metrika, external widget and lead delivery remain disabled; internal PHP/config paths are denied, protected production fingerprints remained unchanged, and the previous preview target was retained for rollback.

The selected supplier offer presented the party as `3 взрослых + 1 ребёнок` after the request entered two adults with ages `0` and `17`. This may be supplier normalization of a 17-year-old as an adult, but it is not yet classified as expected behavior. Investigate without changing supplier, price or lead contracts; fix only if the presentation or handoff is confirmed wrong. Physical iPhone/Safari and unavailable-flight/retry recovery remain before owner acceptance. Main and production were not changed.
