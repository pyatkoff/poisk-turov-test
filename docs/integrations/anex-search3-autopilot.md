...
| INT #1717 | Auth/catalog and price capture completed;50 actual offers normalized. Private offer store/resolver ready; #1778 has13 proposed identities, none accepted. Provider start/resume handler now joins client→private store→public offers, standalone namespaced hotel groups; default disabled, CI pending | Priority is coordinated SEARCH consumption and isolated HTTP route with existing private storage/auth/account budget. Full hotel matching does not block standalone Andromeda cards. Exact saved pilot only; no API/SQL recapture. Shared renderer #1779 has another writer: handoff through #996. See andromeda-search-handoff.md and reports/andromeda-search-runtime-20260909.json; not published |
