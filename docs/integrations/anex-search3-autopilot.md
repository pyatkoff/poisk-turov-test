...
| INT #1717 | #1782/source73cb5956: provider start/resume joins client→private store→public response.33 new checks + saved50 offers/16 hotel groups; run34375204139 and Security34375204180 passed, artifact10113547043. Resume reproduces saved response without API; standalone namespaced hotel groups, no new accepted mappings | Next: scoped HTTP route using existing owner-login.php authority and durable private state/account budget, then coordinated SEARCH consumption. Full matching does not block standalone source cards. Exact pilot only; default disabled; supplier0/SQL0/not published. Renderer #1779 has another writer: handoff #996. See andromeda-search-handoff.md and reports/andromeda-search-runtime-20260909.json |
