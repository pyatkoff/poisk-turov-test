...
| INT #1717 | Wiki/auth/search прочитаны. Draft1763: клиент/HTTPS/CLI, executable860cd5e5;92+10 offline checks. LIVE run34361393106/job102498987683: login+townfrom success,270 городов,2calls,no replay; artifact10107960917. Credentials подключены. Draft1761 provider context готов | Далее отдельный bounded catalog capture: сохранить очищенные справочники, выбрать ровно одно направление по полученным ID и прочитать state/all. Первый access уже completed, не повторять его как проверку. Pilot сохранил только count; полный каталог/лимиты/цены/нормализатор ещё не подтверждены. Не merge runtime drafts вслепую: ANEX push вызывает live workflow. Audit: reports/andromeda-client-readiness-20260909.json |
