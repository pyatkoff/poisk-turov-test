...
затем AGENTS/scoped README и этот документ на свежей ANEX-ветке. Координатор #3419:
...
| INT #1717 | #1901 sourcefdd099e3/artifact10159104848 установлен: #1914/run34510605994/receipt10165617032,6 hashes/5 activated/backups verified; НЕ переустанавливать. #1917 CLI06f93033/#1925 relay2a352a49 готовы. #1927 ЯВНО ОДОБРЕН и merged main d285a76c на ОДИН max1broninit capture; разрешение НЕ использовано. #1936 A3.2 projection sourcef093f540 merged6cee4476; Security34524990752/offline34524990681 SUCCESS:59 projection+78 evidence+113 CLI checks, current mapping regressions passed. После merge run34525081025 offline success/private verify-access skipped. Projection source-only, НЕ installed/live; tourist buyer net отделён от agency cost, quote/selection остаются false. Audit reports/andromeda-package-public-projection-20260910.json + installation audit; [A1–A6](multi-provider-search-plan.md) | next_action=A3.1: получить actual CURRENT publicSelection DTO+local country из общей saved-details приёмки SEARCH #1646 (handoff5623816616), затем без нового допуска выполнить ровно ОДИН уже одобренный capture #1927 через #1917/#1925; durable reservation/max1broninit/private readback. После captured read использовать готовую #1936 projection для exact catalogKey↔selected-offer binding, состава и buyerMoneys tourist price; это всё ещё package_unverified, quote=false. DTO пока не получен, команду без него не посылать; не фабриковать/не брать historical/expired и не запускать replacement search. #1914 install/#1927 permission/password/relay повторно не запрашивать. get_flights/calc отдельны и не запускаются автоматически; unknown не replay. SEARCH сохраняет A2/UI/publication; A4 ждёт реального пакета/цены. Mobile/Safari/production gate сохранены |
...
| Координация #3419/#1493 | Четыре направления, один UI-владелец; общая модель в multi-provider-search-plan.md | Отдельный bounded handoff в свежую release; не merge старую ANEX-основу и не создавать третий renderer |
