...
| INT #1647 | PR1728: пять таблиц/263 persistent досье/readback. PR1741: отдельный owner login,39 core/21 HTTP/4 publisher и Security/build passed. Владелец подтвердил: существующего admin/Bitrix нет. Readiness34349122412/artifact10102950128 выполнена, SQL0/supplier0 | Применить закреплённую одноразовую публикацию, затем владелец задаёт пароль через HTTPS activation. Write=false до реальных gates; raw token/password не хранить в Git/Actions. Storage/auth inventory не повторять. Audit: reports/anex-owner-login-20260909.json |
