...
| INT #1717 | A1 producer/consumer defect fixed in #1863 source ac9f1606, merged ccc442bc: server preserves resolved only after existing accepted-local/catalog/filter guards. Actual PHP→current shared JS baseline rejects / patched accepts; CI34463350468, Security34463350411, required build34463350379 passed. Single-file artifact10146455195; audit reports/andromeda-public-projection-20260910.json. Not published. A2 #1846 remains context-only; A3 #1837 execution blocked | Publish only the checked API file via approved exact runtime path; verify hash and common-preview variants/details/return. Do not publish the old-base whole-site artifact or start legacy supplier queues. Dispatch/private-runner access still required; A3 package is separate. No matching/shared UI changes; unknown searches not replayed |
