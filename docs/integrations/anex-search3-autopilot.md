...
| INT #1717 | Auth/catalog/price/pages и own-preview grouped expansion/details работают. #1829/#1831 merged в release; #1830 details и общая публикация остаются у SEARCH. A3 #1833 source f420bb08: выключенный private package client/coordinator, 303 PHP checks + ANEX regression + Security passed; аудит reports/andromeda-package-client-20260910.json. Runtime/quote/selection этим пакетом не опубликованы | A1–A6 из [подробного плана Андромеды](multi-provider-search-plan.md): SEARCH завершает общую приёмку/публикацию. Независимый INT next — подключить существующие private lock/save/readback/current mapping helpers, закрепить один актуальный saved offer и получить broninit package; подтвердить PRICES.id/claiminc/catalogKey и только затем public composition/price projection. Полный claim приватный; unknown не replay. Matching/новые связи/повтор auth/catalog запрещены. Sol high рекомендован, фактическая модель текстом prompt не меняется |
