...
| INT #1647 | PR1728: пять таблиц/263 persistent досье/readback и сохранность. PR1735: installed auth inventory, SQL0/supplier0; fixed Bitrix/auth paths отсутствуют, файл review entry присутствует | Установить фактический вход владельца AnyTour, затем trusted adapter/точное происхождение entrypoint/HTTP gates. Ссылки на Bitrix в repo не означают установленную авторизацию. CLI env не доказывает web-env; реальный вход/доступ не проверены, write не включать. Storage не повторять. Audit: reports/anex-review-auth-inventory-20260909.json |
