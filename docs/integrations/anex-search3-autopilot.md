...
| INT #1717 | A1 confirmed producer/consumer defect: published server projection omits mapping_status while common provider requires resolved. Fix preserves resolved only after existing accepted-local/catalog/filter guards; actual PHP→shared JS regression in reports/andromeda-public-projection-20260910.json. Code/CI candidate in fix/int-andromeda-projection-status-20260910; publication not claimed. A2 #1846 remains context-only; A3 #1837 cannot dispatch from default | Verify baseline rejection and patched accepted output plus Security; retain exact single-file API artifact for approved publication and common-preview variants/details/return. No matching or shared UI changes. A3 real package still needs an approved execution path; unknown supplier searches are not replayed |
