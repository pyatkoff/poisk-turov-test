...
| INT #1717 | Auth/catalog/price/pages и own-preview grouped expansion/details уже работают. #1829 merged в release; #1830 details checked, #1831 fixes/browser acceptance у SEARCH. Точные source/ссылки в multi-provider-search-plan.md и свежих комментариях #1717; это не подтверждение shared publication | Выполнять A1–A6 из [подробного плана Андромеды](multi-provider-search-plan.md): сначала совместная приёмка/публикация #1830/#1831, затем выбранный offer context, пакет/quote, preview lead handoff. Пока UI занят — независимая серверная A3. Matching/новые связи/повтор auth/catalog запрещены этой задаче. Sol high рекомендован; фактическая модель автозапуска не изменяется текстом prompt |
