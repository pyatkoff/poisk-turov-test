...
Актуализировано: 2026-09-15 (published Andromeda receipt fix / distinct quote cohort). Репозиторий: только `pyatkoff/poisk-turov-test`.
...
### 4.1 AdditionalPricesDaily — evidence и границы применимости
...
Коррекция current state из #1685 comment5671662430: прежний общий blocker про отсутствие namespace binding устарел. APD v4 artifact10295753048 фиксирует `TourId=778 → tour=778`; SearchTour v6 artifact10296903698 — `TourId=778 → 198 rows/tourKey778/operator5/town46`. #2362 retained adult/child APD, #2367 batch6 и #2370 exact-context same-day cache уже merged; повторять binding778 и завершённые captures нельзя. Это не равенство APD, Tourvisor fuelCharge, flight surcharge, package или final money и не новый допуск production arithmetic. Следующий P0 evidence — новая программа/дата/native currency только с установленным request context; blind numeric probes запрещены, missing/ambiguous остаётся unknown.

### 4.2 Andromeda — estimated listing и цена, реально выданная API

Owner-approved модель: базовая PRICE + доступная транспортная доплата даёт `estimated`, не final; отсутствие сбора остаётся unknown. Цель 80% не runtime gate.

**Published:** #2480 source `6c174898ebf4876ae024f7173d53e1f6dc4837f7`, checked artifact10372349715/run34906332715, установлен через #2483/run34911727400/receipt10374721928. Все16 hashes и обе HTTP entrypoints проверены; относительно #2464 изменился только `andromeda-price-observation.php`. Исправлен реальный отказ `resolveServed`: selected offer сохраняет normalizer metadata, receipt — amount/currency; известные metadata проверяются отдельно, money/context/session/TTL/hash guards прежние. #2461 assembly refactor и #2468 точное quote HTTP исключение завершены; повторная установка, access repair или generic wrapper не нужны. Source/receipt details: #1717 comment5672614339.

`price_observation` из #2433 сравнивает оценку, пересчитанную во время актуализации, с `calc`. Для отдельного измерения search-response→calc существующий HTTP owner сохраняет точные `base_price`, `served_price`, время и контекст в текущей PHP-сессии (до2000 receipts/900с) и выдаёт `tour.listing_price_ref`. Quote читает только серверный receipt по этому ref, после обычной current-offer/mapping/session/TTL проверки, и сохраняет `served_price_observation` в прежний completed quote checkpoint. Нет ref, receipt устарел/вытеснен/не совпал — измерения нет, новая оценка не подставляется. Сам пересчёт и no-replay контракты не меняются.

**External receiving:** SEARCH #1646 должен сохранить поле рядом с конкретным предложением и вернуть его как top-level `listing_price_ref` в существующий quote request. Внедрение JS и полный browser flow пока не подтверждены; публикация INT сама по себе не доказывает показ туристу, live coverage или 80% accuracy.

**Sealed evidence:** E2E v1/run34874678175 и v2/run34901558904 terminal/no-replay. #2477 исправил ложное diagnostic сравнение base metadata. #2479 архивное чтение завершено: Swiss Heaven Sharming Inn/local453,2026-12-15,7n,2a+child8; `112724 RUB + 240 USD × 89.82 = recorded estimate134280.80 RUB`. В v2 `calc_calls=0`, listing receipt ещё не записан; это не actualization pair. Не восстанавливать TTL, не повторять архивные readers или эти supplier операции.

**Следующий конкретный проход:** существующий `andromeda_search_surcharge_e2e.php` подготовлен для одной materially different группы Moscow→Egypt/ANEX,2026-12-20,10n,3adults,no children; operation `andromeda-search-surcharge-e2e-1717-v3-egypt-2026-12-20-3a-10n`, generation17171220, runtime pin6c174898. Сохранённые listing money попадают в текущий result до quote, чтобы поздний отказ не терял уже проверенные суммы. Исполнение только после exact-source focused/Security и control CI, через прежний owner-controlled workflow, с current budget headroom≥100 и durable reservation. Один новый search, прежняя bounded package/unique-flight/quote цепочка, без bron/bron_ticket/реальных заявок. Pending source preparation не считать supplier evidence; терминальный результат и pins фиксируются в #1717/#996, не запускаются заново по этой строке.
