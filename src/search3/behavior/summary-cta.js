...
function correctTradeoffs(){const variants=[...document.querySelectorAll('#selectedTour .flight-variant')],prices=variants.map(v=>{const price=v.querySelector('.flight-choice>b');if(price)price.textContent=price.textContent.replace(' ₽','\u00a0₽');return localizedPrice(price)});if(variants.length<2||prices.some(v=>!v))return;const minimum=Math.min(...prices);variants.forEach((variant,index)=>{const label=[...variant.querySelectorAll('.flight-choice-tradeoffs span')].find(node=>/минимальн/i.test(node.textContent));if(!label)return;const delta=prices[index]-minimum;label.textContent=delta?'+'+new Intl.NumberFormat('ru-RU').format(delta)+' ₽ к минимальной':'Самая низкая цена';label.classList.toggle('is-best-price',!delta)})}
...
function enterLead(source){const r=root(),form=r&&r.querySelector('.lead-form');if(!r||!form)return false;r.classList.remove('search3-final-review');r.classList.add('search3-lead-entry');window.dispatchEvent(new CustomEvent('search3:lead-entry',{detail:{active:true,source:source||'native'}}));const phone=form.querySelector('input[name="phone"]');if(phone)phone.focus({preventScroll:true});(phone||form).scrollIntoView({behavior:'smooth',block:phone?'center':'start'});return true}
