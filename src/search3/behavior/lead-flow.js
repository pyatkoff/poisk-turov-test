...
(function(){'use strict';
const statusClass='search3-lead-status';
...
function messengerButton(link,name,variant){
return link.url?'<a class="search3-msg-btn search3-msg-btn--'+variant+'" href="'+link.url+'" target="_blank" rel="noopener noreferrer" aria-label="'+(link.direct?'Продолжить общение в '+name:'Открыть AnyTour в '+name)+'">'+(link.direct?'Продолжить в '+name:'Открыть '+name)+'</a>':'<span class="search3-msg-btn search3-msg-btn--disabled" aria-disabled="true">'+name+' недоступен</span>';
}
function ensureStatus(form){let box=form.querySelector('.'+statusClass);if(!box){box=document.createElement('div');box.className=statusClass;box.hidden=true;form.prepend(box)}return box}
...
box.innerHTML=('<div class="'+statusClass+'__icon '+statusClass+'__icon--sending">✈</div><div><h3>Отправляем заявку…</h3><p>Пожалуйста, подождите. Это займёт несколько секунд.</p><ol><li>Сохраняем ваши данные</li><li>Отправляем заявку менеджеру</li><li>Подтверждаем получение</li></ol></div>');
}else if(state==='success'){
const leadId=detail&&detail.leadId?'<p class="search3-lead-id">Заявка № '+String(detail.leadId)+'</p>':'';
const max=messengerButton(links.max,'MAX','max'),tg=messengerButton(links.telegram,'Telegram','tg');
box.innerHTML=('<div class="'+statusClass+'__icon '+statusClass+'__icon--success">✓</div><div><h3>Заявка отправлена!</h3><p>Менеджер уже получил информацию о поездке и свяжется с вами в ближайшее время.</p>')+leadId+'<strong>Хотите открыть AnyTour в мессенджере?</strong><div class="search3-messenger-actions">'+max+tg+'<button type="button" class="search3-stay-site">Остаться на сайте</button></div></div>';
}else if(state==='error'){
box.innerHTML=('<div class="'+statusClass+'__icon '+statusClass+'__icon--error">!</div><div><h3>Не удалось отправить заявку</h3><p>Проверьте данные и попробуйте ещё раз.</p><div class="search3-error-actions"><button type="button" class="search3-retry-lead">Повторить отправку</button><button type="button" class="search3-edit-lead">Изменить данные</button></div><p class="search3-error-preserve-note">Ваши данные сохранены. Можно изменить их и отправить заявку снова.</p></div>');
}else{return false}
return true;
}
function clearState(){const form=qs('#selectedTour .lead-form');if(!form)return;delete form.dataset.search3LeadState;const box=form.querySelector('.'+statusClass);if(box)box.hidden=true}
