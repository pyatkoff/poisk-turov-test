...
function fields(){return[field,budgetField,mealField,providerField,operatorField,categoryField,ratingField,seaField];}
function active(){return!!(normalize(input.value)||Number(categorySelect.value)||mealSelect.value||providerSelect.value||operatorSelect.value||budgetActive||Number(ratingSelect.value)||Number(seaSelect.value));}
...
function ensure(){
  ...
  input=field.querySelector('input');status=field.querySelector('small');budgetInput=budgetField.querySelector('input');budgetLabel=budgetField.querySelector('small');mealSelect=mealField.querySelector('select');mealPresets=mealField.querySelector('.search3-filter-presets');providerSelect=providerField.querySelector('select');operatorSelect=operatorField.querySelector('select');categorySelect=categoryField.querySelector('select');categoryPresets=categoryField.querySelector('.search3-filter-presets');ratingSelect=ratingField.querySelector('select');seaSelect=seaField.querySelector('select');
...
function reset(event){ensure();const trigger=event&&event.currentTarget;input.value='';categorySelect.value='0';mealSelect.value='';providerSelect.value='';operatorSelect.value='';ratingSelect.value='0';seaSelect.value='0';budgetActive=false;window.V2Results.rerender();if(trigger)focusAfterReset(trigger);}
function clear(event){
  ...
  sourceItems=[];projectedItems=[];unmatched=new Set();input.value='';categorySelect.value='0';mealSelect.value='';providerSelect.value='';operatorSelect.value='';ratingSelect.value='0';seaSelect.value='0';budgetActive=false;budgetInput.value='0';cards().forEach(card=>{card.hidden=false;});status.textContent='';fields().forEach(node=>{node.hidden=true;});mobilePanel.open=false;syncContainers(0);
}
