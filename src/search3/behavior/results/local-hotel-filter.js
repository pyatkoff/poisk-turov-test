...
function apply(){
  ...
  if(summary&&list.length)summary.textContent=(active()?'Показано отелей: '+shown+' из '+list.length:'Найдено отелей: '+list.length)+(window.V2SearchLifecycle?.restoredAt?' · сохранённая выдача':' · цены из текущего поиска');
