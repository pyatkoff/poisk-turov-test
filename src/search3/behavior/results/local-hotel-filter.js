...
function ensure(){
  ...
  categoryField=document.createElement('div');categoryField.className='search3-category-filter';categoryField.hidden=true;categoryField.innerHTML='<label><span>Категория отеля</span><select aria-describedby="search3HotelFilterStatus search3CategoryCoverage"><option value="0">Любая категория</option></select></label><small id="search3CategoryCoverage" data-search3-category-coverage></small><div class="search3-filter-presets" role="group" aria-label="Быстрый выбор категории" hidden></div>';
  ...
  input=field.querySelector('input');status=field.querySelector('small');budgetInput=budgetField.querySelector('input');mealSelect=mealField.querySelector('select');mealPresets=mealField.querySelector('.search3-filter-presets');operatorSelect=operatorField.querySelector('select');regionSelect=regionField.querySelector('select');categorySelect=categoryField.querySelector('select');categoryPresets=categoryField.querySelector('.search3-filter-presets');categoryCoverage=categoryField.querySelector('[data-search3-category-coverage]');ratingSelect=ratingField.querySelector('select');ratingCoverage=ratingField.querySelector('[data-search3-rating-coverage]');seaSelect=seaField.querySelector('select');
...
function syncHotelFacets(){
  ...
  const categoryState=numericCoverage(categories,.95),category=syncSelect(categoryField,categorySelect,categoryState.available?categories.filter(value=>value>0):[],'Любая категория',value=>value+'★');
  categoryCoverage.textContent=categoryState.available?'Категория указана у '+categoryState.known+' из '+categoryState.total+' отелей':'';
