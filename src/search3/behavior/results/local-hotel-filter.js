...
function apply(){
  ...
  if(summary&&list.length)summary.textContent=(active()?'Показано отелей: '+shown+' из '+list.length:'Найдено отелей: '+list.length)+(window.V2SearchLifecycle?.hotelDetail?.profileOnly?' · предложения нужно обновить':sourceItems.some(h=>(h.tours||[]).some(t=>t.restoredListing===true))?' · сохранённая выдача':' · цены из текущего поиска');
