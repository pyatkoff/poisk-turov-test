...
function render(){
  ...
  if(saved.length){const list=node('div','search3-shortlist__items');saved.forEach(item=>{
    ...
    [['Вылет',item.date],['Ночей',String(item.nights)],['Туристы',item.adults+' взр.'+(item.childs?' · '+item.childs+' дет.':'')],['Питание',item.meal],['Номер',item.room],['Размещение',item.placement||'Уточняется'],['Оператор',item.operator||'Уточняется']].forEach(pair=>{const wrap=node('div');wrap.append(node('dt','',pair[0]),node('dd','',pair[1]));facts.append(wrap);});
