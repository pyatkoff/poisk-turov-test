...
let saved=[],currentItems=[],root=null,message='',persistence=true,returnFocus=null,mobileOpen=false;
...
function compareState(){const different=new Set(),labels=[],dimensions=[['hotel','отель',item=>item.hotelName],['country','страна',item=>item.country],['region','курорт',item=>item.region],['date','дата вылета',item=>item.date],['nights','ночи',item=>item.nights],['party','туристы',item=>String(item.adults)+':'+String(item.childs)],['meal','питание',item=>item.meal],['room','номер',item=>item.room],['placement','размещение',item=>item.placement],['operator','туроператор',item=>item.operator],['price','цена при сохранении',item=>item.observedPrice]];if(saved.length<2)return{different,labels,minimumPrice:null};dimensions.forEach(dimension=>{if(new Set(saved.map(item=>text(dimension[2](item)))).size>1){different.add(dimension[0]);labels.push(dimension[1]);}});return{different,labels,minimumPrice:different.has('price')?Math.min(...saved.map(item=>Number(item.observedPrice))):null};}
...
function render(){
  ...
  if(saved.length){const list=node('div','search3-shortlist__items');saved.forEach(item=>{
    ...
    [['date','Вылет',displayDate(item.date)],['nights','Ночей',String(item.nights)],['party','Туристы',partyLabel(item)],['meal','Питание',item.meal],['room','Номер',item.room],['placement','Размещение',item.placement||'Уточняется'],['operator','Оператор',item.operator||'Уточняется']].forEach(fact=>{const wrap=node('div'),label=compare.different.has(fact[0])?fact[1]+' · отличается':fact[1];wrap.append(node('dt','',label),node('dd','',fact[2]));facts.append(wrap);});
...
load();render();
window.Search3Shortlist={storageKey,items:()=>saved.map(item=>Object.assign({},item)),get persistent(){return persistence;},get mobileOpen(){return mobileOpen;},version:1};
})();