...
function factRows(item){return[['date','Вылет',displayDate(item.date)],['nights','Ночей',String(item.nights)],['party','Туристы',partyLabel(item)],['meal','Питание',meal(item)],['room','Номер',room(item)],['placement','Размещение',placement(item.placement)||'Уточняется'],['operator','Оператор',item.operator||'Уточняется']];}
