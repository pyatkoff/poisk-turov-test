...
function factRows(item){return[['date','Вылет',displayDate(item.date)],['nights','Ночей',String(item.nights)],['party','Туристы',partyLabel(item)],['meal','Питание',item.meal],['room','Номер',item.room],['placement','Размещение',item.placement||'Уточняется'],['operator','Оператор',item.operator||'Уточняется']];}
