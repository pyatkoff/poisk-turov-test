...
window.addEventListener('v2:tour-selected',()=>{const r=root();if(r){r.classList.remove('search3-final-review');setTimeout(()=>{syncHeading(r)},0)}});
window.addEventListener('v2:flight-selected',()=>setTimeout(ensure,0));
window.addEventListener('v2:booking-review',()=>setTimeout(()=>{syncHeading(root())},0));
