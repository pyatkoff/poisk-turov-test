...
The live search journey through results → tour variant → selected-tour → return remains production-stable at 768/1024/1440. Homepage and the first editorial-route hierarchy pass are aligned with DS2, and the representative country re-audit has now removed the confirmed tablet orphan-card geometry from country detail facts.

Continue from `/country/` catalog into representative country detail hero/facts/related-destination and the handoff into `/hot/` and `/poisk-turov/` at 375/430/768/1024/1440. Consume the current broad standalone-content production evidence first. If it remains green and no second country defect is confirmed, move to shared breadcrumb/footer/CTA hierarchy across the country/editorial shell rather than manufacturing route-local styling. Preserve the short search handoff and do not make editorial surfaces as dense as the search application.

Whole-product site-wide score remains **7.2/10**. The country tablet fix is real and broadly shared by representative detail pages, but one incremental responsive correction is not enough evidence for another whole-product score increase; the broader country/editorial re-audit must support that move.
