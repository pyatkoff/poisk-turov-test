...
Updated: 2026-08-30 02:53 +02:00
...
The mature tour-search flow remains the strongest product surface at approximately **8.5–9.0/10**, but that score is not the score of the whole public site. The current site-wide score remains **7.2/10 pending production visual evidence for the latest standalone hierarchy pass**. Do not raise it from branch code or CI alone.

Current scorecard before the latest production verification:
...
- PR #293 deployed successfully with public pages, unchanged lead bridge and live search smoke green. PR #294 then repaired the post-deploy visual guard so it explicitly tracks the known legacy search-header seam instead of failing every deploy.
- PR #295 added architecture/test source-of-truth documentation only; no runtime behavior changed.
- Current branch `visual/standalone-orientation-v2` applies the already-shared breadcrumb, section-heading and branded search-callout primitives to `/hot/`, `/contacts/`, `/how-to-buy/` and `/rb/`. It removes ad-hoc inline spacing from those pages, gives each page a consistent orientation layer, and makes the handoff into the common search visually coherent. No new primitive or business logic was invented.
...
1. **Validate and deploy the current standalone hierarchy pass** across 375/430/768/1024/1440; verify `/hot/`, `/contacts/`, `/how-to-buy/`, `/rb/`, plus unchanged search/lead smoke.
