...
Active owner-directed continuation is [Search3 site completion, issue #996](https://github.com/pyatkoff/poisk-turov-test/issues/996). Finish the new design across the site before expanding SEO or refactoring. The coherent whole-site preview is published from checked source `000f6a63052aba7c29efca6f0082cd6068ec188d`; desktop real-catalogue acceptance reaches the lead form, and the unavailable-flight/retry branches pass a deterministic browser check at `928f25ec`. Result-card readability improvements are prepared and visually checked at `17b674fc`, but are not yet in the published whole-site preview. Physical iPhone/Safari remains. The existing search-only deploy still accepts only nine search files and is unchanged. The stable DS2 mode/phase/stage identifiers remain unchanged; Search3 is the active substage.
...
Prepared after the currently published preview: result cards at `17b674fc54fa6b49aaa73379bcbdc35bfccfda28` use taller mobile hotel photos and readable fact/action typography while preserving a compact desktop layout. Visual run `33941679993` passed at 375/430/1024/1440; artifact `9962036028` was inspected. Date, nights, meal and flight are not repeated below the price; that line contains only the party composition. This correction must be included in the next separately pinned whole-site preview artifact before it can be treated as live preview evidence.
