...
Next: complete post-merge production verification for #667. Then continue site-wide DS2 convergence with the homepage route-local token/header CSS debt: remove duplicate global token ownership and obsolete `.at-home-header*` implementation only where the canonical shared header already owns the live UI. In parallel resume the results-first visual pass: results toolbar → hotel/tour-card hierarchy → selected-tour handoff at 768/1024/1440, and sample `/country/`, `/country/turkey/` plus another representative country page at 375/430/768/1024/1440. Preserve the owner-approved loaded-results local filtering architecture; do not replace it with a new Tourvisor search on every result-filter click.
