...
The live search journey through results → tour variant → selected-tour → return is production-stable at 768/1024/1440. Homepage DS2 base/final ownership is internally aligned for the proof-card grid, and the public-route pass has now removed confirmed issues on `/hot/`, `/contacts/` and `/how-to-buy/`.

Continue with `/rb/`. Inspect existing 375/430/768/1024/1440 evidence and loaded CSS ownership before editing. If `/rb/` has no confirmed hierarchy/spacing/wrapping/overflow/shared-shell defect, do not manufacture one: move directly to a representative country/editorial re-audit and whole-site score reassessment. Preserve the short search handoff and do not make editorial surfaces as dense as the search application.

Keep the whole-product site-wide score at **7.1/10** until a broader visible product slice materially improves. The latest `/how-to-buy/` change improves semantic process hierarchy but is not a large visible redesign by itself.
