...
EXPERIMENT='int_anex_distinct_charter_apd_20260917_v2'
DATES=['2026-10-12','2026-10-13','2026-10-14']
SPEC={'experiment_id':EXPERIMENT,'departure_local_id':1,'country_local_id':4,'dates':DATES,'nights':7,'adults':2,'children':0,'currency':'RUB','target_distinct_programs':5}
...
function ds_main(): array {
  ...
  $out=['schema_version'=>1,'experiment_id'=>'int_anex_distinct_charter_apd_20260917_v2','status'=>'blocked',
  ...
  try{
    ...
    $expected=['experiment_id'=>'int_anex_distinct_charter_apd_20260917_v2','departure_local_id'=>1,'country_local_id'=>4,
    ...
    $ledger=$home.'/.anytour-ops/int_anex_distinct_charter_apd_20260917_v2';
    ...
    $q->execute([1,4]);$local=$q->fetch(PDO::FETCH_ASSOC);
    ...
    $country=ds_dict_id(ds_req($client,'SearchTour_STATES',['TOWNFROMINC'=>$departure],$reqs),[$local['country_name'],'Турция','Turkey']);
    ...
    $out=['schema_version'=>1,'experiment_id'=>'int_anex_distinct_charter_apd_20260917_v2','status'=>'completed','observed_at'=>gmdate('c'),
...
def main():
    if len(sys.argv)!=2:
        raise SystemExit('usage: int_anex_distinct_charter_apd_v2.py OUTPUT')
    out=Path(sys.argv[1])
    try:
