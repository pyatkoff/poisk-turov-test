...
$diagnose = static function (string $directory) use ($readJson, $hist): array {
    ...
    foreach (new DirectoryIterator($directory) as $entry) {
        ...
        if (!is_array($criteria) || !is_array($store)
            || ($state['generation'] ?? null) !== 17171830
            || !is_int($store['created_at'] ?? null)) continue;
        if ((string)($criteria['CHECKIN_BEG'] ?? '') !== '20261018'
            || (string)($criteria['CHECKIN_END'] ?? '') !== '20261018'
...
$argv = [
    ...
    '--date-from=2026-10-18','--date-to=2026-10-18',
    '--nights=7','--adults=2','--meal=7',
    '--generation=17171830','--max-captures=6',
];
$argc = count($argv);
...
$result = [
    ...
    'scope' => [
        ...
        'date_from' => '2026-10-18', 'date_to' => '2026-10-18',
