...
if($site===false||!is_dir($site)||basename($site)!=='anytoour.ru'||$runtime===false)throw new RuntimeException('ANEX_OWNER_SCOPE_ROOT');
...
if(!defined('ANEX_API_TOKEN')||!defined('ANEX_B2B_TOKEN'))throw new RuntimeException('ANEX_OWNER_SCOPE_PRIVATE_CONFIG');

$config=$site.'/config.php';if(!is_file($config)||is_link($config))throw new RuntimeException('ANEX_OWNER_SCOPE_PROJECT_CONFIG');
require_once $config;
...
if(!is_file($collectorScript)||is_link($collectorScript))throw new RuntimeException('ANEX_OWNER_SCOPE_COLLECTOR');
...
if(!is_array($collector)||($collector['status']??null)!=='complete')throw new RuntimeException('ANEX_OWNER_SCOPE_COLLECTOR_RECEIPT');
