...
stage="$RUNNER_TEMP/writer-runtime-v5"
...
helper_sha="$(sha256sum "$stage/app/integrations/tourvisor-anytour-offer-autosave.php" | awk '{print $1}')"
archive="$RUNNER_TEMP/writer-runtime-v5.tar.gz"
tar -C "$stage" -czf "$archive" .
archive_sha="$(sha256sum "$archive" | awk '{print $1}')"
echo "PRODUCTION_WRITER_V5_PAYLOAD_OK api=$api_sha helper=$helper_sha archive=$archive_sha"
...
set +e
ssh "${opts[@]}" "$remote" bash -s -- "$OPERATION_ID" "$remote_archive" "$BASE_SHA" "$api_sha" "$helper_sha" "$archive_sha" <<'REMOTE'
set -euo pipefail
operation="$1"; archive="$2"; source_sha="$3"; expected_api_sha="$4"; expected_helper_sha="$5"; expected_archive_sha="$6"
site="$HOME/www/anytoour.ru"
parent="$HOME/www"
runtime_app="$parent/app"
runtime_v2="$parent/v2"
ledger="$HOME/.anytour-ops/$operation"
state_dir="$site/.cache/tourvisor-offer-autosave"
work=''
app_created=0
v2_created=0
api_activated=0
...
before_json="$(DOCUMENT_ROOT="$site" php -r '
  ...
  echo json_encode([
    "refreshes"=>$q("SELECT COUNT(*) FROM anytour_offer_refreshes WHERE provider=\"tourvisor\" AND status=\"completed\""),
...
test "$(sha256sum "$work/app/integrations/tourvisor-anytour-offer-autosave.php" | awk '{print $1}')" = "$expected_helper_sha"
...
grep -Fq "'tourvisor-offer-autosave'" "$work/app/integrations/tourvisor-anytour-offer-autosave.php"
grep -Fq 'Require all denied' "$work/app/integrations/tourvisor-anytour-offer-autosave.php"
...
state_name="anytour-tourvisor-offer-$(printf '%s' "$search_id" | sha256sum | awk '{print $1}').json"
state_path="$state_dir/$state_name"
test -s "$state_path"
test -s "$state_dir/.htaccess"
test "$(cat "$state_dir/.htaccess")" = 'Require all denied'
state_http="$(curl -sS -o /dev/null -w '%{http_code}' --max-time 30 "https://anytoour.ru/.cache/tourvisor-offer-autosave/$state_name")"
test "$state_http" = 403 -o "$state_http" = 404
...
test "$complete" = 1
php -r '$d=json_decode(file_get_contents($argv[1]),true,16,JSON_THROW_ON_ERROR);if(!is_int($d["terminal_at"]??null))exit(2);' "$state_path"
...
php -r '$d=json_decode(file_get_contents($argv[1]),true,16,JSON_THROW_ON_ERROR);if(!is_int($d["saved_at"]??null))exit(2);' "$state_path"
...
receipt="$(SOURCE_SHA="$source_sha" BEFORE_API_SHA="$before_api_sha" API_SHA="$expected_api_sha" HELPER_SHA="$expected_helper_sha" SEARCH_ID="$search_id" RESULT_HOTELS="$result_hotels" STATE_HTTP="$state_http" BEFORE_JSON="$before_json" AFTER_JSON="$after_json" php -r '
  $out=[
    "schema_version"=>1,
    "operation"=>"int-tourvisor-production-writer-20260917-v5",
    "status"=>"completed",
    "release_sha"=>getenv("SOURCE_SHA"),
    "before_api_sha256"=>getenv("BEFORE_API_SHA"),
    "after_api_sha256"=>getenv("API_SHA"),
    "helper_sha256"=>getenv("HELPER_SHA"),
    "search_id"=>(int)getenv("SEARCH_ID"),
    "result_hotels"=>(int)getenv("RESULT_HOTELS"),
    "state_http"=>(int)getenv("STATE_HTTP"),
    "state_terminal_persisted"=>true,
    "state_saved_persisted"=>true,
...
if [ "$remote_rc" -eq 0 ]; then
  ...
  jq -e '.status == "completed" and (.after.refreshes > .before.refreshes) and (.after.offers > 0) and (.after.ready > 0) and (.state_http == 403 or .state_http == 404) and .state_terminal_persisted and .state_saved_persisted and .lead_writes == 0 and .metrika_writes == 0 and .booking_calls == 0 and .mapping_writes == 0' "$RUNNER_TEMP/receipt.json" >/dev/null
