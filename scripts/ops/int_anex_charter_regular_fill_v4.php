...
if(!is_file($collectorScript)||is_link($collectorScript))throw new RuntimeException('ANEX_COMBINED_COLLECTOR');

$oldArgv=$argv;$oldArgc=$argc;
$argv=[
    $collectorScript,'--departure=1','--country=4',
    '--date-from=2026-11-03','--date-to=2026-11-09',
    '--nights=7','--adults=2','--meal=7',
    '--max-expands=60','--max-apd=300','--generation=25061854',
];
$argc=count($argv);
ob_start();
...
if(!is_array($collector)||($collector['status']??null)!=='complete')throw new RuntimeException('ANEX_COMBINED_COLLECTOR_RECEIPT');
...
$result=[
    ...
    'scope'=>['departure_id'=>1,'country_id'=>4,'date_from'=>'2026-11-03','date_to'=>'2026-11-09','nights'=>7,'adults'=>2,'meal'=>'7'],
