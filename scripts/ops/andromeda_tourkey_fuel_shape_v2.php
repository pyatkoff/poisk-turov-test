...
foreach(array_slice($argv,1) as $arg){
    if(!str_starts_with($arg,'--')||!str_contains($arg,'=')) throw new InvalidArgumentException('ANDROMEDA_FUEL_SHAPE_ARG');
    [$k,$v]=explode('=',substr($arg,2),2);$args[$k]=$v;
}
$get=static function(string $key)use($args):string{
    $v=$args[$key]??null;
    if(!is_string($v)||$v==='') throw new InvalidArgumentException('ANDROMEDA_FUEL_SHAPE_ARG_'.$key);
    return $v;
};
$int=static function(string $value,int $min,int $max):int{
    if(!preg_match('/\A(?:0|[1-9][0-9]{0,9})\z/D',$value)) throw new InvalidArgumentException('ANDROMEDA_FUEL_SHAPE_INT');
    $n=(int)$value;if($n<$min||$n>$max) throw new InvalidArgumentException('ANDROMEDA_FUEL_SHAPE_INT');return $n;
};
$date=static function(string $value):string{
    if(!preg_match('/\A(\d{4})-(\d{2})-(\d{2})\z/D',$value,$m)||!checkdate((int)$m[2],(int)$m[3],(int)$m[1])) throw new InvalidArgumentException('ANDROMEDA_FUEL_SHAPE_DATE');
    return $value;
};
$idText=static function(mixed $value):?string{
    ...
    return preg_match('/\A[A-Za-z0-9_-]{1,128}\z/D',$value)?$value:null;
};
$numericId=static function(mixed $value):?string{
    if(is_int($value))$value=(string)$value;
    if(!is_string($value))return null;$value=trim($value);
    return preg_match('/\A[1-9][0-9]{0,18}\z/D',$value)?$value:null;
};
...
if($site===false||basename($site)!=='anytoour.ru'||$privateConfig===false||!is_file($privateConfig)
    ...
    ||!preg_match('/\A[a-z0-9][a-z0-9._-]{20,160}\z/D',$operation)) throw new RuntimeException('ANDROMEDA_FUEL_SHAPE_ROOT');
$targets=array_values(array_filter(array_map('trim',explode(',',$get('tour-keys'))),static fn($v)=>$v!==''));
if(count($targets)<1||count($targets)>10||count(array_unique($targets))!==count($targets)) throw new InvalidArgumentException('ANDROMEDA_FUEL_SHAPE_TARGETS');
foreach($targets as $target)if(!preg_match('/\A[0-9]{1,18}\z/D',$target))throw new InvalidArgumentException('ANDROMEDA_FUEL_SHAPE_TARGET');
...
if(!is_array($config)||($config['enabled']??null)!==true)throw new RuntimeException('ANDROMEDA_FUEL_SHAPE_CONFIG');
...
$dateFrom=$date($args['date-from']??'2026-09-19');$dateTo=$date($args['date-to']??$dateFrom);if($dateTo<$dateFrom)throw new InvalidArgumentException('ANDROMEDA_FUEL_SHAPE_DATE_RANGE');
$nightsFrom=$int($args['nights-from']??'7',1,28);$nightsTo=$int($args['nights-to']??'10',$nightsFrom,28);$adults=$int($args['adults']??'2',1,6);
$fixedCheckIn=$date($args['fixed-checkin']??'2026-09-19');$fixedNights=$int($args['fixed-nights']??'7',$nightsFrom,$nightsTo);$generation=$int($args['generation']??'17171904',1,2147483647);
...
$searchRef=hash('sha256','tourkey-fuel-shape-v2|'.$operation.'|'.json_encode($criteria,JSON_THROW_ON_ERROR));
...
$session=$client->privateSession();if(!is_string($session['sid']??null)||!is_int($session['expires']??null))throw new RuntimeException('ANDROMEDA_FUEL_SHAPE_SESSION');
...
$coverageOverall=['rows'=>0,'with'=>0,'missing'=>0,'invalid'=>0,'keys'=>[]];$coverageOps=[];
$cover=static function(array $raw)use(&$coverageOverall,&$coverageOps,$numericId):void{
    $op=$numericId($raw['operatorKey']??null)??'__missing_or_invalid__';
    if(!isset($coverageOps[$op]))$coverageOps[$op]=['rows'=>0,'with'=>0,'missing'=>0,'invalid'=>0,'keys'=>[]];
    ++$coverageOverall['rows'];++$coverageOps[$op]['rows'];
    if(!array_key_exists('tourKey',$raw)||$raw['tourKey']===null||$raw['tourKey']===''){++$coverageOverall['missing'];++$coverageOps[$op]['missing'];return;}
    $tour=$numericId($raw['tourKey']);if($tour===null){++$coverageOverall['invalid'];++$coverageOps[$op]['invalid'];return;}
    ++$coverageOverall['with'];++$coverageOps[$op]['with'];$coverageOverall['keys'][$tour]=true;$coverageOps[$op]['keys'][$tour]=true;
};
$finalizeCoverage=static function(array $row):array{$keys=array_keys($row['keys']);sort($keys,SORT_STRING);return ['rows'=>$row['rows'],'with_tourKey'=>$row['with'],'missing_tourKey'=>$row['missing'],'invalid_tourKey'=>$row['invalid'],'distinct_tourKey_count'=>count($keys),'tourKeys'=>$keys];};

$candidates=[];$pages=0;$priceRows=0;$target=1;
for($page=1;$page<=$target;++$page){
    if($page>AnyTourAndromedaPaginationV1::MAX_PAGES)throw new RuntimeException('ANDROMEDA_FUEL_SHAPE_PAGE_BUDGET');
    ...
    foreach(($payload['PRICES']??[]) as $raw)if(is_array($raw))$cover($raw);
    ...
    foreach(($normalized['offers']??[]) as $offer){
        ...
        $rawId=$byRef[$offerRef];$already=false;foreach(($candidates[$tour]??[]) as $row)if($row['supplier_offer_id']===$rawId){$already=true;break;}if($already)continue;
        $candidates[$tour][]=['supplier_offer_id'=>$rawId,'identity'=>['operatorKey'=>$operator,'tourKey'=>$tour,'programKey'=>$idText($ctx['program_ref']??null),'spoKey'=>$idText($ctx['spo_ref']??null),'checkIn'=>$fixedCheckIn,'nights'=>$fixedNights,'adult'=>(int)$offer['adults'],'child'=>(int)$offer['children']]];
    }
...
ksort($coverageOps,SORT_STRING);$coverageByOp=[];foreach($coverageOps as $op=>$row)$coverageByOp[$op]=$finalizeCoverage($row);
$rawCoverage=['scope'=>'before_mapping_and_operator_filtering','overall'=>$finalizeCoverage($coverageOverall),'operators'=>$coverageByOp];
$selected=[];foreach($targets as $tour){if(count($candidates[$tour]??[])<3)throw new RuntimeException('ANDROMEDA_FUEL_SHAPE_THIRD_NOT_FOUND');$selected[$tour]=$candidates[$tour][2];}

[$operatorLogin,$operatorPassword]=anytour_andromeda_operator_credentials_from_config($config);
$secretEcho=null;$secretEcho=static function(mixed $value,string $sid,int $depth=0)use(&$secretEcho):bool{
...
$flatten=static function(array $doc,string $container,string $leaf):array{$out=[];foreach(($doc[$container]??[]) as $block){if(!is_array($block)||!is_array($block[$leaf]??null))continue;foreach($block[$leaf] as $row)if(is_array($row))$out[]=$row;}return $out;};
...
$transportFact=static function(array $transport)use($safeScalar):array{$out=[];foreach(['key','name','type','datebeg','dateend','groupId'] as $k)if(array_key_exists($k,$transport)&&is_scalar($transport[$k]))$out[$k]=$safeScalar($transport[$k]);return $out;};
$safeError=static function(mixed $error):array{$encoded=json_encode($error,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES|JSON_THROW_ON_ERROR);$text=is_string($error)?$error:$encoded;$category='unclassified';foreach(['flight_or_freight'=>'/(?:flight|freight|avia|airline|airfare|рейс|перел[её]т|авиа)/iu','claim_or_package'=>'/(?:claim|package|booking|заявк|пакет|брони)/iu','service'=>'/(?:service|услуг)/iu','price_or_fare'=>'/(?:price|cost|fare|tariff|цен|тариф)/iu'] as $c=>$rx)if(preg_match($rx,$text)){ $category=$c;break; }return ['shape'=>gettype($error),'error_sha256'=>hash('sha256',$encoded),'reason_category'=>$category];};
...
foreach($targets as $tour){
    ...
    try{
        ...
        if(($response['status']??null)!==200||!is_string($response['body']??null))throw new RuntimeException('ANDROMEDA_FUEL_SHAPE_HTTP');
        $reply=json_decode($response['body'],true,64,JSON_THROW_ON_ERROR);if(!is_array($reply))throw new RuntimeException('ANDROMEDA_FUEL_SHAPE_JSON');
        if($secretEcho($reply,$session['sid']))throw new RuntimeException('ANDROMEDA_FUEL_SHAPE_SECRET_ECHO');
        ...
        $docs=$reply['claimDocument']??null;$doc=is_array($docs)&&isset($docs[0])&&is_array($docs[0])?$docs[0]:null;
        $entry['claimDocument_count']=is_array($docs)?count($docs):0;
        if($doc===null){$entry['status']='raw_shape_without_claim_terminal_no_replay';$results[]=$entry;continue;}
        $entry['claim_tourKey']=$idText($doc['tourKey']??null);$entry['claim_spoKey']=$idText($doc['spoKey']??null);$entry['freightExternal']=$safeScalar($doc['freightExternal']??null);
        $services=$flatten($doc,'services','service');$transports=$flatten($doc,'transports','transport');
    ...
    }catch(Throwable $e){$entry['status']='failed_terminal_no_replay';$m=$e->getMessage();$entry['failure_class']=is_string($m)&&preg_match('/\A[A-Z0-9_:-]{1,96}\z/D',$m)?$m:'ANDROMEDA_FUEL_SHAPE_FAILURE';}
    $results[]=$entry;
}

$out=['version'=>2,'operation'=>$operation,'source'=>$source,'status'=>'completed_terminal_no_replay','scope'=>['departureId'=>$departure,'countryId'=>$country,'dateFrom'=>$dateFrom,'dateTo'=>$dateTo,'fixedCheckIn'=>$fixedCheckIn,'fixedNights'=>$fixedNights,'adult'=>$adults,'child'=>0,'currency'=>'RUB'],'price_pages'=>$pages,'price_rows'=>$priceRows,'raw_tourKey_coverage'=>$rawCoverage,'tourKeys'=>$targets,'specimens'=>$results,'broninit_calls'=>count($targets),'get_flights_calls'=>0,'changeservice_calls'=>0,'calc_calls'=>0,'booking_calls'=>0,'db_writes'=>0,'autosave_calls'=>0,'search3_publication'=>0,'replay_allowed'=>false];
