...
if($site===false||!is_dir($site)||basename($site)!=='anytoour.ru'||$runtime===false)throw new RuntimeException('ANEX_OWNER_SCOPE_8N_V1_ROOT');

$private=dirname($site,2).'/.anytoour-anex/search3-preview.php';
if(!is_file($private)||is_link($private))throw new RuntimeException('ANEX_OWNER_SCOPE_8N_V1_PRIVATE_CONFIG');
require_once $private;
if(!defined('ANEX_API_TOKEN')||!defined('ANEX_B2B_TOKEN'))throw new RuntimeException('ANEX_OWNER_SCOPE_8N_V1_PRIVATE_CONFIG');

$config=$site.'/config.php';if(!is_file($config)||is_link($config))throw new RuntimeException('ANEX_OWNER_SCOPE_8N_V1_PROJECT_CONFIG');
require_once $config;
...
if(!is_file($collectorScript)||is_link($collectorScript))throw new RuntimeException('ANEX_OWNER_SCOPE_8N_V1_COLLECTOR');

$oldArgv=$argv;$oldArgc=$argc;
$argv=[
    $collectorScript,'--departure=1','--country=4',
    '--date-from=2026-09-18','--date-to=2026-09-24',
    '--nights=8','--adults=2','--meal=7',
    '--max-expands=120','--max-apd=600','--generation=25061863',
];
$argc=count($argv);
ob_start();
...
if(!is_array($collector)||($collector['status']??null)!=='complete')throw new RuntimeException('ANEX_OWNER_SCOPE_8N_V1_COLLECTOR_RECEIPT');
...
$result=[
    ...
    'scope'=>['departure_id'=>1,'country_id'=>4,'date_from'=>'2026-09-18','date_to'=>'2026-09-24','nights'=>8,'adults'=>2,'meal'=>'7'],
