#!/usr/bin/env python3
"""One authorized #3032 helper installation; no supplier or DB mutation.
...
BASE = 'bfc82d6080a06e73ebe771e4c89f7b50cf82d14f'
SOURCE = '27327df389e1b81b63f69df77b1a3bcab4ab51d3'
OP = 'search3-autosave-helper-3032-20260919-v1'
BRANCH = 'ops/' + OP
CLAIM = 5740657719
HELPER = 'app/integrations/tourvisor-anytour-offer-autosave.php'
OLD = '8e4c5a88a4c47ff7a4e5da41f28e8ed5257570f7'
NEW = '563642d1191377d34eba4c282f882084dc33fc71'
API_SHA = '0aee743972d3ac9eebeb5a54f899355375f4e5891369a2e873ceeb71097f68e2'
PATHS = ['.github/workflows/search3-autosave-helper-activate-v1.yml',
         'scripts/ops/search3_autosave_helper_activate_v1.py']
RUNTIME_RUNS = {35432737262: 'Tourvisor AnyTour offer autosave',
                35432737291: 'Tourvisor autosave persistent state',
                35432737282: 'Security guard'}
...
def authorize(wait_security):
    ...
    need(claim.get('user', {}).get('id') == 226193297 and claim.get('author_association') == 'OWNER'
         ...
         and 'explicitly AUTHORIZED' in claim.get('body', ''), 'fresh_authorization')
...
def replace_if(path, expected, replacement):
    ...
    fd, temporary = tempfile.mkstemp(prefix='.autosave-3032-', suffix='.php', dir=str(path.parent))
    try:
...
foreach($labels as $i=>$label){
 $tour=['id'=>'OFFLINE-3032-'.($i+1),'date'=>'2026-10-05','nights'=>7,'price'=>150824,'currency'=>'RUB','meal'=>['id'=>7,'name'=>'Все включено'],'roomType'=>'Standard Room','operator'=>['name'=>$label],'fuelCharge'=>0];
...
def execute():
    ...
    opts = ['-i', str(key), '-o', 'IdentitiesOnly=yes', '-o', 'BatchMode=yes',
            '-o', 'StrictHostKeyChecking=accept-new', '-o', 'UserKnownHostsFile=' + str(tmp / 'autosave3032.known_hosts'),
