...
const SBLC4_OP='hotel-match-samo-business-live30-common4-postwrite-1971-20260925-v24';
...
function sblc4_execute(PDO $db,string $root,int $cutTs):array{
    ...
    try{
        ...
        $dedup=[];$dedupConflicts=0;
        foreach($rows as $row){
            $local=(int)($row['local_hotel_id']??0);if($local<1)continue;
            $lanes=$row['operator_lanes']??[];$sig=sblc4_json($lanes);
            if(isset($dedup[$local])&&$dedup[$local]['lane_sig']!==$sig)$dedupConflicts++;
            if(!isset($dedup[$local]))$dedup[$local]=['lane_sig'=>$sig,'operator_lanes'=>$lanes,'acquisition_ready'=>false];
            if(($row['acquisition_ready']??false)===true)$dedup[$local]['acquisition_ready']=true;
        }
        $dedupDist=['0'=>0,'1'=>0,'2'=>0,'3'=>0,'4'=>0];$dedupMissing=array_fill_keys(array_keys(SBLC4_NS_OPS),0);$dedupReady=0;
        foreach($dedup as $d){
            $n=0;foreach(SBLC4_NS_OPS as $ns=>$op){$status=(string)($d['operator_lanes'][$ns]['status']??'missing');if($status==='missing')$dedupMissing[$ns]++;else$n++;}
            $dedupDist[(string)$n]++;if($d['acquisition_ready'])$dedupReady++;
        }
        return['operation'=>SBLC4_OP,'state'=>'samo_business_live30_common4_plan_ready','generated_at_utc'=>gmdate('c'),'cutoff_utc'=>gmdate('Y-m-d H:i:s',$cutTs),
            ...
            'common4_lanes_by_count_mapped'=>$dist,'missing_lane_counts'=>$missing,'saved_catalog_ready_count'=>$savedReady,'saved_catalog_not_ready_count'=>$savedNotReady,'acquisition_ready_count'=>$ready,'saved_catalog_file_count'=>$saved['file_count'],'deduped_unique_local_count'=>count($dedup),'deduped_common4_lanes_by_count_mapped'=>$dedupDist,'deduped_missing_lane_counts'=>$dedupMissing,'deduped_acquisition_ready_count'=>$dedupReady,'deduped_lane_state_conflicts'=>$dedupConflicts,'rows'=>$rows,
...
if(PHP_SAPI==='cli'&&realpath($_SERVER['SCRIPT_FILENAME']??'')===__FILE__){
    if(in_array('--self-test',$argv??[],true)){$x=sblc4_canonical_catalog([['andromeda_catalog','10'],['operator_115','b']],[ 'operator_115|b'=>[7=>true]],[7=>['10'=>true]]);sblc4_need(count($x['catalog'])===1&&isset($x['catalog']['10'])&&$x['operator_resolved']===1,'canonical');echo"MATCH_SAMO_BUSINESS_LIVE30_COMMON4_POSTWRITE_V24_SELFTEST_OK\n";exit;}
