...
OP=os.environ.get('MATCH_CHILD_OPERATION','hotel-match-live30-common4-acquire-1971-20260923-o0-n150-v1')
DAILY_LIMIT=3000
...
def execute(root,opdir,plan_path):
    ...
    if reservation.get('operation')!=OP or plan.get('state')!='live30_common4_ready' or plan.get('frontier_count')!=1799 or len(plan.get('rows',[]))!=1799:
        raise RuntimeError('input_guard')
    if plan.get('operator_ids')!=list(OPS):raise RuntimeError('operator_guard')
    offset=int(os.environ.get('MATCH_OFFSET','0'));limit=int(os.environ.get('MATCH_LIMIT','150'))
    if offset<0 or limit<1 or limit>150 or offset>=1799 or offset+limit>1799:raise RuntimeError('scope_guard')
    ...
    groups=chunks(scope)
    save(opdir/'tv-plan.json',{'operation':OP,'frontier_count':1799,'scope_offset':offset,'scope_count':len(scope),'groups':groups,'operator_ids':list(OPS),
    ...
    out={'operation':OP,'state':state,'reason':reason,'frontier_count':1799,'scope_offset':offset,'scope_count':len(scope),
    ...
    print(json.dumps({k:out[k] for k in ['state','reason','planned_groups','completed_batches','searched_hotels','incomplete_status_batches',
