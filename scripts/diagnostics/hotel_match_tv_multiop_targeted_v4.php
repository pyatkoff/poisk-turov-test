...
/** MATCH #1971 v4: pinned v2 runtime with longer bounded readiness + durable search checkpoint. */
...
$newOp='hotel-match-tv-multiop-targeted-1971-20260916-v4-egypt';
if(substr_count($src,$oldOp)!==1)throw new RuntimeException('v2_operation_marker_changed');
$src=str_replace($oldOp,$newOp,$src);
if(substr_count($src,'2026-11-15')<2)throw new RuntimeException('v2_date_marker_changed');
$src=str_replace('2026-11-15','2026-11-17',$src);
...
if(str_contains($src,$oldOp)||str_contains($src,'2026-11-15')||substr_count($src,'search_created_before_poll')!==1||substr_count($src,'foreach([8,12,15,15,15] as $wait)')!==1)throw new RuntimeException('v4_patch_incomplete');
if(in_array('--emit',$argv??[],true)){echo $src;exit(0);}
$tmp=sys_get_temp_dir().'/hotel-match-tv-multiop-targeted-v4-'.getmypid().'.php';
if(file_put_contents($tmp,$src,LOCK_EX)!==strlen($src))throw new RuntimeException('v4_temp_write_failed');
