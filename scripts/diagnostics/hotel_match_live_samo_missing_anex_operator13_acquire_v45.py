...
OP='hotel-match-live-samo-missing-anex-operator13-acquire-1971-20260925-v45'
...
def build_plan(router,consumed):
    ...
    if len(consumed)!=V41B_CONSUMED_BATCHES or not consumed.issubset(all_sigs):raise RuntimeError('consumed_not_router_subset')
    candidates=[b for b in all_candidates if batch_signature(b['context_key'],b['hotel_ids']) not in consumed]
    if len(candidates)!=221:raise RuntimeError('remaining_router_batch_count')
...
def execute(root,opdir,router_path,v41b_dir,source_sha):
    router=load(router_path);consumed=load_consumed(v41b_dir);plan=build_plan(router,consumed)
...
def selftest():
    ...
    print('MATCH_V45_SELFTEST_OK')

if __name__=='__main__':
    if '--self-test' in sys.argv:selftest();sys.exit(0)
    if '--execute' not in sys.argv:raise SystemExit('disabled')
    root=pathlib.Path(os.environ['ANYTOUR_ROOT']);opdir=pathlib.Path(os.environ['MATCH_OPERATION_DIR']);router=pathlib.Path(os.environ['MATCH_ROUTER_RESULT']);v41b=pathlib.Path(os.environ['MATCH_V41B_DIR']);source=os.environ['MATCH_SOURCE_SHA']
    if opdir.name!=OP or not root.is_dir() or not router.is_file() or not v41b.is_dir() or not re.fullmatch(r'[0-9a-f]{40}',source):raise SystemExit('runtime_scope')
    ...
    out=execute(root,opdir,router,v41b,source)
