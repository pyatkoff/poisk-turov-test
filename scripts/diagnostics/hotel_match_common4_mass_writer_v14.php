...
const HMC14_OP='hotel-match-common4-mass-writer-1971-20260924-v14';
...
function hmc14_manifest(array $audit,int $expected):array{
    hmc14_need(($audit['state']??'')==='completed_read_only_mass_current','audit_state');
    hmc14_need(($audit['operation']??'')==='hotel-match-common4-mass-current-1971-20260924-v13','audit_operation');
    hmc14_need($expected>=1&&$expected<=300,'expected_count');
    $out=[];$sources=[];$nt=[];
    ...
    hmc14_need(count($out)===$expected,'manifest_count');usort($out,fn($a,$b)=>strcmp($a['supplier_namespace'].'|'.$a['external_hotel_id'],$b['supplier_namespace'].'|'.$b['external_hotel_id']));return $out;
}
...
if(PHP_SAPI==='cli'&&realpath($_SERVER['SCRIPT_FILENAME']??'')===__FILE__){
    ...
    hmc14_need(($argv[1]??'')==='--execute','disabled');$root=(string)getenv('ANYTOUR_ROOT');$dir=(string)getenv('MATCH_OPERATION_DIR');$input=(string)getenv('MATCH_AUDIT_RESULT');$auditSha=(string)getenv('MATCH_AUDIT_SHA');$sourceSha=(string)getenv('MATCH_SOURCE_SHA');$expected=(int)getenv('MATCH_EXPECTED_WRITES');
    hmc14_need(is_dir($root)&&basename($root)==='anytoour.ru'&&is_dir($dir)&&is_file($input)&&preg_match('/^[0-9a-f]{64}$/D',$auditSha)===1&&hash_file('sha256',$input)===$auditSha&&preg_match('/^[0-9a-f]{40}$/D',$sourceSha)===1,'runtime_scope');$res=hmc14_load($dir.'/reservation.json');hmc14_need(($res['state']??'')==='reserved_before_write'&&(int)($res['expected_writes']??0)===$expected,'reservation');
    try{$manifest=hmc14_manifest(hmc14_load($input),$expected);require_once $root.(is_file($root.'/data/db-v1.php')?'/data/db-v1.php':'/v2/data/db-v1.php');$result=hmc14_write(v2_data_db(),$manifest,$sourceSha,$auditSha,$dir);$h=hmc14_save($dir.'/result.json',['operation'=>HMC14_OP]+$result);hmc14_save($dir.'/receipt.json',['operation'=>HMC14_OP,'state'=>$result['state'],'result_sha256'=>$h,'readback_verified'=>true,'provider_accessed'=>false,'provider_calls'=>0,'database_writes'=>$result['database_writes'],'mapping_writes'=>$result['mapping_writes'],'no_replay'=>true]);echo hmc14_json(['state'=>$result['state'],'inserted'=>$result['inserted'],'inserted_by_namespace'=>$result['inserted_by_namespace'],'unique_targets'=>$result['unique_targets']])."\n";}
