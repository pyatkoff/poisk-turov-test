#!/usr/bin/env python3
"""Pure authorization gate for the one-shot ANEX Search3 preview refresh."""
...
def main() -> int:
    ...
    try:
        ...
        print("ANEX_PREVIEW_CONTROL_DENIED invalid_event", file=sys.stderr)
        return 1
    allowed, reason = authorize(event, dict(os.environ))
    if not allowed:
        print("ANEX_PREVIEW_CONTROL_DENIED " + reason, file=sys.stderr)
        return 1
    print("ANEX_PREVIEW_CONTROL_AUTHORIZED")
    return 0
