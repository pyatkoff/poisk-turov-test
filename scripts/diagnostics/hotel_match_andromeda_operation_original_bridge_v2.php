...
const NEWOP='hotel-match-andromeda-operation-original-bridge-1971-20260916-v2';
function runtime_source():string{
  ...
  $needle=";}}}}\n\$db=null;";
  if(substr_count($s,$needle)!==1)throw new RuntimeException('brace_marker_count');
  $s=str_replace(OLD,NEWOP,$s);
  $s=str_replace($needle,";}}}\n\$db=null;",$s);
  return $s;
}
if(in_array('--self-test',$argv??[],true)){$s=runtime_source();if(str_contains($s,OLD)||substr_count($s,NEWOP)!==1||substr_count($s,";}}}\n\$db=null;")!==1)throw new RuntimeException('rewrite_test');echo"PASS\n";exit(0);}
