...
const SBLC4C4_EXPECTED_CAPTURED=507;
...
function sb4_source_edges(array $source,string $sourceResultSha):array{
    ...
    sb4_need(($source['state']??'')==='completed_supplier_free_v26_salvage','source_state');
    sb4_need((int)($source['target_edge_count']??0)===SBLC4C4_EXPECTED_EDGES,'source_edge_count');
    sb4_need((int)($source['pages_count']??0)===18&&(int)($source['pages_drained']??0)===18,'source_pages');
    sb4_need((int)($source['provider_http_reservations_observed']??0)===19,'source_reservations');
...
if(PHP_SAPI==='cli'&&realpath($_SERVER['SCRIPT_FILENAME']??'')===__FILE__){
    if(in_array('--self-test',$argv??[],true)){sb4_self_test();echo "MATCH_COMMON4_RETAINED_CONTEXT_CURRENT_V27_SELFTEST_OK\n";exit;}
