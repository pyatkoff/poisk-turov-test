...
function sb4_source_edges(array $source,string $sourceResultSha):array{
    ...
    sb4_need(in_array(($source['state']??''),['completed_read_only','completed_read_only_partial'],true),'source_state');
    sb4_need(($source['plan_operation']??'')===SBLC4C4_PLAN_OP,'source_plan');
    sb4_need((int)($source['catalog_target_count']??0)===1887&&(int)($source['acquisition_ready_count']??0)===1753,'source_scope');
    sb4_need((int)($source['queried_edge_count']??0)===SBLC4C4_EXPECTED_EDGES,'source_edge_count');
    sb4_need(($source['checkin_beg']??'')==='20260926'&&($source['checkin_end']??'')==='20261016'&&(int)($source['nights']??0)===7&&(int)($source['adults']??0)===2,'source_context');
...
if(PHP_SAPI==='cli'&&realpath($_SERVER['SCRIPT_FILENAME']??'')===__FILE__){
    if(in_array('--self-test',$argv??[],true)){sb4_self_test();echo "MATCH_SAMO_BUSINESS_LIVE30_COMMON4_CURRENT_V4_SELFTEST_OK\n";exit;}
