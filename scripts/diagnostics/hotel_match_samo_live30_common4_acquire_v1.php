...
const SLC4A_OP='hotel-match-samo-live30-common4-acquire-1971-20260924-v1';
const SLC4A_PLAN_OP='hotel-match-samo-live30-common4-plan-1971-20260924-v1';
const SLC4A_OPS=[5=>'operator_5',115=>'operator_115',315=>'operator_315',342=>'operator_342'];
const SLC4A_MONTHLY_LIMIT=5000000;
const SLC4A_HTTP_CAP=1500;
...
function slc4a_plan(array $plan):array{
    slc4a_need(($plan['operation']??'')===SLC4A_PLAN_OP&&($plan['state']??'')==='samo_live30_common4_plan_ready','plan_state');
    slc4a_need(($plan['catalog_live30_count']??null)===450&&($plan['acquisition_ready_count']??null)===434,'plan_counts');
    slc4a_need(($plan['saved_catalog_ready_count']??null)===445&&($plan['saved_catalog_not_ready_count']??null)===5,'plan_catalog_counts');
    slc4a_need(($plan['missing_lane_counts']??null)===['operator_5'=>252,'operator_115'=>426,'operator_315'=>199,'operator_342'=>227],'plan_lane_counts');
    foreach(['provider_http_calls','tourvisor_calls','samo_calls','anex_calls','database_writes','mapping_writes'] as $k)slc4a_need(($plan[$k]??null)===0,'plan_authority_'.$k);
    slc4a_need(($plan['safe_to_write_now']??null)===false&&is_array($plan['rows']??null)&&count($plan['rows'])===450,'plan_rows');
    $rows=[];
    ...
    slc4a_need(count($rows)===434,'plan_ready_rows');return $rows;
}
function slc4a_groups(array $rows):array{
    ...
    foreach($buckets as $key=>$set){
        [$state,$op]=array_map('intval',explode('|',$key,2));$ids=array_keys($set);sort($ids,SORT_NATURAL);
        for($i=0;$i<count($ids);$i+=30)$out[]=['stateinc'=>$state,'operator_id'=>$op,'hotel_ids'=>array_slice($ids,$i,30)];
    }
...
function slc4a_execute(string $root,string $dir,string $planPath,string $sourceSha):int{
    ...
    $from=(new DateTimeImmutable('tomorrow',new DateTimeZone('Europe/Moscow')))->format('Ymd');
    $to=(new DateTimeImmutable('tomorrow +20 days',new DateTimeZone('Europe/Moscow')))->format('Ymd');
    ...
    $out=['operation'=>SLC4A_OP,'state'=>$state,'reason'=>$reason,'source_sha'=>$sourceSha,'plan_operation'=>SLC4A_PLAN_OP,'plan_result_sha256'=>hash_file('sha256',$planPath),
        'catalog_target_count'=>450,'acquisition_ready_count'=>434,'queried_edge_count'=>count($edges),'planned_batch_count'=>count($groups),'completed_batch_count'=>count($batchOut),'batch_error_count'=>$batchErrors,
...
if(PHP_SAPI==='cli'&&realpath($_SERVER['SCRIPT_FILENAME']??'')===__FILE__){
    if(in_array('--self-test',$argv??[],true)){
        ...
        echo "MATCH_SAMO_LIVE30_COMMON4_ACQUIRE_V1_SELFTEST_OK\n";exit;
    }
