...
const OP='hotel-match-tourvisor-identity-backfill-1971-20260917-v4';
...
ON DUPLICATE KEY UPDATE
 first_seen_at=LEAST(tour_operator_identity_observations.first_seen_at,VALUES(first_seen_at)),
 last_seen_at=GREATEST(tour_operator_identity_observations.last_seen_at,VALUES(last_seen_at)),
 observation_count=GREATEST(tour_operator_identity_observations.observation_count,VALUES(observation_count)),
 country_id=VALUES(country_id),
 region_id=COALESCE(VALUES(region_id),tour_operator_identity_observations.region_id),
 subregion_id=COALESCE(VALUES(subregion_id),tour_operator_identity_observations.subregion_id),
 hotel_name=COALESCE(VALUES(hotel_name),tour_operator_identity_observations.hotel_name),
 region_name=COALESCE(VALUES(region_name),tour_operator_identity_observations.region_name),
 subregion_name=COALESCE(VALUES(subregion_name),tour_operator_identity_observations.subregion_name),
 latitude=COALESCE(VALUES(latitude),tour_operator_identity_observations.latitude),
 longitude=COALESCE(VALUES(longitude),tour_operator_identity_observations.longitude)
SQL;}
...
function selftest():void{req(str_contains(insertSql(),'tour_operator_identity_observations.first_seen_at'),'qualified_first');req(str_contains(insertSql(),'tour_operator_identity_observations.region_id'),'qualified_region');req(str_contains(insertSql(),"SHA2(CONCAT('tourvisor|'"),'fingerprint');req(str_contains(expectedSql(),'GROUP BY p.hotel_id,p.operator_id'),'expected');req(!preg_match('/https?:\/\//i',insertSql()),'network');echo "hotel_match_tourvisor_identity_backfill_v4 self-test: PASS\n";}
...
try{
 ...
 try{
  $expected=(int)$db->query(expectedSql())->fetchColumn();$o['expected_hotel_operator_pairs']=$expected;req($expected>0,'empty_expected');
  $affected=$db->exec(insertSql());
  $inside=(int)$db->query("SELECT COUNT(*) FROM tour_operator_identity_observations")->fetchColumn();req($inside>=$expected,'inside_rows_short');
  $insideDups=(int)$db->query("SELECT COUNT(*) FROM (SELECT hotel_id,operator_id FROM tour_operator_identity_observations GROUP BY hotel_id,operator_id HAVING COUNT(*)>1) x")->fetchColumn();req($insideDups===0,'inside_duplicates');
  $db->commit();$committed=true;$o['committed']=true;$o['database_identity_writes']=(int)$affected;
 ...
 $linkless=(int)$db->query("SELECT COUNT(*) FROM tour_operator_identity_observations WHERE operator_link IS NULL")->fetchColumn();$native=(int)$db->query("SELECT COUNT(*) FROM tour_operator_identity_observations WHERE native_id_value IS NOT NULL")->fetchColumn();$hotels=(int)$db->query("SELECT COUNT(DISTINCT hotel_id) FROM tour_operator_identity_observations")->fetchColumn();$operators=(int)$db->query("SELECT COUNT(DISTINCT operator_id) FROM tour_operator_identity_observations")->fetchColumn();
 $o['rows_after']=$after;$o['covered_pairs_readback']=$covered;$o['duplicate_pair_groups']=$dups;$o['linkless_rows']=$linkless;$o['native_enriched_rows']=$native;$o['distinct_hotels']=$hotels;$o['distinct_operators']=$operators;
 req($after===$o['expected_hotel_operator_pairs'],'rows_after_mismatch');req($covered===$o['expected_hotel_operator_pairs'],'coverage_mismatch');req($dups===0,'duplicates_after');$o['state']='completed';$o['read_at_utc']=gmdate('c');
...
finally{if($db instanceof PDO&&$locked){try{$db->query("SELECT RELEASE_LOCK('anytour_match_tv_identity_backfill_v4')");}catch(Throwable $ignore){}}}
