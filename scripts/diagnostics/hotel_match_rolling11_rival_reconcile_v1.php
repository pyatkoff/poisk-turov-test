...
const R11_OP='hotel-match-rolling11-rival-reconcile-1971-20260919-v1';
const R11_RELATED_SHA='f795bd5b9020cb05906a704f9ec2aa1efca14ea0c7c8df7b08fad09c50ea27c2';
const R11_FINAL_SHA='678ba78b19600d5c071683e9cfe0c6ebc08cbe480e28fa8e85a463d290ebd1ef';
const R11_PAIRS=[5580=>4054,7938=>2536,13495=>50869,13838=>69521,15132=>1143,19706=>61606,20720=>60156,22930=>63190,35319=>1346,35437=>1599,41303=>118906];
$library=__DIR__.'/guard-library.php';
if(hash_file('sha256',$library)!=='5e3875e9835a8de388317e0bd6ef41d76b56531596ed7fab99ea286b66a96878')throw new RuntimeException('guard_library_digest');
require_once $library;
function r11_load(string $path,string $sha):array{if(hash_file('sha256',$path)!==$sha)throw new RuntimeException('input_digest');return json_decode((string)file_get_contents($path),true,512,JSON_THROW_ON_ERROR);}
function r11_inputs(string $dir):array{
    $related=r11_load($dir.'/related.json',R11_RELATED_SHA);$final=r11_load($dir.'/plan.json',R11_FINAL_SHA);
    if($related['state']!=='completed_read_only'||$related['mapping_writes']!==0||count($related['dossiers'])!==16||$final['state']!=='planned_read_only'||$final['mapping_writes']!==0)throw new RuntimeException('input_state');
    $ps=r54_input($dir.'/audit.json');$out=[];
    foreach($ps as $p){$aid=$p['native_anex_id'];if(!isset(R11_PAIRS[$aid]))continue;if(R11_PAIRS[$aid]!==$p['hotel_id'])throw new RuntimeException('pair_manifest');
    ...
    if(count($out)!==11)throw new RuntimeException('exact_eleven');return $out;
}
function r11_rival_reason(array $target,array $current,array $expected):?string{
    ...
    // This cohort is eligible only through exact accepted-other identities.
    return null;
}
function r11_profiles(PDO $db,array $items):array{
...
function r11_review(array $item,array $snapshot,array $profiles):array{
    ...
    foreach($d['saved_candidates'] as $c){$other=(int)$c['anex_hotel_id'];
        ...
        $why=r11_rival_reason($target,$profiles[$other],$item['rivals'][$other]);
...
function r11_selftest(string $dir):void{
    r54_selftest($dir.'/audit.json');$items=r11_inputs($dir);$n=0;$ok=function(bool $x)use(&$n){$n++;if(!$x)throw new RuntimeException('reconcile_test_'.$n);};
    foreach($items as $item){
        $p=$item['anchor'];$s=array_fill_keys(['maps','decisions','exclusions','native','auto','observations','candidates','andromeda'],[]);$s['locals']=[$item['related']['target_local']];$s['candidates']=$item['held']['saved_candidates'];foreach(['native'=>'native_catalog','auto'=>'auto_review','observations'=>'saved_observation'] as $k=>$v)if($item['held'][$v]!==null)$s[$k]=[$item['held'][$v]];$profiles=$item['rivals'];
        $d=r11_review($item,$s,$profiles);$ok($d['eligible_for_guarded_append']);$ok(count($d['candidate_adjudications'])===count($s['candidates']));
        $x=$s;$x['maps']=[['anex_hotel_id'=>$p['native_anex_id'],'catalog_hotel_id'=>999]];$ok(!r11_review($item,$x,$profiles)['eligible_for_guarded_append']);
        $x=$s;$x['decisions']=[['anex_hotel_id'=>$p['native_anex_id'],'catalog_hotel_id'=>$p['hotel_id'],'decision_status'=>'rejected']];$ok(!r11_review($item,$x,$profiles)['eligible_for_guarded_append']);
        $x=$s;$x['exclusions']=[['anex_hotel_id'=>$p['native_anex_id'],'catalog_hotel_id'=>$p['hotel_id']]];$ok(!r11_review($item,$x,$profiles)['eligible_for_guarded_append']);
        $x=$s;$x['candidates'][0]['score']='0.000000';$ok(!r11_review($item,$x,$profiles)['eligible_for_guarded_append']);
        $y=$profiles;$k=array_key_first($y);$y[$k]['effective_accepted_local_id']=$p['hotel_id'];$ok(!r11_review($item,$s,$y)['eligible_for_guarded_append']);
        $y=$profiles;$y[$k]['native_catalog'][0]['source_fingerprint']='changed';$ok(!r11_review($item,$s,$y)['eligible_for_guarded_append']);
        $x=$s;$x['locals'][0]['country_id']=999;$ok(!r11_review($item,$x,$profiles)['eligible_for_guarded_append']);
        $x=$s;$x['candidates'][]=['anex_hotel_id'=>$p['native_anex_id'],'catalog_hotel_id'=>999];$ok(!r11_review($item,$x,$profiles)['eligible_for_guarded_append']);
        $x=$s;$x['maps'][]=['anex_hotel_id'=>999,'catalog_hotel_id'=>$p['hotel_id']];$ok(!r11_review($item,$x,$profiles)['eligible_for_guarded_append']);
        $y=$profiles;$y[$k]['effective_accepted_local_id']=null;$ok(!r11_review($item,$s,$y)['eligible_for_guarded_append']);
        $y=$profiles;$y[$k]['effective_local']['name']='CHANGED PROPERTY';$ok(!r11_review($item,$s,$y)['eligible_for_guarded_append']);
        $y=$profiles;$y[$k]['effective_local']['is_active']=0;$ok(!r11_review($item,$s,$y)['eligible_for_guarded_append']);
        $x=$s;$x['auto'][]=['anex_hotel_id'=>$p['native_anex_id'],'automated_reason'=>'protected_review'];$ok(!r11_review($item,$x,$profiles)['eligible_for_guarded_append']);
        $x=$s;$x['locals'][0]['is_active']=0;$ok(!r11_review($item,$x,$profiles)['eligible_for_guarded_append']);
        $x=$s;$x['observations']=[['anex_hotel_id'=>$p['native_anex_id'],'country_id'=>999,'hotel_name'=>$p['hotel_name'],'last_catalog_hotel_id'=>null]];$ok(!r11_review($item,$x,$profiles)['eligible_for_guarded_append']);
        $x=$s;$x['locals'][0]['latitude']=70;$ok(!r11_review($item,$x,$profiles)['eligible_for_guarded_append']);
    }
    echo 'ROLLING11_RECONCILE_SELFTEST_OK '.$n."\n";
}
if(($argv[1]??'')==='--self-test'){r11_selftest($argv[2]??__DIR__);exit;}
...
if(($res['operation']??'')!==R11_OP||($res['state']??'')!=='reserved_before_db_write'||($res['related_sha256']??'')!==R11_RELATED_SHA||($res['plan_sha256']??'')!==R11_FINAL_SHA||!preg_match('/^[0-9a-f]{40}$/D',$res['source_sha']??''))throw new RuntimeException('reservation');
$base=['operation'=>R11_OP,'source_sha'=>$res['source_sha'],'related_sha256'=>R11_RELATED_SHA,'plan_sha256'=>R11_FINAL_SHA,'audit_sha256'=>R54_AUDIT_SHA,'supplier_calls'=>0,'no_replay'=>true];
$db=null;$commitStarted=false;$committed=false;$written=[];$post=[];
try{
    r54_save($dir.'/execution-reservation.json',$res);$items=r11_inputs($dir.'/payload');
    ...
    $snapshot=r54_current($db,array_column($items,'anchor'),true);$profiles=r11_profiles($db,$items);$eligible=[];$held=[];
    foreach($items as $item){$d=r11_review($item,$snapshot,$profiles);if($d['eligible_for_guarded_append'])$eligible[]=$d;else $held[]=$d;}
...
$sha=r54_save($dir.'/result.json',$out);r54_save($dir.'/receipt.json',['operation'=>R11_OP,'state'=>$out['state'],'result_sha256'=>$sha,'readback_verified'=>hash_file('sha256',$dir.'/result.json')===$sha,'mapping_writes'=>$out['mapping_writes'],'no_replay'=>true]);echo r54_json(['state'=>$out['state'],'written_count'=>$out['written_count']??null,'held_count'=>$out['held_count']??null,'result_sha256'=>$sha]);exit($out['state']==='committed_verified'?0:2);
