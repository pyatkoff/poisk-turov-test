...
/* One IL Mercato URL observation. No database, booking or site writes. */
const ITV_OP='hotel-match-ilmercato-tv-links-1971-20260915-v2';
const ITV_PRIOR='caedc7d8efcef4fcf2691ff0f16a76278e41b1824382bc3d38a3f26b029c3b81';
...
function itv_params():array{return ['departureId'=>1,'countryId'=>1,'dateFrom'=>'2026-09-23','dateTo'=>'2026-09-23','nightsFrom'=>8,'nightsTo'=>8,'adults'=>2,'hotelIds'=>'2904','operatorIds'=>'43','currency'=>'RUB','onlyCharter'=>false,'onlyDirect'=>false];}
function itv_get(string $path,array $params,string $token,array &$out):array{
    if(count($out['calls'])>=8)throw new RuntimeException('call_budget');
    if(!preg_match('~^/(tours/search|tours/search/[1-9][0-9]*(/status)?|tours/[A-Za-z0-9_.-]+)$~D',$path)||str_contains($path,'13616631542'))throw new RuntimeException('endpoint_or_no_replay_guard');
    if($path==='/tours/search'&&($params!==itv_params()||$out['new_searches']!==1))throw new RuntimeException('exact_search_guard');
    $query=itv_query($params);$url='https://api.tourvisor.ru/search/api/v1'.$path.($query!==''?'?'.$query:'');
    $requestHash=hash('sha256',$path.'?'.$query);foreach($out['calls'] as $old)if($path==='/tours/search'&&($old['request_sha256']??'')===$requestHash)throw new RuntimeException('search_replay');
    ...
    foreach($headers as $k=>$v)if(str_contains($k,'remaining')&&ctype_digit($v)&&(int)$v<8-count($out['calls']))throw new RuntimeException('advertised_remaining_budget_insufficient');
...
function itv_run():array{
    if(PHP_SAPI!=='cli'||(getenv('MATCH_OPERATION_ID')?:'')!==ITV_OP||!preg_match('/^[0-9a-f]{40}$/D',getenv('MATCH_SOURCE_SHA')?:'')||(getenv('MATCH_PRIOR_RESULT_SHA')?:'')!==ITV_PRIOR)throw new RuntimeException('environment_guard');
    ...
    try{
        ...
        $phase='one_hotel_new_search';$out['new_searches']=1;$reply=itv_get('/tours/search',itv_params(),$token,$out);$sid=(string)($reply['searchId']??'');if(!preg_match('/^[1-9][0-9]*$/D',$sid))throw new RuntimeException('search_id_missing');$out['new_search_id']=$sid;
        ...
        $phase='single_hotel_result';$reply=itv_get('/tours/search/'.$sid,['limit'=>1],$token,$out);$out['returned_hotel_rows']=count($reply);$h=itv_target($reply);if(!$h)throw new RuntimeException('target_not_returned');
        ...
        if($host!=='intourist.ru'&&!str_ends_with($host,'.intourist.ru')){
            $tour=null;foreach($h['tours']??[] as $t)if(is_array($t)&&(int)($t['operator']['id']??0)===ITV_OPERATOR){$tour=$t;break;}
            $tid=(string)($tour['id']??'');if(!preg_match('/^[A-Za-z0-9_.-]{1,250}$/D',$tid))throw new RuntimeException('intourist_tour_missing');
            $phase='intourist_detail';$detail=itv_get('/tours/'.$tid,['currency'=>'RUB'],$token,$out);
            if((int)($detail['hotel']['id']??0)!==ITV_TARGET||(int)($detail['operator']['id']??0)!==ITV_OPERATOR||!itv_intourist($detail['operator']??[]))throw new RuntimeException('detail_identity_mismatch');
