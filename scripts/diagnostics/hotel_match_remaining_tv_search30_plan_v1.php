...
const OP='hotel-match-remaining-tv-search30-1971-20260921-v1';
...
if(($argv[1]??'')==='--self-test'){if(count(array_chunk(range(1,61),30))!==3)throw new RuntimeException('chunk');$x=attempted(['attempted_batches'=>[['hotel_ids'=>[1,2]]]]);if(array_keys($x)!==[1,2])throw new RuntimeException('attempted');echo "REMAINING_SEARCH30_PLAN_V1_SELFTEST_OK\n";exit;}
...
if(($r20['operation']??'')!=='hotel-match-reverse686-live-anex-batches-1971-20260920-v1'||($r21['operation']??'')!=='hotel-match-reverse110-live-anex-continuation-1971-20260921-v2'||($prior['operation']??'')!=='hotel-match-residual2041-search30-common4-1971-20260921-v3')throw new RuntimeException('input_operation');
$sent=attempted($r20)+attempted($r21)+attempted($prior);foreach(PROTECTED as$id)$sent[$id]=true;
