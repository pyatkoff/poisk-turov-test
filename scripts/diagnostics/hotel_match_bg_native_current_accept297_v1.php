...
const OP = 'hotel-match-bg-native-current-accept297-1971-20260922-v1';
const MIN_WRITE = 100;
const NS = 'bgoperator';
const INPUT_HASHES = [
...
function seeds(string $dir): array {
    ...
    foreach ($v['rows'] as $r) {
        ...
        need(count($in['accepted_catalog_ids'] ?? []) === 1 && count($in['canonical_evidence'] ?? []) === 1, 'single_canonical');
        need(count($in['official_evidence'] ?? []) === 1, 'single_official');
        $f4 = (string)$in['f4_candidates'][0]; $official = $in['official_evidence'][0]; $anchor = $in['canonical_evidence'][0];
        need(preg_match('/^[1-9][0-9]{5,19}$/D', $f4) === 1 && (string)($official['native_id'] ?? '') === $f4 && ($official['native_namespace'] ?? null) === NS, 'native_identity');
        need((string)$in['accepted_catalog_ids'][0] === (string)($anchor['external_hotel_id'] ?? ''), 'canonical_identity');
