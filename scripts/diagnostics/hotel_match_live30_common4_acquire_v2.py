...
OP=os.environ.get('MATCH_CHILD_OPERATION','hotel-match-live30-common4-continuation-acquire-1971-20260923-o0-n30-v1')
DAILY_LIMIT=3000
...
PLAN_RESULT_SHA='b66f8cfac90f0a83484a9d4a1d4cdd95bd2af1dfbdcc6b31aa7c97fd3cc244b9'
PLAN_TARGET_SHA='ce464a7b71dc72cf425197c73c1b8a4770adaf586ee05d169f4c67f8fc43ccca'

def continuation_scope(plan,plan_sha,offset,limit):
    if plan_sha!=PLAN_RESULT_SHA: raise RuntimeError('plan_hash_guard')
    if not isinstance(plan,dict) or plan.get('state')!='live30_common4_continuation_ready':
        raise RuntimeError('plan_state_guard')
    rows=plan.get('rows')
    frontier=plan.get('acquisition_target_count')
    if (frontier!=1349 or plan.get('never_attempted_current_count')!=1349
            or plan.get('attempted_hotel_count')!=450
            or plan.get('acquisition_target_id_sha256')!=PLAN_TARGET_SHA
            or plan.get('safe_to_write_now') is not False
            or plan.get('provider_http_calls')!=0 or plan.get('database_writes')!=0
            or plan.get('mapping_writes')!=0 or not isinstance(rows,list) or len(rows)!=frontier):
        raise RuntimeError('plan_contract_guard')
    ids=[];seen=set()
    for row in rows:
        if not isinstance(row,dict): raise RuntimeError('plan_row_shape')
        hid=num(row.get('tv_hotel_id'))
        if hid is None or hid in seen: raise RuntimeError('plan_row_id')
        seen.add(hid);ids.append(hid)
        missing=row.get('missing_operator_ids')
        if (not isinstance(missing,list) or not missing
                or len(set(missing))!=len(missing)
                or any(type(x) is not int or x not in OPS for x in missing)):
            raise RuntimeError('plan_missing_lanes')
    canonical=json.dumps(sorted(ids),ensure_ascii=False,separators=(',',':')).encode()
    if hashlib.sha256(canonical).hexdigest()!=PLAN_TARGET_SHA: raise RuntimeError('plan_target_digest')
    if offset<0 or limit<1 or limit>150 or offset>=frontier or offset+limit>frontier:
        raise RuntimeError('scope_guard')
    return frontier,rows[offset:offset+limit]
...
def execute(root,opdir,plan_path):
    ...
    if reservation.get('operation')!=OP: raise RuntimeError('reservation_operation')
    offset=int(os.environ.get('MATCH_OFFSET','0'));limit=int(os.environ.get('MATCH_LIMIT','30'))
    frontier,scope=continuation_scope(plan,plan_sha,offset,limit)
    ...
    groups=chunks(scope)
    save(opdir/'tv-plan.json',{'operation':OP,'continuation_plan_sha256':plan_sha,'continuation_target_set_sha256':PLAN_TARGET_SHA,
                              'frontier_count':frontier,'scope_offset':offset,'scope_count':len(scope),'scope_hotel_ids':scope_ids,
    ...
    out={'operation':OP,'state':state,'reason':reason,'continuation_plan_sha256':plan_sha,
         'continuation_target_set_sha256':PLAN_TARGET_SHA,'frontier_count':frontier,
         'scope_offset':offset,'scope_count':len(scope),'scope_hotel_ids':scope_ids,
    ...
    print(json.dumps({k:out[k] for k in ['state','reason','scope_offset','scope_count','searched_hotels','planned_groups','completed_batches',
        'returned_targets','returned_missing_operator_pairs','returned_operator_pairs','provider_calls','daily_accounted_after_local_ledger',
...
if __name__=='__main__':
    if '--self-test' in sys.argv:
        ...
        assert native_projection(13,'https://agent.anextour.ru/search/tour?HOTELLIST=5200')['positive_native_candidates']==[5200]
        assert native_projection(13,'https://evil.example/x?hotelCode=5844')['link_state']=='unexpected_anex_host'
        ...
        x=chunks([{'departure_id':1,'country_id':4,'departure_date':'2026-10-01','nights':7,'adults':2,'child_ages_signature':'',
                   'tv_hotel_id':i,'missing_operator_ids':[13,18,25,43]} for i in range(1,66)])
        assert [len(z['hotel_ids']) for z in x]==[30,30,5]
        print('MATCH_LIVE30_COMMON4_ACQUIRE_V1_SELFTEST_OK')
