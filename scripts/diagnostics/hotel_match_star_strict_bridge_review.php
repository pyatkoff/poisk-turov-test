...
function hmssb_review(PDO $db,string $operation=HMSSB_OPERATION): array {
    ...
    foreach($base['rows'] as $row){
        if(empty($row['strict_unique_name']))continue;
        $stats['strict_mismatch']++;
        $target=$row['target']??[];$local=(int)($target['local_hotel_id']??0);$anchors=$anexByLocal[$local]??[];
        $place=fc_place(array_map('strval',$row['source_places']??[]),[(string)($target['region']??''),(string)($target['subregion']??'')]);
        if($anchors)$stats['target_has_anex_anchor']++;if($place)$stats['direct_place_match']++;if($anchors&&$place)$stats['both_anchor_and_place']++;
    ...
    return ['status'=>'completed','operation_id'=>$operation,'database_writes'=>0,'supplier_calls'=>0,'historical_operations_replayed'=>false,'coverage'=>$base['coverage'],'stats'=>$stats,'rows'=>$out];
}
