...
const C4R26_OP='hotel-match-common4-retained-context-acquire-1971-20260925-v26b';
...
const C4R26_CONTEXTS=[
    ['context_id'=>2,'operator_id'=>315,'stateinc'=>5,'checkin_beg'=>'20261011','checkin_end'=>'20261011','nights'=>7,'meal'=>null],
    ['context_id'=>3,'operator_id'=>315,'stateinc'=>5,'checkin_beg'=>'20261016','checkin_end'=>'20261016','nights'=>7,'meal'=>null],
    ['context_id'=>4,'operator_id'=>5,'stateinc'=>3,'checkin_beg'=>'20261005','checkin_end'=>'20261005','nights'=>7,'meal'=>null],
    ['context_id'=>5,'operator_id'=>5,'stateinc'=>3,'checkin_beg'=>'20261006','checkin_end'=>'20261006','nights'=>7,'meal'=>null],
    ['context_id'=>6,'operator_id'=>5,'stateinc'=>5,'checkin_beg'=>'20261011','checkin_end'=>'20261011','nights'=>7,'meal'=>null],
    ['context_id'=>7,'operator_id'=>5,'stateinc'=>5,'checkin_beg'=>'20261012','checkin_end'=>'20261012','nights'=>7,'meal'=>'5,7'],
    ['context_id'=>8,'operator_id'=>5,'stateinc'=>5,'checkin_beg'=>'20261025','checkin_end'=>'20261025','nights'=>7,'meal'=>'5,7'],
];
...
function c4r26_file_key(array $spec,int $page):string{
    $id=(int)($spec['context_id']??0);c4r26_need($id>=2&&$id<=8,'context_id');
    $state=(int)$spec['stateinc'];$op=(int)$spec['operator_id'];$nights=(int)$spec['nights'];
    $beg=(string)$spec['checkin_beg'];$end=(string)$spec['checkin_end'];
    $meal=$spec['meal']===null?'none':substr(hash('sha256',(string)$spec['meal']),0,12);
    c4r26_need($page>=1&&$page<=C4R26_PAGE_CAP,'context_page');
    return 'context-'.$id.'-'.$state.'-'.$op.'-'.$beg.'-'.$end.'-'.$nights.'-'.$meal.'-page-'.$page;
}
function c4r26_execute(string $root,string $dir,array $post,string $postSha,string $sourceSha):array{
    ...
    foreach(C4R26_CONTEXTS as $ci=>$spec){
        ...
        $contextId=(int)$spec['context_id'];$ck=$contextId.'|'.$state.'|'.$op.'|'.$spec['checkin_beg'].'|'.$spec['checkin_end'].'|'.$nights.'|'.($spec['meal']??'');
        ...
        for($page=1;$page<=C4R26_PAGE_CAP;$page++){
            ...
            $fileKey=c4r26_file_key($spec,$page);c4r26_save($dir.'/'.$fileKey.'-reserved.json',['operation'=>C4R26_OP,'stateinc'=>$state,'operator_id'=>$op,'nights'=>$nights,'page'=>$page,'state'=>'reserved_before_price']);
            ...
            file_put_contents($evidence.'/'.$fileKey.'.json',$raw."\n",LOCK_EX);
...
function c4r26_self_test():void{
    c4r26_need(count(C4R26_CONTEXTS)===7,'context_count');
    $keys=[];foreach(C4R26_CONTEXTS as $spec){$k=c4r26_file_key($spec,1);c4r26_need(!isset($keys[$k]),'context_file_collision');$keys[$k]=true;}
...
if(PHP_SAPI==='cli'&&realpath($_SERVER['SCRIPT_FILENAME']??'')===__FILE__){
    if(in_array('--self-test',$argv??[],true)){c4r26_self_test();echo "MATCH_COMMON4_RETAINED_CONTEXT_ACQUIRE_V26B_SELFTEST_OK\n";exit;}
