...
const HMA_OP='hotel-match-user-seen-anex-details-mass-1971-20260918-v5';
const HMA_MAX_REQUESTS=1000;
...
function hma_tokens(mixed $v): array {
    $v=(string)$v;
    if($v===''||!preg_match('/\\p{L}/u',$v))return [];
    $drop=['hotel'=>1,'hotels'=>1,'otel'=>1,'отель'=>1,'гостиница'=>1,'resort'=>1,'resorts'=>1,'spa'=>1,'the'=>1,'and'=>1,'ex'=>1];
    $out=[];foreach(preg_split('/\\s+/u',hma_norm($v),-1,PREG_SPLIT_NO_EMPTY)?:[] as $t)if(!isset($drop[$t]))$out[(string)$t]=true;
...
if(in_array('--self-test',$argv??[],true)){if(hma_key('Porto Bello Hotel Resort & Spa')!=='bello porto')throw new RuntimeException('key');if(hma_score('Ozkaymak Falez Hotel','OZKAYMAK FALEZ HOTEL')<95)throw new RuntimeException('score');if(hma_tokens(123)!==[]||hma_tokens('123')!==[])throw new RuntimeException('numeric_only_guard');if(hma_tokens('7Pines Resort')===[])throw new RuntimeException('alphanumeric_name_guard');echo"ANEX_MASS_DETAILS_V5_SELFTEST_OK\n";exit;}
...
try{
  ...
  $details=[];$calls=0;$attempts=0;$stop=null;$home=(string)getenv('HOME');$rateDir=rtrim($home,'/').'/.anytour-anex-rate';
  if(is_link($rateDir))throw new RuntimeException('rate_dir_link');
  if(!is_dir($rateDir)&&!mkdir($rateDir,0700,true)&&!is_dir($rateDir))throw new RuntimeException('rate_dir_create');
  if(!chmod($rateDir,0700))throw new RuntimeException('rate_dir_mode');
  foreach($candidateIds as$i=>$aid){try{if($i%12===0)$client=new AnyTourAnexClient($token,null,$rateDir);$attempts++;$p=$client->request('Hotels_DETAILS',['HOTELINC'=>$aid]);$calls++;if($p)$details[$aid]=hma_detail($p);}catch(Throwable$e){$m=$e->getMessage();if($m==='ANEX_HTTP_ERROR'||$m==='ANEX_TRANSPORT_ERROR'||$m==='ANEX_TRANSPORT_UNAVAILABLE'){$stop=$m;break;}$details[$aid]=['error'=>$m];}}
