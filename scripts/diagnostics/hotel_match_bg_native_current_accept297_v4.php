...
// Owner-directed CURRENT acceptance of official Biblio-Globus identities.
// Raw F4 stays in its own bgoperator namespace; it is never converted to SAMO.
const OP = 'hotel-match-bg-native-current-accept297-1971-20260922-v4';
const MIN_WRITE = 100;
const NS = 'bgoperator';
const INPUT_HASHES = [
    'alias-output.json' => 'ce33022e4bbfa12e689b5312730260eb7d02db1835799ccb54954027cba9d061',
];
...
function seeds(string $dir): array {
    ...
    $v = $d['alias-output.json'];
    need(($v['schema'] ?? null) === 'match-bg-saved-alias-evidence/1', 'input_schema');
    need(($v['input_zip_sha256'] ?? null) === 'a52b4dbdf4a9eafdd49746a5a1aae461374f8d558e088096165ff82964e2b515', 'source_zip');
    need(($v['input_result_sha256'] ?? null) === '3917d36c7b665d1b44b84e7c009ced24321982367aa155e35554c5dd2f27515a', 'source_result');
    need(($v['counts']['hotels_examined'] ?? null) === 816 && ($v['counts']['alias_evidence_supported'] ?? null) === 297, 'input_counts');
    $out = []; $keys = []; $pairs = [];
    foreach ($v['rows'] as $r) {
        if (($r['alias_evidence_supported'] ?? false) !== true) continue;
        $in = $r['input_row'] ?? null; need(is_array($in), 'input_row');
        need(($r['safe_to_write_now'] ?? true) === false, 'source_not_write_authority');
        need(($in['safe_to_write_now'] ?? true) === false, 'retained_not_write_authority');
        need(($in['manual'] ?? null) === [], 'saved_manual_hold');
        need(count($in['f4_candidates'] ?? []) === 1, 'single_f4');
        need(count($in['accepted_catalog_ids'] ?? []) === 1 && is_array($in['canonical_evidence'] ?? null), 'single_selected_canonical');
        need(count($in['official_evidence'] ?? []) === 1, 'single_official');
        $f4 = (string)$in['f4_candidates'][0]; $official = $in['official_evidence'][0];
        $selectedId = (string)$in['accepted_catalog_ids'][0];
        $matchingAnchors = array_values(array_filter($in['canonical_evidence'], fn($x) => is_array($x) && (string)($x['external_hotel_id'] ?? '') === $selectedId));
        need(count($matchingAnchors) === 1, 'selected_canonical_evidence_not_unique'); $anchor = $matchingAnchors[0];
        need(preg_match('/^[1-9][0-9]{5,19}$/D', $f4) === 1 && (string)($official['native_id'] ?? '') === $f4 && ($official['native_namespace'] ?? null) === NS, 'native_identity');
        need($selectedId === (string)($anchor['external_hotel_id'] ?? ''), 'canonical_identity');
        need(($anchor['supplier_namespace'] ?? null) === 'andromeda_catalog' && ($anchor['decision_status'] ?? null) === 'accepted' && ($anchor['recorded_evidence_hash_matches'] ?? false) === true, 'canonical_anchor');
        need(preg_match('/^[0-9a-f]{64}$/D', (string)$anchor['catalog_sha256']) === 1 && preg_match('/^[0-9a-f]{64}$/D', (string)$anchor['evidence_sha256']) === 1, 'canonical_hash');
        ...
        $tv = (int)$r['tv_hotel_id']; need($tv > 0 && (int)($in['catalog_hotel']['id'] ?? 0) === $tv && (int)($anchor['local_hotel_id'] ?? 0) === $tv, 'target_identity');
        $seed = ['supplier_namespace'=>NS,'native_hotel_id'=>$f4,'tv_hotel_id'=>$tv,'expected_target'=>$in['catalog_hotel'],'expected_anchor'=>$anchor,
            'source'=>['operator_link'=>$link,'operator_link_sha256'=>$in['operator_link_sha256'],'official_evidence'=>$in['official_evidence'],'retained_geography'=>$in['retained_geography'],'proofs'=>$r['proofs'],'baseline_compound_evidence'=>$r['baseline_compound_evidence'],'new_alias_evidence_candidate'=>$r['new_alias_evidence_candidate']]];
        $key = keyOf($seed); $pair = pairOf($seed); need(!isset($keys[$key]) && !isset($pairs[$pair]), 'input_collision'); $keys[$key]=true; $pairs[$pair]=true; $out[]=$seed;
    }
    need(count($out) === 297 && count($keys) === 297 && count($pairs) === 297, 'cohort');
