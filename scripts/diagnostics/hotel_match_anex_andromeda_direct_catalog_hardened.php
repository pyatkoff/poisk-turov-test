...
function hmadcrh_supplier_rows(PDO $db): array {
    ...
    $rows=[];foreach($db->query("SELECT * FROM andromeda_hotel_identities WHERE supplier_namespace='andromeda_catalog' ORDER BY external_hotel_id")->fetchAll(PDO::FETCH_ASSOC) as $r){$id=(string)$r['external_hotel_id'];$obs=$latest[$id]??null;$country=(int)($obs['country_id']??0);if(!isset(HMADCR_CORE8[$country]))$country=(int)($shaCountry[$r['catalog_sha256']]??0);if(!isset(HMADCR_CORE8[$country]))continue;$s=hmgcr_andromeda_source($r,$obs);$names=hmadcr_expand_names($s['names']);if(!$names)continue;$rows[$id]=['external_id'=>$id,'country_id'=>$country,'names'=>$names,'places'=>$s['places'],'latitude'=>$s['latitude'],'longitude'=>$s['longitude'],'category'=>$s['category'],'live'=>(int)($obsCount[$id]??0)>0,'observation_count'=>(int)($obsCount[$id]??0),'local_id'=>($r['decision_status']==='accepted'&&$r['local_hotel_id']!==null)?(int)$r['local_hotel_id']:null,'decision_status'=>(string)$r['decision_status']];}
    return $rows;
}
