...
def build_report(results,status,reused_cases,transport=None):
...
def run(output,resume=None):
    output=Path(output);php=source();results=load_resume(resume);reused_cases=tuple(results)
    for case,value in results.items(): base.save(output/f'{case}.json',value)
    for case in CASES[len(results):]:
        try:
            value=validate_case(base.ssh_php_no_mux(php,dict(SPEC,case_id=case)),case)
        except Exception as exc:
            if type(exc).__name__!='SSHBatchError': raise
            failure=base.transport_failure(exc)
            base.save(output/'failure.json',failure)
            report=build_report(results,failure['status'],reused_cases,failure)
    ...
    report=build_report(results,status,reused_cases)
    base.save(output/'report.json',report);return report


def main():
    if len(sys.argv) not in (2,3): raise SystemExit('usage: anex_three_source_family_price.py OUTPUT_DIR [RESUME_DIR]')
    output=Path(sys.argv[1]);resume=Path(sys.argv[2]) if len(sys.argv)==3 else None
    try:
        report=run(output,resume);print(json.dumps(report,ensure_ascii=False,sort_keys=True));raise SystemExit(0 if report['status']=='completed' else 1)
