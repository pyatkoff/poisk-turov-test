...
def main():
    ...
    if isinstance(payload,dict):
        ...
        if err: result['error']=err if err in SAFE else 'other_error'
        data=payload.get('data')
        if isinstance(data,dict):
            if data.get('provider')=='anex': result['provider']='anex'
            if isinstance(data.get('hotels'),list): result['hotels']=len(data['hotels'])
            for key in ('received_offers','mapped_offers','pages_read'):
                if isinstance(data.get(key),int): result[key]=data[key]
