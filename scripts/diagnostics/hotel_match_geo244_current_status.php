...
try{
 ...
 $raw=defined('HM_GEO244_REPORT_B64')?base64_decode((string)HM_GEO244_REPORT_B64,true):null;
 if($raw===false)throw new RuntimeException('report_capsule_decode');
 if($raw===null){$reportPath=$root.'/reports/hotel-match-geo-context-review-20260912.json';if(!is_file($reportPath))throw new RuntimeException('report_missing');$raw=file_get_contents($reportPath);}
 ...
 $mapLocal=pick($mapCols,['catalog_hotel_id','local_hotel_id']);$exLocal=pick($exCols,['catalog_hotel_id','local_hotel_id']);
