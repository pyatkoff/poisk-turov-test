...
const HMTV_OP='hotel-match-user-seen-tv-detail-saturation-1971-20260918-v2';
const HMTV_MAX_ATTEMPTS=270;
...
function hmtv_http_once(string $token,string $tourId): array {
    ...
    try{
        curl_setopt_array($ch,[
            ...
            CURLOPT_PROXY=>'',CURLOPT_NOPROXY=>'*',CURLOPT_USERAGENT=>'AnyTour-MATCH-TV-detail-saturation/2.0',
...
function hmtv_quota_attempt(string $home): int {
    ...
    try{
        ...
        if($new>=HMTV_MAX_ATTEMPTS)throw new RuntimeException('operation_attempt_cap');
        $new++;$j=['day'=>$day,'match_new_attempts'=>$new,'known_prior_attempt_floor'=>25,'owner_daily_limit'=>300,'operation_cap'=>HMTV_MAX_ATTEMPTS,'updated_at'=>gmdate('c')];
...
if(in_array('--self-test',$argv??[],true)){
    ...
    echo"TV_DETAIL_SATURATION_SELFTEST_OK\n";exit;
}
...
try{
    ...
    $schedule=[];$round=0;
    while(count($schedule)<HMTV_MAX_ATTEMPTS){
        $added=0;
        foreach($hotelIds as$lid){
            if(isset($byHotel[$lid][$round])){$schedule[]=$byHotel[$lid][$round];$added++;if(count($schedule)>=HMTV_MAX_ATTEMPTS)break;}
        }
    ...
    echo json_encode([
        ...
        'scheduled_attempts'=>count($schedule),'http_attempts'=>$attempted,'http_200'=>$http200,'http_404'=>$notFound,'transport_or_shape_failures'=>$transport,
