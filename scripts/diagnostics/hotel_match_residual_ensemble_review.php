...
function hmre_review(PDO $db,array $base,array $strict,array $coord,array $sold,string $operation=HMRE_OPERATION): array {
    ...
    try{
        ...
        $mapped=[];$claimed=[];foreach($db->query('SELECT anex_hotel_id,catalog_hotel_id FROM anex_hotel_search_mappings WHERE enabled=1')->fetchAll(PDO::FETCH_ASSOC) as $r){$aid=(int)$r['anex_hotel_id'];$hid=(int)$r['catalog_hotel_id'];$mapped[$aid]=true;$claimed[$hid][$aid]=true;}
        ...
        foreach(['auto_accept','needs_extra_evidence','hard_conflict','manual_last'] as $bucket){
            foreach(($base['buckets'][$bucket]??[]) as $baseRow){
                ...
                if($provider==='anex'){
                    $aid=(int)$external;foreach(array_merge($coordIndex[$aid]??[],$soldIndex[$aid]??[]) as $ev){$target=(int)$ev['target_local_hotel_id'];if(!isset($active[$target])){$dropped['inactive_target']++;continue;}if((int)$active[$target]['country_id']!==$country){$dropped['country_target_mismatch']++;continue;}if(isset($excluded[$aid][$target])){$dropped['pair_excluded']++;continue;}$other=array_filter(array_keys($claimed[$target]??[]),static fn($x)=>(int)$x!==$aid);if($other){$dropped['target_claimed']++;continue;}$evidence[]=$ev;}
                }
                $filtered=[];foreach($evidence as $ev){$target=(int)$ev['target_local_hotel_id'];if(!isset($active[$target])||(int)$active[$target]['country_id']!==$country){$dropped['stale_overlay']++;continue;}$filtered[]=$ev;}$evidence=$filtered;
        ...
        $db->commit();
        return ['status'=>'completed','operation_id'=>$operation,'mode'=>'mass_current_residual_ensemble_read_only','database_writes'=>0,'mapping_writes'=>0,'supplier_calls'=>0,'historical_operations_replayed'=>false,'coverage'=>$coverage,'counts'=>$counts,'dropped'=>$dropped,'reason_counts'=>$reasonCounts,'country_counts'=>$countryCounts,'auto_evidence_sources'=>$sourceCounts,'buckets'=>$buckets,'guards'=>['core8_only'=>true,'manual_decisions_overwritten'=>false,'pair_exclusions_overwritten'=>false,'existing_mappings_overwritten'=>false,'coordinate_conflict_auto_block_m'=>5000,'coordinate_conflict_override_requires_direct_details_and_same_sold_date'=>true,'generic_hotel_resort_spa_not_identity'=>true,'critical_qualifiers_preserved'=>['ANNEX','BEACH','GARDEN','NORTH','SOUTH'],'same_provider_target_collision_blocks_auto'=>true,'database_writes'=>0,'supplier_calls'=>0]];
