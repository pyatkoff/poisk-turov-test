...
const SLC4A_OP='hotel-match-samo-business-live30-common4-wave5-recovery-acquire-1971-20260925-v15';
const SLC4A_GROUP_START=401;
const SLC4A_GROUP_END=502;
...
const SLC4A_CHECKIN_BEG='20260927';
const SLC4A_CHECKIN_END='20261017';
...
function slc4a_bridge(array $row,int $operator,array $requested):array{
    ...
    $origHotel=slc4a_pos($original['hotelKey']??null);
    if($hotel!==null&&isset($requested[$hotel])&&$flag==='0'){
        if($origHotel!==null)return ['state'=>'exact_catalog_to_native','catalog_id'=>$hotel,'native_id'=>$origHotel];
...
function slc4a_execute(string $root,string $dir,string $planPath,string $sourceSha):int{
    ...
    $allGroups=slc4a_groups($rows);slc4a_need(count($allGroups)===502,'global_group_count');
    slc4a_need(SLC4A_GROUP_START>=1&&SLC4A_GROUP_END>=SLC4A_GROUP_START&&SLC4A_GROUP_END<=count($allGroups),'wave_bounds');
    $groups=array_slice($allGroups,SLC4A_GROUP_START-1,SLC4A_GROUP_END-SLC4A_GROUP_START+1);
    slc4a_need(count($groups)===(SLC4A_GROUP_END-SLC4A_GROUP_START+1),'wave_group_count');
    ...
    try{
        ...
        $edgeMap=[];
        foreach($groups as $g)foreach($g['hotel_ids'] as $cid){
            $key=$cid.'|'.$g['operator_id'];
            $edgeMap[$key]=['catalog_id'=>$cid,'stateinc'=>$g['stateinc'],'operator_id'=>$g['operator_id'],'namespace'=>SLC4A_OPS[$g['operator_id']],'returned_catalog'=>false,'native_ids'=>[],'price_rows'=>0,'partial_hold'=>false,'safe_to_write_now'=>false];
        }
        foreach($groups as $bi=>$g){
            $batch=SLC4A_GROUP_START+$bi;$requested=array_fill_keys($g['hotel_ids'],true);$pages=0;$pagesCount=null;$rowsSeen=0;$unboundNative=[];$hashes=[];$batchState='complete';$err=null;
            ...
            if($batchState!=='complete'){
                foreach($g['hotel_ids'] as $cid){
                    $key=$cid.'|'.$g['operator_id'];
                    if(isset($edgeMap[$key])){$edgeMap[$key]['partial_hold']=true;$edgeMap[$key]['native_ids']=[];$edgeMap[$key]['returned_catalog']=false;}
                }
            }
        ...
        foreach($edgeMap as $e){
            $ids=array_keys($e['native_ids']);sort($ids,SORT_NATURAL);unset($e['native_ids']);$partial=(bool)($e['partial_hold']??false);unset($e['partial_hold']);$e['positive_native_candidates']=$ids;
            if($partial)$e['state']='partial_unresolved_no_replay';
            elseif(count($ids)===1)$e['state']='captured_single_native';
    ...
    $out=['operation'=>SLC4A_OP,'state'=>$state,'reason'=>$reason,'source_sha'=>$sourceSha,'plan_operation'=>SLC4A_PLAN_OP,'plan_result_sha256'=>hash_file('sha256',$planPath),
        'catalog_target_count'=>1887,'acquisition_ready_count'=>1753,'global_group_count'=>count($allGroups),'group_start'=>SLC4A_GROUP_START,'group_end'=>SLC4A_GROUP_END,'queried_edge_count'=>count($edges),'planned_batch_count'=>count($groups),'completed_batch_count'=>count($batchOut),'batch_error_count'=>$batchErrors,
...
if(PHP_SAPI==='cli'&&realpath($_SERVER['SCRIPT_FILENAME']??'')===__FILE__){
    if(in_array('--self-test',$argv??[],true)){
        ...
        $x=slc4a_bridge(['operatorKey'=>315,'hotelKey'=>'2001','isOperatorHotelKey'=>0,'original'=>['hotel'=>'Example','hotelKey'=>'8123','tourKey'=>44]],315,$req);
        ...
        echo "MATCH_SAMO_BUSINESS_LIVE30_COMMON4_ACQUIRE_V3_WAVE5_SELFTEST_OK\n";exit;
    }
