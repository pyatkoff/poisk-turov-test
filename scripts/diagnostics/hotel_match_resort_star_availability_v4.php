...
const OP='hotel-match-resort-star-adaptive-1971-20260911-v4';
...
$pdo=v2_data_db();
$wanted=['Antalya','Alanya','Kemer','Belek','Side','Bodrum','Marmaris'];
...
foreach($wanted as $w){
  $hits=[];
  foreach($all as $r){
    if(mb_stripos((string)$r['name'],$w,0,'UTF-8')!==false)$hits[]=$r;
  }
  $regions[]=(count($hits)===1)
    ? ['wanted'=>$w,'id'=>(int)$hits[0]['id'],'name'=>(string)$hits[0]['name']]
    : ['wanted'=>$w,'id'=>null,'name'=>null,'catalog_hits'=>count($hits)];
}
...
echo 'MATCH_RS_JSON:'.json_encode([
  'status'=>'completed','operation_id'=>OP,'country'=>'Turkey','searches_started'=>$searches,
