...
const HMC_OP = 'hotel-match-tv-samo-common4-side-star4-samo-1971-20260922-v5';
const HMC_PREVIOUS_OP = 'hotel-match-tv-samo-common4-side-star4-1971-20260922-v4b';
...
function hmc3_previous_checkpoint(string $prevDir): array {
    ...
    if(($result['operation']??null)!==HMC_PREVIOUS_OP||($result['state']??null)!=='terminal_failed_no_replay'||($result['reason']??null)!=='tv_call_budget')throw new RuntimeException('previous_result');
    ...
    foreach($metas as $metaPath){
        ...
        if($path==='/tours/search'&&(int)($params['hotelCategory']??0)===4){
    ...
    if(!$sid||$continues!==32||$lastRequests===null||$lastRequests<=0||!$union)throw new RuntimeException('previous_checkpoint_incomplete');
...
function hmc_execute(string $root,string $opDir,string $user,string $pass): array {
    ...
    $plan=[
        ...
        'scope_provenance'=>HMC_PREVIOUS_OP,'acquisition_scope'=>'retained_partial_tv4_plus_fresh_samo4','tourvisor_new_calls_planned'=>0,
    ];
    ...
    foreach([4] as $star){
        ...
        $td=['search_id'=>$checkpoint['search_id'],'rows'=>$checkpoint['rows'],'rounds'=>[],'fully_drained'=>false,'exhaustion'=>'retained_partial_tv_call_budget'];
    ...
    return [
        ...
        'scope_provenance'=>HMC_PREVIOUS_OP,'acquisition_scope'=>'retained_partial_tv4_plus_fresh_samo4',
        'retained_tv_search_id'=>$checkpoint['search_id'],'retained_tv_continue_calls'=>$checkpoint['prior_continue_calls'],'retained_tv_last_request_count'=>$checkpoint['last_request_count'],
...
if(in_array('--self-test',$argv??[],true)){
    ...
    echo "MATCH_TV_SAMO_COMMON4_SIDE_STAR4_SAMO_V5_SELFTEST_OK\n";exit(0);
}
