#!/usr/bin/env python3
"""Bounded MATCH identity acquisition; never drains a tour search or writes mappings."""
import collections
...
OP = 'hotel-match-anex-rolling30-1971-20260919-v1'
DAY = '2026-09-19'
LIMIT = 3000
CALL_CAP = 180
MAX_BATCHES = 6
...
def save(path, obj):
    """Immutable and fsynced. Never replace a prior receipt or evidence body."""
    raw = encode(obj)
    with open(path, 'xb') as f:
        if f.write(raw) != len(raw):
            raise RuntimeError('short_write')
        f.flush(); os.fsync(f.fileno())
    fd = os.open(str(pathlib.Path(path).parent), os.O_DIRECTORY)
    try: os.fsync(fd)
    finally: os.close(fd)
...
def read(f):
    ...
    if not isinstance(obj, dict):
        raise RuntimeError('ledger_shape')
    return obj
...
def numeric(v):
    return int(v) if re.fullmatch(r'[1-9][0-9]{0,21}', str(v)) else None
...
def batches_next(pool, attempts, found, carry):
    ...
    if not new: return [], []  # Do not restart a residual-only search.
    ids = (pending[:29] + new)[:30]
    return ids, [i for i in ids if attempts[i] > 0]


def strict_link(detail):
    """Only the actual ANEX operatorLink. Never discard signed/unknown tokens."""
    u = detail.get('operatorLink')
    if not isinstance(u, str) or len(u) > 4096:
        return {'reason': 'operator_link_missing'}
    try:
        p = urllib.parse.urlsplit(u)
        port = p.port
    except ValueError:
        return {'reason': 'operator_link_malformed'}
    ...
    if len(vals) != 1 or len(tokens) != 1 or not re.fullmatch(r'[1-9][0-9]{0,7}', tokens[0]):
        ...
        out['native_anex_id'] = int(tokens[0])
    return out
...
class Provider:
    def __init__(self, root, opdir):
        self.home = pathlib.Path(os.environ['HOME']); self.dir = opdir
        self.used = 0; self.last_accounted = None
        ...
        if dt.datetime.now(ZoneInfo('Europe/Moscow')).date().isoformat() != DAY:
            raise RuntimeError('provider_day_mismatch')
        # Existing shared ledger is mandatory. Never initialize a fresh budget.
        with open(self.dayfile, 'r+b') as f:
            ...
            if accounted < 396 or accounted >= min(LIMIT, int(d['owner_daily_limit'])):
                raise RuntimeError('daily_budget_guard')
    ...
    def charge(self, action, path, params):
        ...
        with open(self.dayfile, 'r+b') as df, open(self.opfile, 'r+b') as of:
            ...
            d, o = read(df), read(of)
            if o['operation'] != OP or o['status'] not in ('reserved_before_provider_access', 'provider_accessed'):
                raise RuntimeError('terminal_operation')
            ...
            if charged >= min(LIMIT, int(d['owner_daily_limit'])) or used >= CALL_CAP:
                raise RuntimeError('quota_exhausted')
            d['match_new_attempts'] = match + 1; d['accounted_requests'] = charged + 1
            d.setdefault('operations', {})[OP] = used + 1
            o.update(status='provider_accessed', used=used + 1, last_action=action)
            write(df, d); write(of, o)
            self.used = used + 1; self.last_accounted = charged + 1
    ...
    def call(self, action, path, params=None):
        params = params or {}
        allowed = action in ('search_start', 'search_status', 'search_results', 'tour_detail')
        if not allowed or '/continue' in path or '/dates' in path:
            raise RuntimeError('forbidden_provider_action')
        self.charge(action, path, params)
        # Keep the proven upstream native array encoding; no public API change.
        ...
        try:
            response = self.opener.open(req, timeout=65)
        except urllib.error.HTTPError as exc:
            response = exc
        with response:
            code = response.code; raw = response.read(BODY_LIMIT + 1)
        ...
        if not safe_body(data, self.token):
            save(self.dir / ('response-%03d.json' % self.used), {'http_status': code, 'withheld_sensitive_body_sha256': hashlib.sha256(raw).hexdigest()})
        ...
        if code == 429 or code in (401, 403): raise RuntimeError('http_' + str(code))
    ...
    def finish(self, status, result_sha):
        with open(self.opfile, 'r+b') as f:
            fcntl.flock(f, fcntl.LOCK_EX); o = read(f)
            o.update(status=status, result_sha256=result_sha); write(f, o)
...
def run_batch(provider, opdir, number, ctx, ids):
    ...
    for delay in (5, 8, 12, 15):
        time.sleep(delay)
        _, status, _ = provider.call('search_status', '/tours/search/%d/status' % sid, {'operatorStatus': False})
    ...
    for h in rows:
        ...
        # Any returned hotel leaves search queue, even if its detail needs review.
        if not usable:
            ev['reason'] = 'no_usable_tour'
        else:
            ...
            elif identity(detail, 'hotel') != hid or identity(detail, 'operator') != 13 or str(detail.get('id') or detail.get('tourId')) != tid:
                ev['reason'] = 'detail_identity_mismatch'
...
def execute(opdir, root):
    ...
    for name, digest in manifest.items():
        if pathlib.PurePath(name).name != name or hashlib.sha256((opdir / 'payload' / name).read_bytes()).hexdigest() != digest:
            raise RuntimeError('source_digest')
    # Failure here occurs before opening any provider credential/budget.
    cp = subprocess.run(['php', str(opdir / 'payload/hotel_match_anex_rolling30_current_v1.php'), '--execute'], capture_output=True, check=False)
    ...
    attempts = collections.Counter(); found = {}; batches = []; provider = None
    try:
        ...
        for ctx in plan['contexts']:
            ...
            for h in ctx['hotels']:
                if h['operator_id'] != 13 or h['source'] != 'user_search' or h['country_id'] != ctx['country_id'] or h['departure_id'] != ctx['departure_id'] or not (ctx['date_from'] <= h['departure_date'] <= ctx['date_to']):
                    raise RuntimeError('plan_observation_binding')
            carry = []
    ...
    # Reconstruct from durable per-hotel captures even after a later failure.
    evidence = [json.loads(p.read_bytes()) for p in sorted(opdir.glob('hotel-*.json'))]
    outcome = {'operation': OP, 'state': state, 'reason': reason, 'batches': batches, 'frontier_before': plan['frontier_unique'], 'current_plan_sha256': plan_sha, 'found_hotels': len(evidence), 'anchors': [e for e in evidence if 'native_anex_id' in e and 'reason' not in e], 'holds': [e for e in evidence if 'reason' in e], 'not_found_ids': sorted(set(attempts) - {e['hotel_id'] for e in evidence}), 'attempts': dict(attempts), 'provider_calls': provider.used if provider else 0, 'call_counts': dict(provider.counts) if provider else {}, 'daily_accounted_after': provider.last_accounted if provider else None, 'continue_calls': 0, 'database_writes': 0, 'mapping_writes': 0, 'room_correspondences': 0, 'no_replay': True}
...
def selftest():
    attempts = collections.Counter(); found = {}; pool = list(range(1, 81))
    first, carry = batches_next(pool, attempts, found, [])
    assert first == list(range(1, 31)) and carry == []
    attempts.update(first); found.update({i: {} for i in range(1, 21)})
    second, carry = batches_next(pool, attempts, found, list(range(21, 31)))
    assert second == list(range(21, 51)) and carry == list(range(21, 31))
    attempts.update(second); found.update({i: {} for i in range(24, 51)})
    third, carry = batches_next(pool, attempts, found, [21, 22, 23])
    assert third == list(range(51, 81)) and carry == []
    assert batches_next([1], collections.Counter({1: 1}), {}, [1]) == ([], [])
    # 2,000 IDs retained; each filled batch is finite, closed IDs never repeated.
    attempts = collections.Counter(); found = {}
    while True:
        ids, _ = batches_next(list(range(1, 2001)), attempts, found, [])
        if not ids: break
        assert len(ids) <= 30; attempts.update(ids); found.update({i: {} for i in ids})
    assert len(found) == 2000 and max(attempts.values()) == 1
    ...
    assert not safe_body({'operatorLink': link + '1&token=x'}, 'NOT_IN_BODY')
    assert identity({'hotel': {'id': 1}}, 'hotel') == 1
    assert identity({}, 'operator') is None
    assert result_rows({'hotels': []}) == []
    assert ready({'progress': 100}) and not ready({'progress': 90})
    import tempfile
    ...
    class Fake:
        ...
        def call(self, action, path, params=None):
            self.calls.append((action, path, params))
            if action == 'search_start':
                assert params['hotelIds'] == [1, 2, 3] and params['operatorIds'] == [13]
                return 200, {'searchId': 123}, 'start'
            if action == 'search_status': return 200, {'progress': 100}, 'status'
            if action == 'search_results': return 200, [
                {'id': i, 'name': 'Hotel '+str(i), 'country': {'id': 4}, 'tours': [{'id': 100+i, 'operator': {'id': 13}, 'roomType': 'FAMILY SEA VIEW ROOM'}]}
                for i in [1, 2]], 'result'
            if action == 'tour_detail':
                i = int(path.split('/')[-1])-100
                return 200, {'id': 100+i, 'hotel': {'id': i}, 'operator': {'id': 13}, 'operatorLink': link+str(500+i)+((', -1025133') if i == 2 else '')}, 'detail'
            raise AssertionError('unnecessary_provider_call')
    with tempfile.TemporaryDirectory() as tmp, patch('time.sleep'):
        fake = Fake()
        seen, sid = run_batch(fake, pathlib.Path(tmp), 1, {'departure_id':1, 'country_id':4, 'date_from':'2026-10-01', 'date_to':'2026-10-07'}, [1, 2, 3])
        assert sid == 123 and set(seen) == {1, 2} and len(fake.calls) == 5
        assert seen[1]['native_anex_id'] == 501 and seen[2]['reason'] == 'ambiguous_or_missing_hotellist'
        assert seen[1]['raw_room_name'] == 'FAMILY SEA VIEW ROOM'
        assert len(list(pathlib.Path(tmp).glob('hotel-*.json'))) == 2
        assert not any('/continue' in x[1] for x in fake.calls)
    # Shared ledger cap and terminal state checked offline, with no HTTP/token.
    with tempfile.TemporaryDirectory() as tmp:
        fake = object.__new__(Provider); fake.dir=pathlib.Path(tmp); fake.used=0; fake.counts=collections.Counter()
        fake.dayfile=fake.dir/'daily.json'; fake.opfile=fake.dir/'op.json'
        save(fake.dayfile, {'accounted_requests':2999,'owner_daily_limit':3000,'known_prior_attempt_floor':184,'match_new_attempts':2815})
        save(fake.opfile, {'operation':OP,'status':'reserved_before_provider_access','used':0})
        fake.charge('search_start','/tours/search',{})
        assert fake.used==1 and fake.last_accounted==3000
        try: fake.charge('tour_detail','/tours/101',{})
        except RuntimeError as e: assert str(e)=='quota_exhausted'
        else: raise AssertionError('daily_overspend')
        assert json.loads(fake.dayfile.read_bytes())['accounted_requests']==3000
        fake.finish('completed_read_only','abc')
        try: fake.charge('tour_detail','/tours/101',{})
        except RuntimeError as e: assert str(e)=='terminal_operation'
        else: raise AssertionError('operation_replay')
    print('ROLLING30_SELFTEST_OK: refill30, retry_limit2, closed_dedupe, 2000 coverage, exact_batch_calls, signed-token holds, durable no-replay, shared_daily_cap')


if __name__ == '__main__':
    if '--self-test' in sys.argv: selftest()
    elif '--execute' in sys.argv:
        sys.exit(execute(pathlib.Path(os.environ['MATCH_OPERATION_DIR']), pathlib.Path(os.environ['ANYTOUR_ROOT'])))
