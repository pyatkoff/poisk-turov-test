...
try {
 ...
 $hotels=$pdo->query('SELECT id,name,country_id,region_name,category FROM catalog_hotels WHERE country_id=1 AND is_active=1 ORDER BY id LIMIT 10001')->fetchAll(PDO::FETCH_ASSOC);
 ...
 $identities=$pdo->query('SELECT supplier_namespace,external_hotel_id,local_hotel_id,decision_status,catalog_sha256,evidence_sha256,evidence_json FROM andromeda_hotel_identities ORDER BY supplier_namespace,external_hotel_id LIMIT 10001')->fetchAll(PDO::FETCH_ASSOC);
 ...
 foreach($files as $file){
  ...
  if(!in_array($state['status']??null,['complete','partial'],true)||($state['criteria']['STATEINC']??null)!==3)continue;
  ++$pages;
