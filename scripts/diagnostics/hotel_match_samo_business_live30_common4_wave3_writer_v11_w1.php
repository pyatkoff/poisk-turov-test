...
function sbw5_manifest(array $audit,int $offset,int $expected):array{
    ...
    sbw5_need($offset===0&&$expected===300&&$total===325,'wave1_exact_slice');
    $all=[];$sources=[];$targets=[];
...
function sbw5_self_test():void{
    ...
    $m=sbw5_manifest($audit,0,300);sbw5_need($m['total']===325&&count($m['rows'])===300,'self_manifest');
}
if(PHP_SAPI==='cli'&&realpath($_SERVER['SCRIPT_FILENAME']??'')===__FILE__){
    if(in_array('--self-test',$argv??[],true)){sbw5_self_test();echo "MATCH_SAMO_BUSINESS_LIVE30_WAVE3_WRITER_V11_W1_SELFTEST_OK\n";exit;}
    ...
    sbw5_need(is_dir($root)&&basename($root)==='anytoour.ru'&&is_dir($dir)&&basename($dir)===$writerOp&&preg_match('/^hotel-match-samo-business-live30-common4-wave3-writer-1971-20260924-v11-w1$/D',$writerOp)===1&&is_file($auditPath)&&sbw5_sha($auditSha)&&hash_file('sha256',$auditPath)===$auditSha&&preg_match('/^[0-9a-f]{40}$/D',$sourceSha)===1,'runtime_scope');
