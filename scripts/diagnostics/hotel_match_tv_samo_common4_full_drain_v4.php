...
const HMC2_OP = 'hotel-match-tv-samo-common4-full-drain-1971-20260918-v4';
const HMC2_MAX_ANEX_DATE_PROBES = 8;
const HMC4_MAX_ANEX_CALLS = 48;
...
final class Hmc4AnexReadClient {
    ...
    public function request(string $action,array $params=[]): array {
        ...
        if(++$this->requests>HMC4_MAX_ANEX_CALLS)throw new RuntimeException('anex_call_budget');
...
function hmc4_samo_anex_full(
    ...
    for($page=1;$page<=20;$page++){
        ...
        $cl=new AnyTourAndromedaClient(function(string $url,array $options)use($tr,&$samoCalls){
            if(++$samoCalls>HMC_MAX_SAMO_CALLS)throw new RuntimeException('samo_call_budget');return $tr($url,$options);
        },true);
        $cl->restorePrivateSession($session);$reply=$cl->price($params);
        $pc=(int)$reply['PAGES_COUNT'];if($pc>20)throw new RuntimeException('samo_probe_page_cap');
        if($pc===0){
...
function hmc2_find_anex_samo_date(
    ...
    foreach($probeDates as $date){
        ...
        if(!$direct['matched_hotel_ids']){$entry['samo_intersection_count']=0;$attempts[]=$entry;continue;}
        ...
        if(!$candidateAndromeda){$attempts[]=$entry;continue;}
        ...
        $entry['strict_samo_hotel_count']=count($finalNative);$attempts[]=$entry;
        if($finalNative){
            $provenAndromeda=[];
    ...
    foreach([3,4,5] as $star){
        ...
        for($page=1;$page<=HMC_MAX_SAMO_PAGES;$page++){
            $params=[
                ...
                'TOWNTOINC'=>(string)$town[0]['id'],'HOTELS'=>implode(',',array_slice($dateProof['proven_andromeda_hotel_ids'],0,30)),
                'PACKETTYPE'=>0,'PAGE'=>$page,'GROUP_BY'=>32,
            ];
            $tr=new AnyTourAndromedaTransport(true);
            $cl=new AnyTourAndromedaClient(function(string $url,array $options)use($tr,&$samoCalls){
...
if(in_array('--self-test',$argv??[],true)){
    ...
    if(hmc2_dict_id($mock,['RUB'])!==1)throw new RuntimeException('v4_dict');
    echo "MATCH_TV_SAMO_COMMON4_FULL_DRAIN_V4_SELFTEST_OK\n";exit(0);
}
