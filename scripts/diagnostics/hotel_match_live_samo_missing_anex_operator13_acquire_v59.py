...
OP='hotel-match-live-samo-missing-anex-operator13-acquire-1971-20260926-v59'
...
V57_OP='hotel-match-live-samo-missing-anex-operator13-acquire-1971-20260926-v57'
V57_RESULT_SHA='3034f56a4c0c73d6792778bc77c516745e879600a18adb380e0f0ffc8101c80e'
V57_RECEIPT_SHA='8bceb1f759a989d5c2c2d932f4deee2ecdaba52abcfbe79de89104746ec0248f'
V57_CONSUMED_BATCHES=1
...
def load_consumed_v57(v57_dir, expected_result_sha, expected_receipt_sha):
    d=pathlib.Path(v57_dir)
    if not d.is_dir() or d.is_symlink(): raise RuntimeError('v57_dir')
    rp=d/'result.json';qp=d/'receipt.json'
    if not rp.is_file() or not qp.is_file() or rp.is_symlink() or qp.is_symlink(): raise RuntimeError('v57_terminal_files')
    raw=rp.read_bytes();qraw=qp.read_bytes()
    if hashlib.sha256(raw).hexdigest()!=expected_result_sha: raise RuntimeError('v57_result_sha')
    if hashlib.sha256(qraw).hexdigest()!=expected_receipt_sha: raise RuntimeError('v57_receipt_sha')
    r=json.loads(raw);q=json.loads(qraw)
    if r.get('operation')!=V57_OP or r.get('state')!='terminal_failed_no_replay': raise RuntimeError('v57_state')
    if r.get('selected_batch_count')!=90 or r.get('selected_hotel_count')!=90 or r.get('provider_calls')!=1 or r.get('operation_tariff_units')!=1 or r.get('completed_batches')!=0: raise RuntimeError('v57_counts')
    if r.get('reason')!='RuntimeError:http_400' or r.get('returned_edge_count')!=0 or r.get('captured_single_native_count')!=0: raise RuntimeError('v57_reason_counts')
    if q.get('operation')!=V57_OP or q.get('state')!='terminal_failed_no_replay' or q.get('result_sha256')!=expected_result_sha or q.get('no_replay') is not True: raise RuntimeError('v57_receipt')
    files=sorted(d.glob('batch-*-reservation.json'))
    if len(files)!=V57_CONSUMED_BATCHES: raise RuntimeError('v57_batch_reservation_count')
    consumed=set()
    for p in files:
        if not p.is_file() or p.is_symlink(): raise RuntimeError('v57_batch_reservation_file')
        x=load(p)
        if x.get('operation')!=V57_OP or x.get('state')!='reserved_before_batch_http': raise RuntimeError('v57_batch_reservation_state')
        k=str(x.get('context_key',''));ids=x.get('hotel_ids')
        if not k or not isinstance(ids,list) or not ids: raise RuntimeError('v57_batch_reservation_shape')
        sig=batch_signature(k,ids)
        if sig in consumed: raise RuntimeError('v57_batch_reservation_duplicate')
        consumed.add(sig)
    if len(consumed)!=V57_CONSUMED_BATCHES: raise RuntimeError('v57_consumed_count')
    return consumed
...
def build_plan(router,consumed):
    ...
    if len(consumed)!=172 or not consumed.issubset(all_sigs):raise RuntimeError('consumed_not_router_subset')
    candidates=[b for b in all_candidates if batch_signature(b['context_key'],b['hotel_ids']) not in consumed]
    if len(candidates)!=89:raise RuntimeError('remaining_router_batch_count')
    candidates.sort(key=lambda x:(-x['hotel_count'],x['context_key'],x['batch_index']))
    stale=[b for b in candidates if str(b['context'].get('departure_date','')) < EXPECTED_DAY]
    if len(stale)!=1 or stale[0]['hotel_ids']!=[21683] or str(stale[0]['context'].get('departure_date',''))!='2026-09-25':
        raise RuntimeError('stale_departure_scope_drift')
    eligible=[b for b in candidates if str(b['context'].get('departure_date','')) >= EXPECTED_DAY]
    if len(eligible)!=88: raise RuntimeError('provider_eligible_count')
    selected=[];budget=0
    for b in eligible:
    ...
    return {'selected_batches':selected,'selected_batch_count':len(selected),'selected_hotel_count':len(ids),'selected_hotel_ids':ids,'worst_case_http':budget,'all_routeable':659,'all_batches':261,'consumed_batch_count':len(consumed),'remaining_router_batch_count':len(candidates),'stale_departure_batch_count':len(stale),'stale_departure_hotel_ids':sorted({h for b in stale for h in b['hotel_ids']}),'provider_eligible_batch_count':len(eligible)}
...
def execute(root,opdir,router_path,v41b_dir,v45_dir,v45_result_sha,v45_receipt_sha,v49_dir,v49_result_sha,v49_receipt_sha,v53_dir,v53_result_sha,v53_receipt_sha,v57_dir,v57_result_sha,v57_receipt_sha,source_sha):
    router=load(router_path)
    consumed=load_consumed(v41b_dir) | load_consumed_v45(v45_dir,v45_result_sha,v45_receipt_sha) | load_consumed_v49(v49_dir,v49_result_sha,v49_receipt_sha) | load_consumed_v53(v53_dir,v53_result_sha,v53_receipt_sha) | load_consumed_v57(v57_dir,v57_result_sha,v57_receipt_sha)
    ...
    scope={'operation':OP,'source_sha':source_sha,'router_operation':router['operation_id'],'selected_batch_count':len(batches),'selected_hotel_count':len(active),
           ...
           'tourvisor_account':ACCOUNT,'consumed_batch_count':plan['consumed_batch_count'],'remaining_router_batch_count':plan['remaining_router_batch_count'],'skipped_stale_departure_count':plan['stale_departure_batch_count'],'skipped_stale_departure_hotel_ids':plan['stale_departure_hotel_ids'],'provider_eligible_batch_count':plan['provider_eligible_batch_count'],'safe_to_write_now':False}
    ...
    if not batches:
        out={'operation':OP,'state':'nothing_current_missing','source_sha':source_sha,'tourvisor_account':ACCOUNT,'provider_calls':0,'selected_hotel_count':0,
             'skipped_current_anex_count':len(skipped_anex),'skipped_current_samo_count':len(skipped_samo),'skipped_inactive_count':len(skipped_inactive),'skipped_stale_departure_count':plan['stale_departure_batch_count'],'skipped_stale_departure_hotel_ids':plan['stale_departure_hotel_ids'],'provider_eligible_batch_count':plan['provider_eligible_batch_count'],
...
def selftest():
    ...
    print('MATCH_V59_SELFTEST_OK')

if __name__=='__main__':
    if '--self-test' in sys.argv:selftest();sys.exit(0)
    if '--execute' not in sys.argv:raise SystemExit('disabled')
    root=pathlib.Path(os.environ['ANYTOUR_ROOT']);opdir=pathlib.Path(os.environ['MATCH_OPERATION_DIR']);router=pathlib.Path(os.environ['MATCH_ROUTER_RESULT']);v41b=pathlib.Path(os.environ['MATCH_V41B_DIR']);v45=pathlib.Path(os.environ['MATCH_V45_DIR']);v45_result_sha=os.environ['MATCH_V45_RESULT_SHA'];v45_receipt_sha=os.environ['MATCH_V45_RECEIPT_SHA'];v49=pathlib.Path(os.environ['MATCH_V49_DIR']);v49_result_sha=os.environ['MATCH_V49_RESULT_SHA'];v49_receipt_sha=os.environ['MATCH_V49_RECEIPT_SHA'];v53=pathlib.Path(os.environ['MATCH_V53_DIR']);v53_result_sha=os.environ['MATCH_V53_RESULT_SHA'];v53_receipt_sha=os.environ['MATCH_V53_RECEIPT_SHA'];v57=pathlib.Path(os.environ['MATCH_V57_DIR']);v57_result_sha=os.environ['MATCH_V57_RESULT_SHA'];v57_receipt_sha=os.environ['MATCH_V57_RECEIPT_SHA'];source=os.environ['MATCH_SOURCE_SHA']
    if opdir.name!=OP or not root.is_dir() or not router.is_file() or not v41b.is_dir() or not v45.is_dir() or not v49.is_dir() or not v53.is_dir() or not v57.is_dir() or not re.fullmatch(r'[0-9a-f]{64}',v45_result_sha) or not re.fullmatch(r'[0-9a-f]{64}',v45_receipt_sha) or not re.fullmatch(r'[0-9a-f]{64}',v49_result_sha) or not re.fullmatch(r'[0-9a-f]{64}',v49_receipt_sha) or not re.fullmatch(r'[0-9a-f]{64}',v53_result_sha) or not re.fullmatch(r'[0-9a-f]{64}',v53_receipt_sha) or not re.fullmatch(r'[0-9a-f]{64}',v57_result_sha) or not re.fullmatch(r'[0-9a-f]{64}',v57_receipt_sha) or not re.fullmatch(r'[0-9a-f]{40}',source):raise SystemExit('runtime_scope')
    ...
    out=execute(root,opdir,router,v41b,v45,v45_result_sha,v45_receipt_sha,v49,v49_result_sha,v49_receipt_sha,v53,v53_result_sha,v53_receipt_sha,v57,v57_result_sha,v57_receipt_sha,source)
