...
const OP = 'hotel-match-tourvisor-passive-target-gap-census-1971-20260917-v2';
...
function self_test(): void {
    ...
    $x=['state'=>'failed_no_replay']; $x['state']='completed_read_only'; req($x['state']==='completed_read_only','state_overwrite');
    echo "hotel_match_tourvisor_passive_target_gap_census_v2 self-test: PASS\n";
}
...
try {
    ...
    foreach (TV_TO_NS as $tv=>$ns) {
        ...
        $st=$db->prepare("SELECT COUNT(*) FROM andromeda_hotel_identities WHERE supplier_namespace=? AND decision_status='pending' AND local_hotel_id IS NULL");
        $st->execute([$ns]); $pendingCounts[$ns]=(int)$st->fetchColumn();
    }
    ...
    $out['passive_identity_rows_total']=$totalIdentity;
    $out['relevant_passive_rows']=count($passive);
    $out['distinct_relevant_pairs']=count($seenKey);
    $out['duplicate_relevant_pairs']=$duplicates;
    $out['by_tourvisor_operator']=$by;
    $out['pending_external_rows_by_namespace']=$pendingCounts;
    $out['missing_accepted_locals_total']=count($gaps);
    $out['core8_active_missing_accepted_locals']=$coreActiveGaps;
    $out['top_missing']=array_slice($gaps,0,200);
    $out['transaction']='REPEATABLE READ / READ ONLY';
    $out['read_at_utc']=gmdate('c');
} catch(Throwable $e) {
