...
// Owner-directed CURRENT acceptance after a proven no-write hash-contract defect.
// The terminal predecessor is immutable; no provider calls, updates or deletes.
const OP = 'hotel-match-current-accept178-1971-20260921-v1';
const PREDECESSOR = 'hotel-match-v9-provider178-write-1971-20260921-v1';
const MIN_WRITE = 100;
...
const INPUT_HASHES = [
    ...
    'terminal178.json' => 'be99d0f141ca2feac27dcb3bc1c760bd91b136f87694636db98ad80a023cd449',
];
...
// Exact #3327 contract, reused from checked PR3340 d83b492: seven fields IN SELECT ORDER.
function precommitAnchorHashV1(array $row): string {
    $projection = [];
    foreach (['supplier_namespace','external_hotel_id','local_hotel_id','decision_status','catalog_sha256','evidence_sha256','evidence_json'] as $field) {
        need(array_key_exists($field, $row), 'precommit_anchor_missing_field:' . $field);
...
function seeds(string $dir): array {
    ...
    $v = $d['v9.json']; $pre = $d['precommit178.json']; $done = $d['committed328.json']; $terminal = $d['terminal178.json'];
    need($terminal['operation'] === PREDECESSOR && $terminal['state'] === 'completed_no_write_below_threshold' && $terminal['mapping_writes'] === 0 && $terminal['database_writes'] === 0 && $terminal['current_safe'] === 0, 'predecessor_not_proven_no_write');
    need($terminal['hold_counts'] === ['canonical_anchor_row_changed' => 178] && count($terminal['holds']) === 178, 'predecessor_reason');
    ...
    $held = []; foreach ($terminal['holds'] as $r) { need($r['reason'] === 'canonical_anchor_row_changed', 'terminal_hold'); $held[$r['key']] = (int)$r['tv_hotel_id']; }
    foreach ($out as $s) need(($held[keyOf($s)] ?? null) === $s['tv_hotel_id'], 'terminal_membership');
    need(count($out) === 178 && count($held) === 178 && count(array_unique(array_column($out, 'tv_hotel_id'))) === 150, 'cohort');
...
function writeMappings(PDO $db, array $seeds, string $dir, string $sha): array {
    $base = ['operation'=>OP,'source_sha'=>$sha,'predecessor'=>PREDECESSOR,'input_candidates'=>count($seeds),'provider_calls'=>0,'tourvisor_calls'=>0,'samo_calls'=>0,'andromeda_calls'=>0];
    ...
    try {
        ...
        $all = query($db, $identitySql . ' FOR UPDATE'); need(count($all) <= 50000, 'identity_cap'); [$keys, $targets] = indexed($all);
