...
function hmgcr_tokens(string $name,array $places=[]): array {
    ...
    foreach(fc_tokens($name,true) as $token){$t=fc_norm(hmgcr_latin((string)$token));if($t===''||isset($drop[$t])||isset($weak[$t]))continue;$out[$t]=true;}
...
function hmgcr_best_pair(array $sourceNames,array $targetNames,array $sourcePlaces,array $targetPlaces): array {
    ...
    foreach($sourceNames as $s){$s=trim((string)$s);if($s==='')continue;foreach($targetNames as $t){$t=trim((string)$t);if($t==='')continue;$p=hmgcr_pair($s,$t,$sourcePlaces,$targetPlaces);$rank=[(int)$p['exact_bag'],(int)$p['critical_ok'],$p['shared'],$p['score']];$old=[(int)($best['exact_bag']??false),(int)($best['critical_ok']??false),(int)($best['shared']??0),(float)($best['score']??0)];if($rank>$old)$best=$p;}}
    return $best;
}
...
function hmgcr_source_candidate_ids(array $sourceNames,array $sourcePlaces,int $country,array $index): array {
    $exact=[];$hits=[];
    foreach($sourceNames as $name){$tokens=hmgcr_tokens((string)$name,$sourcePlaces);if(!$tokens)continue;$key=hmgcr_key($tokens);foreach(array_keys($index['exact'][$country][$key]??[]) as $id)$exact[(int)$id]=true;foreach($tokens as $t)foreach(array_keys($index['token'][$country][$t]??[]) as $id)$hits[(int)$id]=($hits[(int)$id]??0)+1;}
...
function hmgcr_decide(array $source,int $country,array $index,array $hotels,array $names,array $sameProviderClaims,array $excluded=[]): array {
    ...
    if(count($pool)>1&&$margin<0.15&&!($best['pair']['exact_bag']??false))return ['bucket'=>'near','reason'=>'bridge_winner_margin_small','target_local_hotel_id'=>$best['id'],'score_margin'=>round($margin,6),'distance_m'=>$best['distance_m'],'pair'=>$best['pair']];
