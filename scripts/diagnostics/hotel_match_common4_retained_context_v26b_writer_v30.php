...
const SBW5_WRITER_OP_RE='/^hotel-match-common4-retained-context-v26b-writer-1971-20260925-v30-w[12]$/D';
...
function sbw5_manifest(array $audit,int $offset,int $expected):array{
    ...
    foreach(($audit['rows']??[]) as $r){
        ...
        sbw5_need(isset(SBW5_NS[$ns])&&SBW5_NS[$ns]===$op&&$ext!==null&&$local>0,'row_identity');
...
function sbw5_self_test():void{
    $rows=[];
    for($i=0;$i<301;$i++){
        $ns=$i<29?'operator_315':'operator_5';$op=$i<29?315:5;$ext=(string)(1000+$i);$local=100+$i;$catalog=(string)(10000+$i);
        $cat=hash('sha256','catalog-'.$i);$ev=hash('sha256','evidence-'.$i);$edge=hash('sha256','edge-'.$i);
        $anchor=['supplier_namespace'=>'andromeda_catalog','external_hotel_id'=>$catalog,'local_hotel_id'=>$local,'decision_status'=>'accepted','catalog_sha256'=>$cat,'evidence_sha256'=>$ev];
        $rows[]=['supplier_namespace'=>$ns,'operator_id'=>$op,'external_hotel_id'=>$ext,'local_hotel_id'=>$local,'source_operation'=>'hotel-match-common4-retained-context-acquire-1971-20260925-v26b','source_result_sha256'=>str_repeat('c',64),'source_edge_sha256'=>$edge,'source_edge_sha256s'=>[$edge],'input_catalog_ids'=>[$catalog],'catalog_hotel'=>['id'=>$local,'name'=>'X'.$i,'country_id'=>1,'country_name'=>'Turkey','region_id'=>2,'region_name'=>'R','subregion_id'=>3,'subregion_name'=>'S','category'=>'5','is_active'=>1],'unanimous_catalog_sha256'=>$cat,'anchors'=>[$anchor],'status'=>'writer_ready','anchor_state'=>'canonical_anchor_ok','writer_ready'=>true,'safe_to_write_now'=>false];
    }
    ...
    $c1=[];foreach($m1['rows'] as $r)$c1[$r['supplier_namespace']]=($c1[$r['supplier_namespace']]??0)+1;ksort($c1);sbw5_need($c1===['operator_315'=>29,'operator_5'=>271],'self_slice1_namespaces');
    sbw5_need($m2['rows'][0]['supplier_namespace']==='operator_5','self_slice2_namespace');
    sbw5_need(preg_match(SBW5_WRITER_OP_RE,'hotel-match-common4-retained-context-v26b-writer-1971-20260925-v30-w1')===1&&preg_match(SBW5_WRITER_OP_RE,'hotel-match-common4-retained-context-v26b-writer-1971-20260925-v30-w2')===1&&preg_match(SBW5_WRITER_OP_RE,'hotel-match-common4-retained-context-v26b-writer-1971-20260925-v30-w3')===0,'self_writer_operation_guard');
}
if(PHP_SAPI==='cli'&&realpath($_SERVER['SCRIPT_FILENAME']??'')===__FILE__){
    if(in_array('--self-test',$argv??[],true)){sbw5_self_test();echo "MATCH_COMMON4_RETAINED_CONTEXT_V26B_WRITER_V30_SELFTEST_OK\n";exit;}
