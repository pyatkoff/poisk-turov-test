...
const HMAN_OP='hotel-match-andromeda-native-anex-mass-1971-20260918-v1';
...
function hman_name_veto(string $provider,array $localNames):array{
    $pn=hman_nums($provider);$pq=hman_quals($provider);$best=0;$ok=false;$reason=null;
    foreach($localNames as$l){$ln=hman_nums($l);$lq=hman_quals($l);if($pn!==$ln){$reason='number_conflict';continue;}if(($pq||$lq)&&$pq!==$lq){$reason='qualifier_conflict';continue;}$ok=true;$best=max($best,hman_score($provider,$l));}
...
if(in_array('--self-test',$argv??[],true)){
    ...
    echo"MATCH_ANDROMEDA_NATIVE_ANEX_MASS_SELFTEST_OK\n";exit;
}
...
try{
    ...
    $db->rollBack();
    if(!$selected)throw new RuntimeException('no_contexts');
    ...
    $result=['operation'=>HMAN_OP,'state'=>'completed_read_only','source_sha'=>$sourceSha,'provider_access'=>true,'login_calls'=>$loginCalls,'andromeda_calls'=>$calls,'price_rows_seen'=>$providerRows,'current_anex_accepted_unique_local'=>count($localToNative),'target_user_seen_anex_accepted_andromeda_missing'=>count($target),'selected_context_count'=>count($selected),'context_status_counts'=>$ctxCounts,'context_meta'=>$contextMeta,'raw_direct_bridge_rows'=>count($raw),'safe_candidate_count'=>count($safe),'safe_candidates'=>$safe,'hold_count'=>count($holds),'holds'=>array_slice($holds,0,300),'database_writes'=>0,'mapping_writes'=>0,'tourvisor_calls'=>0,'direct_anex_calls'=>0,'no_replay'=>true];
