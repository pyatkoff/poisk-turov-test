...
const R30_OP = 'hotel-match-anex-rolling30-1971-20260919-v1';
const R30_EXCLUDED = [
 ...
 1063,1441,66037,950,1580,42576,2191,75538,72865,1244,115349,75791,152266,132017,42596,103212,42592,131058,42581,75871,156593,9443,147573,9427,99582,157197,62868,967,132716,1151,1546,1500,
 44669,2559,74083,2590,32230,2522,73509,55495,73055,63906,2586,113007,2565,2582,66192,133061,88323,54603,2634,49851,
 11745,11751,11754,11756,11758,11761,11770,11775,11788,11789,26841,64582,11769,11771,11792,11802,11805,15788,15794,26842,28452,35164,35170,44413,60234,68977,106703
];
...
if(($argv[1]??'')==='--self-test') {
 if(!r30_product('FORTUNA 5*')||r30_product('HOTEL SU')||count(R30_EXCLUDED)!==109)throw new RuntimeException('planner_test');
 echo "ROLLING30_PLANNER_SELFTEST_OK\n";exit;
}
...
try {
 ...
 $excluded=array_fill_keys(R30_EXCLUDED,true);$stats=['observation_rows'=>count($rows),'accepted'=>0,'protected'=>0,'prior_or_foreign_claim'=>0,'product_or_country'=>0];$groups=[];$front=[];
 foreach($rows as $h){$id=(int)$h['hotel_id'];
  ...
  $h['source']='user_search';$h['operator_id']=13;$h['has_accepted_andromeda']=isset($and[$id]);$front[$id]=true;
  $key=$h['departure_id'].'|'.$h['country_id'];$groups[$key][]=$h;
 }
 $contexts=[];
 foreach($groups as $g){$dates=array_values(array_unique(array_column($g,'departure_date')));sort($dates,SORT_STRING);
  foreach($dates as $start){$end=(new DateTimeImmutable($start))->modify('+6 days')->format('Y-m-d');$by=[];
   ...
   if(!$by)continue;$hs=array_values($by);usort($hs,fn($a,$b)=>(int)$b['has_accepted_andromeda']<=>(int)$a['has_accepted_andromeda']?:$b['observations']<=>$a['observations']?:$a['hotel_id']<=>$b['hotel_id']);
   $contexts[]=['departure_id'=>$hs[0]['departure_id'],'country_id'=>$hs[0]['country_id'],'country_name'=>$hs[0]['country_name'],'date_from'=>$start,'date_to'=>$end,'nights_from'=>7,'nights_to'=>10,'adults'=>2,'children_count'=>0,'hotels'=>$hs,'bridge_count'=>count(array_filter($hs,fn($h)=>$h['has_accepted_andromeda']))];
  }
 }
 usort($contexts,fn($a,$b)=>count($b['hotels'])<=>count($a['hotels'])?:$b['bridge_count']<=>$a['bridge_count']?:strcmp($a['date_from'],$b['date_from'])?:$a['country_id']<=>$b['country_id']);
 $clock=r30_rows($db,'SELECT UTC_TIMESTAMP AS db_utc_timestamp')[0];$db->exec('ROLLBACK');
 echo r30_json(['operation'=>R30_OP,'state'=>'current_read_only_complete','captured_at'=>$clock['db_utc_timestamp'],'frontier_unique'=>count($front),'stats'=>$stats,'contexts'=>$contexts,'excluded_ids'=>R30_EXCLUDED,'supplier_calls'=>0,'database_writes'=>0,'mapping_writes'=>0]);
