...
const OP='search3-condition-mapping-current-audit-3353-20260921-v3';
...
function snapshot(PDO $db):array{
 ...
 try{
  ...
  $analysis=['by_namespace'=>[],'by_meal_id'=>[],'numeric_external_keys'=>0,'non_numeric_external_keys'=>0];
  foreach($mealRows as $r){$ns=(string)$r['namespace'];$mid=(string)$r['meal_id'];$key=(string)$r['external_key'];$analysis['by_namespace'][$ns]=($analysis['by_namespace'][$ns]??0)+1;$analysis['by_meal_id'][$mid]['count']=($analysis['by_meal_id'][$mid]['count']??0)+1;$analysis['by_meal_id'][$mid]['namespaces'][$ns]=($analysis['by_meal_id'][$mid]['namespaces'][$ns]??0)+1;if(count($analysis['by_meal_id'][$mid]['sample_external_keys']??[])<20)$analysis['by_meal_id'][$mid]['sample_external_keys'][]=$key;if(preg_match('/^[0-9]+$/D',$key))$analysis['numeric_external_keys']++;else $analysis['non_numeric_external_keys']++;}
  ksort($analysis['by_namespace']);ksort($analysis['by_meal_id']);
  ...
  $db->rollBack();
  return ['stay_mapping_table_present'=>$exists,'meal_mapping_rows'=>$mealRows,'meal_mapping_rows_truncated'=>count($mealRows)===1001,'mapping_counts'=>array_count_values(array_map(fn($r)=>$r['namespace'].'|'.$r['key_kind'].'|'.$r['state'],$mealRows)),'meal_mapping_analysis'=>$analysis,'dictionary_schema'=>$tables,'schema_discovery_truncated'=>count($meta)===2001,'runtime_files'=>$files];
