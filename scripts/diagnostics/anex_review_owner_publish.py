...
def prepare(directory, source, plan):
    if not re.fullmatch('[0-9a-f]{40}',source) or plan.get('action') not in ('inspect','apply'):
    ...
    if plan['action']=='apply':storage.write_json(cp_path,next_cp)
...
def main():
    ...
    if plan['action']=='apply':storage.write_json(directory/CHECKPOINT,dict(cp,state='executing'))
    report=storage.execute(payload,publish_owner=True)
    expected='published' if plan['action']=='apply' else 'ready'
    ...
    if plan['action']=='apply':
        ...
        storage.write_json(directory/CHECKPOINT,dict(cp,state='completed',report=report,report_sha256=canonical(report)))
    storage.write_json(directory/('anex-owner-publish-report.json' if plan['action']=='apply' else 'anex-owner-publish-readiness.json'),report)
