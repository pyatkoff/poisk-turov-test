...
def prepare(directory, source, plan):
    if not re.fullmatch('[0-9a-f]{40}',source) or plan.get('action') not in ('inspect','apply','repair'):
    ...
    cp_path=directory/('anex-owner-repair-checkpoint.json' if plan['action']=='repair' else CHECKPOINT)
    ...
    if plan['action']=='repair':
        published=json.loads((directory/CHECKPOINT).read_bytes())
    ...
    if plan['action'] in ('apply','repair'):storage.write_json(cp_path,next_cp)
...
def main():
    ...
    cp_name='anex-owner-repair-checkpoint.json' if plan['action']=='repair' else CHECKPOINT
    if plan['action'] in ('apply','repair'):storage.write_json(directory/cp_name,dict(cp,state='executing'))
    report=storage.execute(payload,publish_owner=True)
    expected='published' if plan['action'] in ('apply','repair') else 'ready'
    ...
    if plan['action'] in ('apply','repair'):
    ...
    storage.write_json(directory/('anex-owner-repair-report.json' if plan['action']=='repair' else 'anex-owner-publish-report.json' if plan['action']=='apply' else 'anex-owner-publish-readiness.json'),report)
