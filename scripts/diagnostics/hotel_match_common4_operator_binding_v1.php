...
function hm_common4_aliases(): array
{
    return [
        ...
        'funsun' => ['FUN&SUN', 'FUN SUN', 'FUN and SUN', 'FUN&SUN Russia', 'ФАН&САН', 'ФАН САН'],
        'biblio_globus' => ['Библио-Глобус', 'Библио Глобус', 'Biblio Globus', 'Biblio-Globus', 'BiblioGlobus'],
...
{
    foreach (['name', 'lName', 'NAME', 'operator', 'operatorName'] as $key) {
...
function hm_common4_resolve(array $payload, string $namespace, ?array $required = null): array
{
    ...
    foreach ($required as $canonical) {
        if (!isset($aliases[$canonical])) {
            throw new InvalidArgumentException('unknown_canonical_operator:' . $canonical);
        }
    }
    ...
    foreach ($aliases as $canonical => $names) {
        foreach ($names as $name) {
            ...
            if ($norm === '') {
                throw new RuntimeException('empty_alias:' . $canonical);
            }
            if (isset($wanted[$norm]) && $wanted[$norm] !== $canonical) {
                throw new RuntimeException('alias_collision:' . $norm);
            }
    ...
    foreach (hm_common4_rows($payload) as $row) {
        ...
        if ($canonical === null || !in_array($canonical, $required, true)) {
            continue;
        }
    ...
    foreach ($required as $canonical) {
        ...
        if (count($candidates) !== 1) {
            throw new RuntimeException('operator_binding_' . $canonical . '_count_' . count($candidates));
        }
        $row = $candidates[0];
        $id = $row['provider_operator_id'];
        if (isset($usedIds[$id]) && $usedIds[$id] !== $canonical) {
            throw new RuntimeException('operator_id_collision:' . $id);
        }
...
function hm_common4_read_json(string $path): array
{
    ...
    if (!is_string($raw) || strlen($raw) > 8 * 1024 * 1024) {
        throw new RuntimeException('dictionary_read');
    }
    $value = json_decode($raw, true, 32, JSON_THROW_ON_ERROR);
    if (!is_array($value)) {
        throw new RuntimeException('dictionary_shape');
    }
    return $value;
}
