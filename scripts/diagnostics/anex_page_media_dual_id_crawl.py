...
UA='AnyTour-MATCH-page-media-dual-id/1.0'
...
def main():
    ...
    for row in ev.get('matched') or []:
        ...
        if not aid or cid not in COUNTRY:continue
        for h in row.get('hits') or []:
            ...
            for v in (d.get('b2cLink'),d.get('slug'),(h or {}).get('path')):
                ...
                elif v.startswith('/'): candidates.append('https://anextour.ru'+v)
                else:candidates.append(f'https://anextour.ru/hotels/{COUNTRY[cid]}/{v.strip("/")}')
            if candidates:
                seeds.append({'anex_hotel_code':aid,'country_id':cid,'page_candidates':list(dict.fromkeys(candidates))});break
    uniq={};
    for s in seeds:uniq.setdefault(s['anex_hotel_code'],s)
    rows=[]; flat=[]; errors=[]; pair_keys=set(); by_anex={}; by_samo={}
    for aid,s in uniq.items():
        urls=set(); page_meta=[]
        for page in s['page_candidates']:
            try:final,body,status,ctype=get(page)
            except Exception as e:
                page_meta.append({'page':page,'error':type(e).__name__+':'+str(e)[:200]});continue
            text=body.decode(errors='replace')
            found=[]
            for raw in URL_RE.findall(text):
                u=norm_url(raw)
                if 'hotelCode=' in u or 'hotelcode=' in u: found.append(u); urls.add(u)
            page_meta.append({'page':page,'final_url':final,'http':status,'content_type':ctype,'bytes':len(body),'media_candidates':len(found)})
            if found:break
        pairs=[]
        for u in sorted(urls):
            p=extract(u)
            if not p:continue
            # Direct bridge is only valid for the ANEX hotel page we are processing.
            if int(p['anex_hotel_code'])!=aid:continue
        ...
        rows.append({'anex_hotel_code':aid,'country_id':s['country_id'],'pages':page_meta,'media_url_count':len(urls),'direct_pairs':pairs,'pair_count':len(pairs)})
        time.sleep(ns.delay)
    ...
    out={'status':'completed','source_seed_count':len(uniq),'page_rows':len(rows),'media_url_count':sum(r['media_url_count'] for r in rows),'direct_pair_count':len(flat),'unique_anex_with_pair':len(by_anex),'unique_samo_with_pair':len(by_samo),'conflicts':conflicts,'pairs':flat,'rows':rows,'errors':errors,'database_writes':0,'mapping_writes':0}
