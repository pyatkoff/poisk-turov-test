...
const HMLGUA_OPERATION = 'hotel-match-literal-geo-union-accept-1971-20260912-v2';
...
function hmlgua_plan_current_in_transaction(PDO $db): array {
    ...
    $mapped=[];$anClaims=[];
    foreach($mappingRows as$r){if((int)($r['enabled']??0)!==1)continue;$aid=(int)$r['anex_hotel_id'];$hid=(int)$r['catalog_hotel_id'];$mapped[$aid]=true;$anClaims[$hid][]=$aid;}
    foreach($manualRows as$r){if((string)($r['decision_status']??'')!=='accepted'||$r['catalog_hotel_id']===null)continue;$anClaims[(int)$r['catalog_hotel_id']][]=(int)$r['anex_hotel_id'];}
    $excluded=[];foreach($exclusionRows as$r)$excluded[(int)$r['anex_hotel_id']][(int)$r['catalog_hotel_id']]=true;
    $andStatus=[];$andClaims=[];$identityByExternal=[];
    foreach($identityRows as$r){$external=(string)$r['external_hotel_id'];$identityByExternal[$external]=$r;$andStatus[$external]=(string)$r['decision_status'];if((string)$r['decision_status']==='accepted'&&$r['local_hotel_id']!==null)$andClaims[(int)$r['local_hotel_id']][]=$external;}
    ...
    $nodes=[];
    foreach($an as$aid=>$s){if($s['local_ids']||!isset(HMLGUR_CORE8[(int)$s['country_id']]))continue;$stats['unresolved_anex']++;if(isset($manual[(int)$aid])){$stats['manual_protected']++;continue;}if(isset($mapped[(int)$aid])){$stats['existing_mapping_protected']++;continue;}$nodes[hmigr_key('anex',$aid)]=['provider'=>'anex','external_id'=>(string)$aid,'country_id'=>(int)$s['country_id'],'live'=>(bool)$s['live'],'observation_count'=>(int)$s['observation_count'],'source'=>$s];}
    ...
    foreach($nodes as$key=>$node){
        $stats['examined']++;if($node['live'])$stats['live_examined']++;$source=$node['source'];$country=$node['country_id'];$literalTargets=hmelmr_targets($source,$country,$literalIndex,$hotels,$names);$singleTargets=hmstg_targets($source,$country,$singleIndex,$hotels,$names);$cons=hmlgur_consensus($literalTargets,$singleTargets);
        ...
        $target=(int)$cons['target_local_hotel_id'];$literalEv=$literalTargets[$target]??null;$singleEv=$singleTargets[$target]??null;$literalEligible=$literalEv!==null&&count($literalTargets)===1&&count(hmelmr_keys($source['names']??[]))>0;$singleEligible=false;$coordOk=false;$placeOk=false;
        ...
        $routes=[];if($literalEligible){$routes[]='exact_ordered_literal';$stats['literal_route_prepared']++;}if($singleEligible){$routes[]=$coordOk?'single_token_coord_le_1km':'single_token_exact_place';$stats['single_geo_route_prepared']++;}if(count($routes)>1)$stats['both_routes_same_target']++;$ev=$literalEv??$singleEv;$rows[$key]=['bucket'=>'prepared','reason'=>'literal_geo_union_consensus','target_local_hotel_id'=>$target,'routes'=>$routes,'evidence'=>$ev]+$node;
    }
    $groups=[];foreach($rows as$key=>$r)if(($r['bucket']??'')==='prepared')$groups[$r['provider'].':'.$r['target_local_hotel_id']][]=$key;foreach($groups as$keys)if(count($keys)>1)foreach($keys as$key){$rows[$key]['bucket']='needs_extra_evidence';$rows[$key]['reason']='same_provider_union_target_collision';$stats['global_uniqueness_demotions']++;}
...
function hmlgua_accept(PDO $db,string $operation=HMLGUA_OPERATION):array{
    if($operation!==HMLGUA_OPERATION)throw new RuntimeException('operation_scope');fc_require_tables($db);mba_require_transactional($db);$db->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);$db->exec('SET SESSION innodb_lock_wait_timeout=30');$db->exec('SET TRANSACTION ISOLATION LEVEL REPEATABLE READ');$writes=0;$committed=false;$acceptedAn=[];$acceptedAnd=[];$mappingDigest=fc_hash([$operation,'literal_geo_union_server_current_v2']);
    try{
        $db->beginTransaction();$plan=hmlgua_plan_current_in_transaction($db);$before=$plan['coverage'];
        ...
        foreach($plan['prepared'] as$row){$target=(int)$row['target_local_hotel_id'];if($target<1)throw new RuntimeException('prepared_target_missing');if($row['provider']==='anex'){$id=(int)$row['external_id'];$evidence=hmlgua_anex_evidence($operation,$row);$digest=fc_hash($evidence);$insert->execute([$id,$target,HMLGUA_POLICY,$digest,$mappingDigest]);if($insert->rowCount()!==1)throw new RuntimeException('anex_insert_not_one');$acceptedAn[$id]=['target'=>$target,'source_row_digest'=>$digest,'routes'=>$row['routes']??[],'live'=>(bool)($row['live']??false)];$writes++;}else{$external=(string)$row['external_id'];$identity=$plan['identity_by_external'][$external]??null;if(!is_array($identity)||(string)$identity['decision_status']!=='pending'||$identity['local_hotel_id']!==null)throw new RuntimeException('andromeda_current_identity_missing');$prior=fc_evidence($identity['evidence_json']??'');$evidence=hmlgua_andromeda_evidence($operation,$prior,$row);$json=fc_json($evidence);$hash=hash('sha256',$json);$update->execute([$target,$hash,$json,$external,$identity['evidence_sha256']]);if($update->rowCount()!==1)throw new RuntimeException('andromeda_concurrent_change');$acceptedAnd[$external]=['target'=>$target,'evidence_sha256'=>$hash,'routes'=>$row['routes']??[],'live'=>(bool)($row['live']??false)];$writes++;}}
        $db->commit();$committed=true;
        $qAn=$db->prepare("SELECT catalog_hotel_id,source_row_digest,mapping_digest,enabled FROM anex_hotel_search_mappings WHERE anex_hotel_id=? AND scope='preview' AND approval_policy=?");foreach($acceptedAn as$id=>$expected){$qAn->execute([(int)$id,HMLGUA_POLICY]);$actual=$qAn->fetch(PDO::FETCH_ASSOC);if(!$actual||(int)$actual['catalog_hotel_id']!==(int)$expected['target']||!hash_equals((string)$expected['source_row_digest'],(string)$actual['source_row_digest'])||!hash_equals($mappingDigest,(string)$actual['mapping_digest'])||(int)$actual['enabled']!==1)throw new RuntimeException('anex_post_commit_readback_failed');}
        $qAnd=$db->prepare("SELECT local_hotel_id,decision_status,evidence_sha256 FROM andromeda_hotel_identities WHERE supplier_namespace='andromeda_catalog' AND external_hotel_id=?");foreach($acceptedAnd as$external=>$expected){$qAnd->execute([(string)$external]);$actual=$qAnd->fetch(PDO::FETCH_ASSOC);if(!$actual||(int)$actual['local_hotel_id']!==(int)$expected['target']||(string)$actual['decision_status']!=='accepted'||!hash_equals((string)$expected['evidence_sha256'],(string)$actual['evidence_sha256']))throw new RuntimeException('andromeda_post_commit_readback_failed');}
    ...
    }catch(Throwable$e){if($db->inTransaction())$db->rollBack();$known=['operation_scope','required_table_missing','required_transactional_table_missing','transaction_required','prepared_target_missing','anex_insert_not_one','andromeda_current_identity_missing','andromeda_concurrent_change','anex_post_commit_readback_failed','andromeda_post_commit_readback_failed','country_contract_changed','hotel_scope_limit','alias_scope_limit','write_scope_limit'];$message=$e->getMessage();$lower=strtolower($message);$reason=in_array($message,$known,true)?$message:'runtime_failure';if($e instanceof PDOException&&strpos($lower,'lock wait timeout')!==false)$reason='db_lock_timeout';if($e instanceof PDOException&&strpos($lower,'deadlock')!==false)$reason='db_deadlock';return ['status'=>'failed','operation_id'=>$operation,'mode'=>'server_current_literal_geo_union_guarded_write','database_writes'=>$committed?$writes:0,'mapping_writes'=>$committed?$writes:0,'committed'=>$committed,'supplier_calls'=>0,'tourvisor_calls'=>0,'historical_operations_replayed'=>false,'reason'=>$reason,'readback_verified'=>false];}
}

if(PHP_SAPI==='cli'&&realpath($_SERVER['SCRIPT_FILENAME']??'')===__FILE__){$root=realpath(__DIR__.'/../..');require_once $root.(is_file($root.'/data/db-v1.php')?'/data/db-v1.php':'/v2/data/db-v1.php');echo fc_json(hmlgua_accept(v2_data_db(),HMLGUA_OPERATION)),"\n";}
