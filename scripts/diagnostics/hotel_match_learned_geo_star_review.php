...
const HMLGS_OPERATION='hotel-match-learned-geo-star-review-2333-20260913-v3';
...
function hmlgs_review(PDO $db,string $operation=HMLGS_OPERATION): array {
    ...
    foreach($db->query("SELECT external_hotel_id,local_hotel_id,evidence_json FROM andromeda_hotel_identities WHERE supplier_namespace='andromeda_catalog' AND decision_status='accepted' AND local_hotel_id IS NOT NULL")->fetchAll(PDO::FETCH_ASSOC) as $r){
        ...
        $src=$ev['prior_evidence']['source']??($ev['source']??[]);if(!is_array($src))$src=[];$geo=$ev['prior_evidence']['geography']??($ev['geography']??[]);if(!is_array($geo))$geo=[];
        $sp=[];foreach([(string)($src['town']??''),(string)($geo['town']??''),(string)($geo['parent']??'')] as $v){$k=hmlgs_place_key($v);if($k!=='')$sp[$k]=1;}$tk=hmlgs_target_keys($hotels[$local]);if(!$sp||!$tk)continue;$accepted++;foreach(array_keys($sp) as $s)foreach($tk as $t)$learn[$s][$t]=($learn[$s][$t]??0)+1;
    }
    $rows=[];$stats=['strict_star_mismatch'=>0,'learned_geo_supported'=>0,'learned_geo_strong'=>0,'unsafe_no_geo_support'=>0];
    ...
    return ['status'=>'completed','operation_id'=>$operation,'database_writes'=>0,'supplier_calls'=>0,'historical_operations_replayed'=>false,'accepted_training_rows'=>$accepted,'learned_place_keys'=>count($learn),'coverage'=>$base['coverage'],'stats'=>$stats,'rows'=>$rows];
}
