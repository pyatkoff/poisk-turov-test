...
function hmd_room_key(string $v):string{$t=hmd_tokens($v);sort($t,SORT_STRING);return implode(' ',$t);}
...
function hmd_resort_aliases(string $v,string $sub=''):array{$base=[];foreach([$v,$sub]as$x){$x=trim($x);if($x!=='')$base[]=hmd_norm($x);}return array_values(array_unique($base));}
