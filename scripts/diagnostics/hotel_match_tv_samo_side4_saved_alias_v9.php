...
function hma9_execute(string $opDir): array {
    ...
    }catch(Throwable){throw new RuntimeException('alias_resolver');}
    return ['operation'=>HMA9_OP,'state'=>'completed_read_only_saved_alias','source_operation'=>HMA7_SOURCE_OP,
        ...
        'biblio_fuel_policy'=>'owner_policy_zero_included_no_fuel_sample','resolver'=>['baseline_exact'=>$baselinePairs,
            'baseline_exact_count'=>count($baselinePairs),'unresolved_tv_hotels'=>count($unresolvedHotels),'alias'=>$aliases],
...
if(PHP_SAPI==='cli'&&realpath($_SERVER['SCRIPT_FILENAME']??'')===__FILE__){
    ...
    try{$result=hma9_execute($opDir);$sha=hmc_write($opDir.'/result.json',$result);hmc_write($opDir.'/receipt.json',['operation'=>HMA9_OP,'state'=>$result['state'],'result_sha256'=>$sha,'readback_verified'=>hash_file('sha256',$opDir.'/result.json')===$sha,'provider_accessed'=>false,'provider_http_calls'=>0,'database_reads'=>0,'database_writes'=>0,'mapping_writes'=>0]);echo hmc_json(['state'=>$result['state'],'baseline'=>$result['resolver']['baseline_exact_count'],'strong_alias'=>$result['resolver']['alias']['strong_count'],'review'=>$result['resolver']['alias']['review_count']])."\n";}
