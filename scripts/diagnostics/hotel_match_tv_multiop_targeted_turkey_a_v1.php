...
/** MATCH #1971 Turkey-A wrapper over pinned Egypt v2 acquisition runtime. */
...
$newOp='hotel-match-tv-multiop-targeted-1971-20260916-v1-turkey-a';
if(substr_count($src,$oldOp)!==1)throw new RuntimeException('v2_operation_marker_changed');$src=str_replace($oldOp,$newOp,$src);
if(substr_count($src,'2026-11-15')<2)throw new RuntimeException('v2_date_marker_changed');$src=str_replace('2026-11-15','2026-11-18',$src);
...
$newTargets='const HMM2_TARGETS=[23894=>1600,45225=>153743,32880=>17443,44573=>59085,43984=>99257,16605=>115500,32692=>70291,32745=>85422,37885=>111046,32724=>17586,29446=>17586,32686=>17390,28933=>17390,30424=>43550,45226=>163543,32742=>132803,29036=>132803,34804=>81578,39389=>1557,43077=>1558,44138=>53531,44139=>67042,8550=>1283,10115=>2160,11241=>1255,11756=>996,12143=>1004,12901=>1474,14812=>5535,15046=>1217,21696=>1070,27777=>17561,29332=>60388];';
...
if(str_contains($src,$oldOp)||str_contains($src,'2026-11-15')||str_contains($src,"['егип','egypt']")||substr_count($src,'search_created_before_poll')!==1||substr_count($src,'foreach([8,12,15,15,15] as $wait)')!==1)throw new RuntimeException('turkey_patch_incomplete');
if(in_array('--emit',$argv??[],true)){echo $src;exit(0);}
$tmp=sys_get_temp_dir().'/hotel-match-tv-multiop-turkey-a-'.getmypid().'.php';if(file_put_contents($tmp,$src,LOCK_EX)!==strlen($src))throw new RuntimeException('temp_write_failed');try{require $tmp;}finally{@unlink($tmp);}
