...
const SBW5_AUDIT_OP='hotel-match-samo-business-live30-common4-wave4-current-1971-20260925-v12';
...
function sbw5_manifest(array $audit,int $offset,int $expected):array{
    ...
    sbw5_need((int)($audit['captured_single_native_edge_count']??0)===22&&(int)($audit['writer_ready_total']??0)===22,'audit_wave1_counts');
    sbw5_need(($audit['writer_ready_counts']??null)===['operator_115'=>22],'audit_wave1_namespace');
    ...
    sbw5_need($offset===0&&$expected===22&&$total===22,'wave1_exact_slice');
    $all=[];$sources=[];$targets=[];
...
function sbw5_evidence(array $m,array $target,array $anchors,string $writerOp,string $sourceSha,string $auditSha):array{
    return ['operation_id'=>$writerOp,'rule'=>'samo_business_live30_wave4_exact_operator_native_current_anchor_v13','source_sha'=>$sourceSha,'audit'=>['operation'=>SBW5_AUDIT_OP,'result_sha256'=>$auditSha],
...
function sbw5_self_test():void{
    ...
    $row=['supplier_namespace'=>'operator_115','operator_id'=>115,'external_hotel_id'=>'900','local_hotel_id'=>100,'source_operation'=>'hotel-match-samo-business-live30-common4-acquire-1971-20260924-v3-wave4','source_result_sha256'=>str_repeat('c',64),'source_edge_sha256'=>str_repeat('d',64),'source_edge_sha256s'=>[str_repeat('d',64)],'input_catalog_ids'=>['10'],'catalog_hotel'=>['id'=>100,'name'=>'X','country_id'=>1,'country_name'=>'Turkey','region_id'=>2,'region_name'=>'R','subregion_id'=>3,'subregion_name'=>'S','category'=>'5','is_active'=>1],'unanimous_catalog_sha256'=>str_repeat('a',64),'anchors'=>[$anchor],'status'=>'writer_ready','anchor_state'=>'canonical_anchor_ok','writer_ready'=>true,'safe_to_write_now'=>false];
    $rows=array_fill(0,22,$row);for($i=0;$i<22;$i++){$rows[$i]['external_hotel_id']=(string)(900+$i);$rows[$i]['local_hotel_id']=100+$i;$rows[$i]['catalog_hotel']['id']=100+$i;$rows[$i]['anchors'][0]['local_hotel_id']=100+$i;}
    $audit=['operation'=>SBW5_AUDIT_OP,'state'=>'completed_read_only_samo_business_common4_current','source_operation'=>$row['source_operation'],'source_result_sha256'=>$row['source_result_sha256'],'captured_single_native_edge_count'=>22,'writer_ready_counts'=>['operator_115'=>22],'writer_ready_total'=>22,'rows'=>$rows,'provider_http_calls'=>0,'tourvisor_calls'=>0,'samo_calls'=>0,'anex_calls'=>0,'andromeda_calls'=>0,'database_writes'=>0,'mapping_writes'=>0,'safe_to_write_now'=>false];
    $m=sbw5_manifest($audit,0,22);sbw5_need($m['total']===22&&count($m['rows'])===22,'self_manifest');
}
if(PHP_SAPI==='cli'&&realpath($_SERVER['SCRIPT_FILENAME']??'')===__FILE__){
    if(in_array('--self-test',$argv??[],true)){sbw5_self_test();echo "MATCH_SAMO_BUSINESS_LIVE30_WAVE4_WRITER_V13_W1_SELFTEST_OK\n";exit;}
    ...
    sbw5_need(is_dir($root)&&basename($root)==='anytoour.ru'&&is_dir($dir)&&basename($dir)===$writerOp&&preg_match('/^hotel-match-samo-business-live30-common4-wave4-writer-1971-20260925-v13-w1$/D',$writerOp)===1&&is_file($auditPath)&&sbw5_sha($auditSha)&&hash_file('sha256',$auditPath)===$auditSha&&preg_match('/^[0-9a-f]{40}$/D',$sourceSha)===1,'runtime_scope');
