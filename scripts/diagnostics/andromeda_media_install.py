"""Publish retained supplier photos and an optional ANEX-only Andromeda search link."""
import base64
import json
import os
...
try {
    ...
    $lock=fopen($private.'/media-update.lock','c');if(!$lock||!flock($lock,LOCK_EX))throw new RuntimeException();
    $release=$private.'/media-'.$request['source_sha'];if(file_exists($release)||!mkdir($release,0700))throw new RuntimeException();
    $expected=['api-andromeda-search3-preview.php'=>'0d57f6efc81e77f99a937952c70400874f0edca7775c6a12ce7e1aea1fb889f6',
    ...
    $files['search-page-v2.php']=str_replace('anex-search3-preview-v1.js?v=365e30fa11dc86d0e7750d16ca52c9d72661120c','anex-search3-preview-v1.js?v='.$request['source_sha'],$page,$count);
...
def main():
    directory=Path(os.environ['RUNNER_TEMP'])/'andromeda-media';directory.mkdir(exist_ok=True)
