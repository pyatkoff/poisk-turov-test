...
OP=os.environ.get('MATCH_CHILD_OPERATION','hotel-match-live942-tv-anex-refresh-1971-20260923-c0-v2')
DAY='2026-09-23'
LIMIT=3000
CALL_CAP=2700
...
def execute(root,opdir,plan_path):
    ...
    offset=int(os.environ.get('MATCH_OFFSET','0'));limit=int(os.environ.get('MATCH_LIMIT','942'))
    if offset<0 or limit<1 or offset>=942 or offset+limit>942:raise RuntimeError('scope_guard')
    scope=plan['rows'][offset:offset+limit]
    groups=chunks(scope);save(opdir/'tv-plan.json',{'operation':OP,'frontier_count':942,'scope_offset':offset,'scope_count':len(scope),'groups':groups,'group_count':len(groups)})
