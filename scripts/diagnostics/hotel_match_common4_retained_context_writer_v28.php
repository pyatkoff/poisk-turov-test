...
const SBW5_AUDIT_OP='hotel-match-common4-retained-context-current-1971-20260925-v27b';
const SBW5_NS=['operator_5'=>5,'operator_115'=>115,'operator_315'=>315,'operator_342'=>342];
const SBW5_WRITER_OP_RE='/^hotel-match-common4-retained-context-writer-1971-20260925-v28-w(?:1b|2b)$/D';
...
function sbw5_manifest(array $audit,int $offset,int $expected):array{
    ...
    sbw5_need((int)($audit['captured_single_native_edge_count']??0)===507&&(int)($audit['writer_ready_total']??0)===494,'audit_wave1_counts');
    sbw5_need(($audit['writer_ready_counts']??null)===['operator_315'=>494],'audit_wave1_namespace');
    ...
    sbw5_need($total===494&&(($offset===0&&$expected===300)||($offset===300&&$expected===194)),'wave1_exact_slice');
    $all=[];$sources=[];$targets=[];
    foreach(($audit['rows']??[]) as $r){
        ...
        sbw5_need(isset(SBW5_NS[$ns])&&SBW5_NS[$ns]===$op&&$ext!==null&&$local>0,'row_identity');sbw5_need($ns==='operator_315'&&$op===315,'wave1_namespace_only');
...
function sbw5_self_test():void{
    ...
    $row=['supplier_namespace'=>'operator_315','operator_id'=>315,'external_hotel_id'=>'900','local_hotel_id'=>100,'source_operation'=>'hotel-match-common4-retained-context-recovery-1971-20260925-v26r','source_result_sha256'=>str_repeat('c',64),'source_edge_sha256'=>str_repeat('d',64),'source_edge_sha256s'=>[str_repeat('d',64)],'input_catalog_ids'=>['10'],'catalog_hotel'=>['id'=>100,'name'=>'X','country_id'=>1,'country_name'=>'Turkey','region_id'=>2,'region_name'=>'R','subregion_id'=>3,'subregion_name'=>'S','category'=>'5','is_active'=>1],'unanimous_catalog_sha256'=>str_repeat('a',64),'anchors'=>[$anchor],'status'=>'writer_ready','anchor_state'=>'canonical_anchor_ok','writer_ready'=>true,'safe_to_write_now'=>false];
    $rows=array_fill(0,494,$row);for($i=0;$i<494;$i++){$rows[$i]['external_hotel_id']=(string)(900+$i);$rows[$i]['local_hotel_id']=100+$i;$rows[$i]['catalog_hotel']['id']=100+$i;$rows[$i]['anchors'][0]['local_hotel_id']=100+$i;}
    $audit=['operation'=>SBW5_AUDIT_OP,'state'=>'completed_read_only_samo_business_common4_current','source_operation'=>$row['source_operation'],'source_result_sha256'=>$row['source_result_sha256'],'captured_single_native_edge_count'=>507,'writer_ready_counts'=>['operator_315'=>494],'writer_ready_total'=>494,'rows'=>$rows,'provider_http_calls'=>0,'tourvisor_calls'=>0,'samo_calls'=>0,'anex_calls'=>0,'andromeda_calls'=>0,'database_writes'=>0,'mapping_writes'=>0,'safe_to_write_now'=>false];
    $m1=sbw5_manifest($audit,0,300);$m2=sbw5_manifest($audit,300,194);sbw5_need($m1['total']===494&&count($m1['rows'])===300&&$m2['total']===494&&count($m2['rows'])===194,'self_manifest');
    sbw5_need(preg_match(SBW5_WRITER_OP_RE,'hotel-match-common4-retained-context-writer-1971-20260925-v28-w1b')===1&&preg_match(SBW5_WRITER_OP_RE,'hotel-match-common4-retained-context-writer-1971-20260925-v28-w2b')===1&&preg_match(SBW5_WRITER_OP_RE,'hotel-match-common4-retained-context-writer-1971-20260925-v28-w1')===0,'self_writer_operation_guard');
}
if(PHP_SAPI==='cli'&&realpath($_SERVER['SCRIPT_FILENAME']??'')===__FILE__){
    if(in_array('--self-test',$argv??[],true)){sbw5_self_test();echo "MATCH_COMMON4_RETAINED_CONTEXT_WRITER_V28_SELFTEST_OK\n";exit;}
