...
def build(c,p,pr):
    ...
    for x in p['rows']:
        ...
        st=x.get('star_difference') is None or int(x['star_difference'])<=1
    ...
    for e,i in pp:
        ...
        try: sd=abs(int(s.get('starKey'))-int(l.get('category'))) if s.get('starKey') is not None and l.get('category') is not None else None
        except (TypeError,ValueError): sd=None
...
def main():
    ...
    Path(x.output).write_text(json.dumps(build(c,p,pr),ensure_ascii=False,sort_keys=True,indent=2)+'\n')
