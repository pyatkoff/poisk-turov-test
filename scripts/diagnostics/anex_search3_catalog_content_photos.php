...
// One separately reserved photograph request for the explicit review-content cohort.
error_reporting(0);
ob_start();
...
try {
    ...
    $batch = $plan['photos_batch_key'];
    $existing = $pdo->prepare('SELECT method,status,report_json FROM anex_hotel_content_requests WHERE batch_key=?');
    $existing->execute([$batch]);
    $saved = $existing->fetch(PDO::FETCH_ASSOC);
    if ($saved) {
        if ($saved['method'] !== 'Hotels_PHOTOS') throw new RuntimeException();
        ...
        $phase = 'reservation';
        $existing->execute([$plan['batch_key']]);
        $details = $existing->fetch(PDO::FETCH_ASSOC);
        if (!$details || $details['method'] !== 'Hotels_DETAILS' || $details['status'] !== 'completed') throw new RuntimeException();
        $select = $pdo->prepare("SELECT anex_hotel_id,status,content_sha256,payload_json FROM anex_hotel_content
            WHERE batch_key=? AND status IN ('ready','empty') ORDER BY anex_hotel_id");
        $select->execute([$plan['batch_key']]);
        $rows = $select->fetchAll(PDO::FETCH_ASSOC);
        if (count($rows) > $plan['limit'] || count($rows) > 30) throw new RuntimeException();
        ...
        try {
            ...
            $write = $pdo->prepare('UPDATE anex_hotel_content SET payload_json=?,content_sha256=?,status=? WHERE anex_hotel_id=? AND content_sha256 <=> ?');
            foreach ($rows as $row) {
                ...
                $photo = $byId[$id];
                $payload = anytour_anex_content_merge_photos($payload, $photo);
                $payload['photos_source'] = ['method' => 'Hotels_PHOTOS', 'source_sha' => $input['source_sha'],
                    'batch_key' => $batch, 'fetched_at_utc' => gmdate('c'),
                    'availability' => $photo['availability']['photos'], 'returned_photos' => count($photo['content']['photos'])];
                $payload['source_sha256'] = $payload['status'] === 'empty' ? null
                    : hash('sha256', json_encode($payload['content'], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_PRESERVE_ZERO_FRACTION | JSON_THROW_ON_ERROR));
                ...
                $write->execute([$encoded, $payload['source_sha256'], $payload['status'] === 'ok' ? 'ready' : 'empty', $id, $row['content_sha256']]);
                if ($write->rowCount() !== 1) throw new RuntimeException();
                $summary[] = ['id' => $id, 'photos' => count($payload['content']['photos'] ?? []),
                    'status' => $payload['availability']['photos'] ?? 'empty'];
            }
            $report = ['requested' => count($ids), 'returned' => count($data), 'rows' => $summary];
            $finish->execute(['completed', json_encode($report), $batch]);
            $pdo->commit();
            $result = ['status' => 'completed', 'supplier_requests' => $attempts, 'cached' => false, 'report' => $report];
            ...
            $result = ['status' => 'source_error', 'supplier_requests' => $attempts, 'cached' => false, 'report' => $error];
        }
    }
} catch (Throwable $ignored) {
...
$result['batch_key'] = $plan['photos_batch_key'] ?? null;
