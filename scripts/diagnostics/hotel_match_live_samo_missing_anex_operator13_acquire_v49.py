...
def build_plan(router,consumed):
    ...
    if len(consumed)!=90 or not consumed.issubset(all_sigs):raise RuntimeError('consumed_not_router_subset')
    candidates=[b for b in all_candidates if batch_signature(b['context_key'],b['hotel_ids']) not in consumed]
    if len(candidates)!=171:raise RuntimeError('remaining_router_batch_count')
...
def execute(root,opdir,router_path,v41b_dir,v45_dir,v45_result_sha,v45_receipt_sha,source_sha):
    router=load(router_path);consumed=load_consumed(v41b_dir) | load_consumed_v45(v45_dir,v45_result_sha,v45_receipt_sha);plan=build_plan(router,consumed)
...
def selftest():
    ...
    print('MATCH_V49_SELFTEST_OK')

if __name__=='__main__':
    if '--self-test' in sys.argv:selftest();sys.exit(0)
    if '--execute' not in sys.argv:raise SystemExit('disabled')
    root=pathlib.Path(os.environ['ANYTOUR_ROOT']);opdir=pathlib.Path(os.environ['MATCH_OPERATION_DIR']);router=pathlib.Path(os.environ['MATCH_ROUTER_RESULT']);v41b=pathlib.Path(os.environ['MATCH_V41B_DIR']);v45=pathlib.Path(os.environ['MATCH_V45_DIR']);v45_result_sha=os.environ['MATCH_V45_RESULT_SHA'];v45_receipt_sha=os.environ['MATCH_V45_RECEIPT_SHA'];source=os.environ['MATCH_SOURCE_SHA']
    if opdir.name!=OP or not root.is_dir() or not router.is_file() or not v41b.is_dir() or not v45.is_dir() or not re.fullmatch(r'[0-9a-f]{64}',v45_result_sha) or not re.fullmatch(r'[0-9a-f]{64}',v45_receipt_sha) or not re.fullmatch(r'[0-9a-f]{40}',source):raise SystemExit('runtime_scope')
    ...
    out=execute(root,opdir,router,v41b,v45,v45_result_sha,v45_receipt_sha,source)
