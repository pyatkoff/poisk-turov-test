...
const HMC_OP = 'hotel-match-tv-samo-common4-weekly-resume-1971-20260922-v3';
...
function hmc_execute(string $root,string $opDir,string $user,string $pass): array {
    ...
    $starIds=[];foreach([3,4,5] as $star){$id=hmc_star_id($all['STARS'],$star);if(!$id)throw new RuntimeException('samo_star_binding_'.$star);$starIds[$star]=$id;}
    ...
    $plan=[
        ...
        'detail_calls_planned'=>0,'flight_calls_planned'=>0,'calc_calls_planned'=>0,'resumed_from'=>HMC_PREVIOUS_OP,
        'previous_search_id'=>$checkpoint['search_id'],'previous_continue_calls'=>$checkpoint['prior_continue_calls'],
    ];
    ...
    foreach([3,4,5] as $star){
        ...
        $td=$star===3?hmc3_tv_resume($checkpoint,$tvCounter):hmc_tv_drain($tvParams,$tvCounter);
    ...
    return [
        ...
        'resumed_from'=>HMC_PREVIOUS_OP,'previous_search_id'=>$checkpoint['search_id'],'previous_continue_calls'=>$checkpoint['prior_continue_calls'],
...
if(in_array('--self-test',$argv??[],true)){
    ...
    echo "MATCH_TV_SAMO_COMMON4_WEEKLY_RESUME_V3_SELFTEST_OK\n";exit(0);
}
