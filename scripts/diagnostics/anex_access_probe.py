...
def clean_report(report):
    if isinstance(report, dict) and report.get("mode") == "adapter":
        return clean_adapter_report(report)
...
def ssh_probe():
    ...
    if adapter:
        ...
        source = source.rsplit('\nif __name__ == "__main__":', 1)[0]
        source += "\n" + Path(__file__).with_name("anex_adapter_probe.py").read_text(encoding="utf-8")
        php_paths = ["app/integrations/hotel-identities.php", "app/integrations/anex-client.php",
                     "app/integrations/anex-normalizer.php", "app/integrations/anex-search.php",
                     "scripts/diagnostics/anex_adapter_runner.php"]
        source += "\nANEX_PHP_SOURCES = " + repr([(root / path).read_text(encoding="utf-8") for path in php_paths])
        source += "\nANEX_REGISTRY_JSON = " + repr((root / "app/integrations/data/anex-hotel-identities.json").read_text(encoding="utf-8"))
        source += '\nif __name__ == "__main__":\n    sys.exit(main())\n'
