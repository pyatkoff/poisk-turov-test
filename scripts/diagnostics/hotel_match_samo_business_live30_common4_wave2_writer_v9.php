...
function sbw5_manifest(array $audit,int $offset,int $expected):array{
    ...
    sbw5_need((int)($audit['captured_single_native_edge_count']??0)===17&&(int)($audit['writer_ready_total']??0)===17,'audit_wave1_counts');
    sbw5_need(($audit['writer_ready_counts']??null)===['operator_115'=>17],'audit_wave1_namespace');
    ...
    sbw5_need($offset===0&&$expected===17&&$total===17,'wave1_exact_slice');
    $all=[];$sources=[];$targets=[];
...
function sbw5_evidence(array $m,array $target,array $anchors,string $writerOp,string $sourceSha,string $auditSha):array{
    return ['operation_id'=>$writerOp,'rule'=>'samo_business_live30_wave2_exact_operator_native_current_anchor_v9','source_sha'=>$sourceSha,'audit'=>['operation'=>SBW5_AUDIT_OP,'result_sha256'=>$auditSha],
...
if(PHP_SAPI==='cli'&&realpath($_SERVER['SCRIPT_FILENAME']??'')===__FILE__){
    if(in_array('--self-test',$argv??[],true)){sbw5_self_test();echo "MATCH_SAMO_BUSINESS_LIVE30_WAVE2_WRITER_V9_SELFTEST_OK\n";exit;}
    ...
    sbw5_need(is_dir($root)&&basename($root)==='anytoour.ru'&&is_dir($dir)&&basename($dir)===$writerOp&&preg_match('/^hotel-match-samo-business-live30-common4-wave2-writer-1971-20260924-v9-w1$/D',$writerOp)===1&&is_file($auditPath)&&sbw5_sha($auditSha)&&hash_file('sha256',$auditPath)===$auditSha&&preg_match('/^[0-9a-f]{40}$/D',$sourceSha)===1,'runtime_scope');
