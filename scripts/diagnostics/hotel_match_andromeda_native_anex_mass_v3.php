...
const HMAN_OP='hotel-match-andromeda-native-anex-mass-1971-20260918-v3';
...
if(in_array('--self-test',$argv??[],true)){
    ...
    if(hman_name_veto('Hotel Alpha',['123'])['ok'])throw new RuntimeException('numeric_guard');if(hman_tokens('Hotel 123')!==['123'])throw new RuntimeException('numeric_token_string');echo"MATCH_ANDROMEDA_NATIVE_ANEX_MASS_V3_SELFTEST_OK\n";exit;
}
...
try{
    ...
    $db->rollBack();
    if(count($selected)<3)throw new RuntimeException('no_contexts');$skippedNoReplay=array_splice($selected,0,2);
    ...
    $result=['operation'=>HMAN_OP,'state'=>'completed_read_only','source_sha'=>$sourceSha,'provider_access'=>true,'login_calls'=>$loginCalls,'andromeda_calls'=>$calls,'price_rows_seen'=>$providerRows,'current_anex_accepted_unique_local'=>count($localToNative),'target_user_seen_anex_accepted_andromeda_missing'=>count($target),'selected_context_count'=>count($selected),'skipped_no_replay_contexts'=>array_map(fn($x)=>['departure_id'=>$x['departure_id'],'country_id'=>$x['country_id'],'date'=>$x['date'],'nights'=>$x['nights']],$skippedNoReplay),'context_status_counts'=>$ctxCounts,'context_meta'=>$contextMeta,'raw_direct_bridge_rows'=>count($raw),'safe_candidate_count'=>count($safe),'safe_candidates'=>$safe,'hold_count'=>count($holds),'holds'=>array_slice($holds,0,300),'database_writes'=>0,'mapping_writes'=>0,'tourvisor_calls'=>0,'direct_anex_calls'=>0,'no_replay'=>true];
