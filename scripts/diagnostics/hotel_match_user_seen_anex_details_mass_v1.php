...
const HMA_OP='hotel-match-user-seen-anex-details-mass-1971-20260918-v1';
...
$root=realpath((string)getenv('ANYTOUR_ROOT'));$opdir=(string)getenv('MATCH_OPERATION_DIR');$token=trim((string)fgets(STDIN));
if(!$root||$opdir===''||$token==='')throw new RuntimeException('runtime');
...
try{
  ...
  $candidateIds=array_slice($candidateIds,0,HMA_MAX_REQUESTS);$details=[];$calls=0;$stop=null;$home=(string)getenv('HOME');$rateDir=rtrim($home,'/').'/.anytour-anex-rate';
