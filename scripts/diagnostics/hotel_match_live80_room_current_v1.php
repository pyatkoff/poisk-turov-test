...
const M80_OP = 'hotel-match-live80-room-current-1971-20260920-v1';
...
function m80_match_concepts(array $tvRows, array $samoRooms, int $contextSearchId): array {
    ...
    foreach (array_intersect(array_keys($tv), array_keys($sa)) as $key) {
        ...
        $base = [
            ...
            'tv_search_ids' => array_values(array_keys($tv[$key]['search_ids'] ?? [])),
...
if (($argv[1] ?? '') === '--self-test') {
    ...
    m80_need($m['concepts'][0]['room_key']==='deluxe' && $m['concepts'][0]['evidence_class']==='same_search_exact_room_key','self_deluxe');
    m80_need($m['concepts'][1]['room_key']==='family sea view','self_qualifier');
