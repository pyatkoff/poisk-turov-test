...
/** MATCH #1971 v2: deterministic repair of v1 PHP built-in helper collision only. */
...
$new='hotel-match-andromeda-pending-exact-current-1971-20260916-v2';
if(substr_count($src,$old)!==2)throw new RuntimeException('v1_operation_marker_changed');
...
if(str_contains($src,'function extract(')||str_contains($src,$old)||substr_count($src,'field_extract(')!==$n)throw new RuntimeException('v2_patch_incomplete');
if(in_array('--emit',$argv??[],true)){echo $src;exit(0);}
$tmp=sys_get_temp_dir().'/hotel-match-andromeda-pending-exact-v2-'.getmypid().'.php';
