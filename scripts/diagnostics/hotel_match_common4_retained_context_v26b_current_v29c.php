...
const SBLC4C4_OP='hotel-match-common4-retained-context-v26b-current-1971-20260925-v29c';
const SBLC4C4_SOURCE_OP='hotel-match-common4-retained-context-acquire-1971-20260925-v26b';
const SBLC4C4_PLAN_OP='hotel-match-samo-business-live30-common4-plan-1971-20260924-v2';
const SBLC4C4_EXPECTED_EDGES=2898;
const SBLC4C4_EXPECTED_CAPTURED=801;
...
function sb4_source_edges(array $source,string $sourceResultSha):array{
    ...
    sb4_need(($source['state']??'')==='completed_read_only_broad_recovery','source_state');
    sb4_need((int)($source['target_edge_count']??0)===SBLC4C4_EXPECTED_EDGES,'source_edge_count');
    sb4_need((int)($source['context_count']??0)===7&&(int)($source['failed_state_operator_count']??-1)===0,'source_contexts');
    sb4_need((int)($source['provider_http_calls']??-1)===49&&(int)($source['samo_http_calls']??-1)===49,'source_provider_membership');
    sb4_need((int)($source['single_native_source_collision_count']??-1)===0,'source_native_collision');
    sb4_need((int)($source['tourvisor_calls']??-1)===0&&(int)($source['direct_anex_calls']??-1)===0,'source_other_provider_zero');
    sb4_need(($source['edge_state_counts']??null)===['captured_ambiguous_native'=>14,'captured_single_native'=>801,'not_returned_in_context'=>2083],'source_edge_states');
...
function sb4_execute(PDO $db,array $source,string $sourceResultSha,string $sourceSha):array{
    ...
    try{
        ...
        return ['operation'=>SBLC4C4_OP,'state'=>'completed_read_only_samo_business_common4_current','source_sha'=>$sourceSha,'source_operation'=>SBLC4C4_SOURCE_OP,'source_result_sha256'=>$sourceResultSha,'source_state'=>$source['state'],
            'queried_edge_count'=>(int)$source['target_edge_count'],'captured_single_native_edge_count'=>count($edges),'unique_input_identity_count'=>count($input['exact']),'deduped_same_identity_edges'=>count($edges)-count($input['exact']),
...
if(PHP_SAPI==='cli'&&realpath($_SERVER['SCRIPT_FILENAME']??'')===__FILE__){
    if(in_array('--self-test',$argv??[],true)){sb4_self_test();echo "MATCH_COMMON4_RETAINED_CONTEXT_V26B_CURRENT_V29C_SELFTEST_OK\n";exit;}
