...
const SBW5_AUDIT_OP='hotel-match-common4-broad-recovery-current-1971-20260925-v22';
...
function sbw5_manifest(array $audit,int $offset,int $expected):array{
    ...
    sbw5_need((int)($audit['captured_single_native_edge_count']??0)===77&&(int)($audit['writer_ready_total']??0)===68,'audit_wave1_counts');
    sbw5_need(($audit['writer_ready_counts']??null)===['operator_115'=>67,'operator_342'=>1],'audit_wave1_namespace');
    ...
    sbw5_need($total===68&&$offset===0&&$expected===68,'wave1_exact_slice');
    $all=[];$sources=[];$targets=[];
    foreach(($audit['rows']??[]) as $r){
        ...
        sbw5_need(isset(SBW5_NS[$ns])&&SBW5_NS[$ns]===$op&&$ext!==null&&$local>0,'row_identity');sbw5_need(in_array($ns,['operator_115','operator_342'],true)&&in_array($op,[115,342],true),'broad_namespace_only');
...
function sbw5_evidence(array $m,array $target,array $anchors,string $writerOp,string $sourceSha,string $auditSha):array{
    return ['operation_id'=>$writerOp,'rule'=>'common4_broad_recovery_exact_operator_native_current_anchor_v23','source_sha'=>$sourceSha,'audit'=>['operation'=>SBW5_AUDIT_OP,'result_sha256'=>$auditSha],
...
function sbw5_self_test():void{
    ...
    $row=['supplier_namespace'=>'operator_115','operator_id'=>115,'external_hotel_id'=>'900','local_hotel_id'=>100,'source_operation'=>'hotel-match-common4-broad-recovery-acquire-1971-20260925-v21','source_result_sha256'=>str_repeat('c',64),'source_edge_sha256'=>str_repeat('d',64),'source_edge_sha256s'=>[str_repeat('d',64)],'input_catalog_ids'=>['10'],'catalog_hotel'=>['id'=>100,'name'=>'X','country_id'=>1,'country_name'=>'Turkey','region_id'=>2,'region_name'=>'R','subregion_id'=>3,'subregion_name'=>'S','category'=>'5','is_active'=>1],'unanimous_catalog_sha256'=>str_repeat('a',64),'anchors'=>[$anchor],'status'=>'writer_ready','anchor_state'=>'canonical_anchor_ok','writer_ready'=>true,'safe_to_write_now'=>false];
    $rows=array_fill(0,68,$row);for($i=0;$i<68;$i++){$rows[$i]['external_hotel_id']=(string)(900+$i);$rows[$i]['local_hotel_id']=100+$i;$rows[$i]['catalog_hotel']['id']=100+$i;$rows[$i]['anchors'][0]['local_hotel_id']=100+$i;}
    $audit=['operation'=>SBW5_AUDIT_OP,'state'=>'completed_read_only_samo_business_common4_current','source_operation'=>$row['source_operation'],'source_result_sha256'=>$row['source_result_sha256'],'captured_single_native_edge_count'=>77,'writer_ready_counts'=>['operator_115'=>67,'operator_342'=>1],'writer_ready_total'=>68,'rows'=>$rows,'provider_http_calls'=>0,'tourvisor_calls'=>0,'samo_calls'=>0,'anex_calls'=>0,'andromeda_calls'=>0,'database_writes'=>0,'mapping_writes'=>0,'safe_to_write_now'=>false];
    $rows[67]['supplier_namespace']='operator_342';$rows[67]['operator_id']=342;$audit['rows']=$rows;
    $m1=sbw5_manifest($audit,0,68);sbw5_need($m1['total']===68&&count($m1['rows'])===68,'self_manifest');
}
if(PHP_SAPI==='cli'&&realpath($_SERVER['SCRIPT_FILENAME']??'')===__FILE__){
    if(in_array('--self-test',$argv??[],true)){sbw5_self_test();echo "MATCH_COMMON4_BROAD_RECOVERY_WRITER_V23_SELFTEST_OK\n";exit;}
    ...
    sbw5_need(is_dir($root)&&basename($root)==='anytoour.ru'&&is_dir($dir)&&basename($dir)===$writerOp&&preg_match('/^hotel-match-common4-broad-recovery-writer-1971-20260925-v23$/D',$writerOp)===1&&is_file($auditPath)&&sbw5_sha($auditSha)&&hash_file('sha256',$auditPath)===$auditSha&&preg_match('/^[0-9a-f]{40}$/D',$sourceSha)===1,'runtime_scope');
