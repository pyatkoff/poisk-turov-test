...
const SBLC4C4_OP='hotel-match-samo-business-live30-common4-wave1-current-1971-20260924-v7';
const SBLC4C4_SOURCE_OP='hotel-match-samo-business-live30-common4-acquire-v3-wave1-recover-1971-20260924-v1';
const SBLC4C4_PLAN_OP='hotel-match-samo-business-live30-common4-plan-1971-20260924-v2';
const SBLC4C4_EXPECTED_EDGES=1182;
const SBLC4C4_EXPECTED_CAPTURED=193;
...
function sb4_source_edges(array $source,string $sourceResultSha):array{
    ...
    sb4_need(($source['state']??'')==='completed_read_only_wave_recovery','source_state');
    sb4_need(($source['plan_operation']??'')===SBLC4C4_PLAN_OP,'source_plan');
    sb4_need((int)($source['global_group_count']??0)===502&&(int)($source['group_start']??0)===1&&(int)($source['group_end']??0)===100,'source_group_scope');
    sb4_need((int)($source['expected_selected_edges']??0)===SBLC4C4_EXPECTED_EDGES,'source_edge_count');
    sb4_need((int)($source['completed_group_count']??0)===74&&(int)($source['partial_group_count']??0)===26&&(int)($source['untouched_group_count']??-1)===0,'source_group_counts');
    sb4_need(array_key_exists('continuation_start_group',$source)&&$source['continuation_start_group']===null,'source_no_continuation');
    ...
    foreach(['provider_http_calls','tourvisor_calls','samo_calls','anex_calls','andromeda_calls','database_writes','mapping_writes'] as $k)sb4_need((int)($source[$k]??-1)===0,'source_zero_'.$k);
...
function sb4_execute(PDO $db,array $source,string $sourceResultSha,string $sourceSha):array{
    ...
    try{
        ...
        return ['operation'=>SBLC4C4_OP,'state'=>'completed_read_only_samo_business_common4_current','source_sha'=>$sourceSha,'source_operation'=>SBLC4C4_SOURCE_OP,'source_result_sha256'=>$sourceResultSha,'source_state'=>$source['state'],
            'queried_edge_count'=>(int)$source['expected_selected_edges'],'captured_single_native_edge_count'=>count($edges),'unique_input_identity_count'=>count($input['exact']),'deduped_same_identity_edges'=>count($edges)-count($input['exact']),
...
if(PHP_SAPI==='cli'&&realpath($_SERVER['SCRIPT_FILENAME']??'')===__FILE__){
    if(in_array('--self-test',$argv??[],true)){sb4_self_test();echo "MATCH_SAMO_BUSINESS_LIVE30_WAVE1_CURRENT_V7_SELFTEST_OK\n";exit;}
