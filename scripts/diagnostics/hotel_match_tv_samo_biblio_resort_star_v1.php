...
const HM_OP = 'hotel-match-tv-samo-biblio-resort-star-1971-20260918-v3';
const HM_HISTORICAL_TV_OPERATOR = 18;
const HM_HISTORICAL_SAMO_OPERATOR = 115;
...
function hm_url(string $v): ?string {
    ...
    if(isset($p['query'])){parse_str((string)$p['query'],$q);foreach(array_keys($q) as $k)if(preg_match('/token|auth|pass|secret|session|sid|cookie|signature|api[_-]?key/i',(string)$k))return null;}
    return $v;
}
...
function hm_tv_hotels(array $rows,int $star,string $date,int $tvOperatorId): array {
    $out=[];
    foreach($rows as $h){
        ...
        foreach((array)($h['tours']??[]) as $t){
            if(!is_array($t))continue;$op=hm_id($t['operator']??null);if($op!==null&&$op!==$tvOperatorId)continue;
...
function hm_samo_row(array $r,int $star,string $date,string $samoOperatorId): ?array {
    if((string)($r['operatorKey']??'')!==$samoOperatorId)return null;
...
function hm_execute(string $root,string $opDir,string $user,string $pass): array {
    ...
    $op=hm_dict($tvOps,['Библио-Глобус','Библио Глобус','Biblio Globus','Biblio-Globus','BiblioGlobus']);if(count($op)!==1)throw new RuntimeException('tv_operator_binding');$tvOperatorId=(int)$op[0]['id'];
    ...
    $sop=hm_dict($all['OPERATORS'],['Библио-Глобус','Библио Глобус','Biblio Globus','Biblio-Globus','BiblioGlobus']);if(count($sop)!==1)throw new RuntimeException('samo_operator_binding');$samoOperatorId=(string)$sop[0]['id'];
    ...
    foreach([3,4,5]as$star){
        $tvParams=['departureId'=>HM_DEPARTURE,'countryId'=>HM_TV_COUNTRY,'dateFrom'=>$date,'dateTo'=>$date,'nightsFrom'=>HM_NIGHTS,'nightsTo'=>HM_NIGHTS,'adults'=>HM_ADULTS,'currency'=>'RUB','onlyCharter'=>false,'regionIds'=>[$region[0]['id']],'operatorIds'=>[$tvOperatorId],'hotelCategory'=>$star];
        $td=hm_tv_drain($tvParams,$tvCounter);$tv=hm_tv_hotels($td['rows'],$star,$date,$tvOperatorId);
        ...
        for($page=1;$page<=HM_MAX_SAMO_PAGES;$page++){
            ...
            $params=['TOWNFROMINC'=>HM_DEPARTURE,'STATEINC'=>HM_SAMO_STATE,'CHECKIN_BEG'=>$ymd,'CHECKIN_END'=>$ymd,'NIGHTS_FROM'=>HM_NIGHTS,'NIGHTS_TILL'=>HM_NIGHTS,'ADULT'=>HM_ADULTS,'CHILD'=>0,'CURRENCYINC'=>643,'STARS'=>(string)$starIds[$star],'OPERATORS'=>$samoOperatorId,'TOWNTOINC'=>(string)$town[0]['id'],'PACKETTYPE'=>0,'PAGE'=>$page,'GROUP_BY'=>32];
            ...
            foreach($reply['PRICES']as$r)if(is_array($r)&&($x=hm_samo_row($r,$star,$date,$samoOperatorId))!==null)$allSamo[$x['hotel_id']]=$x;
        ...
        $stars[(string)$star]=['tourvisor'=>['search_id'=>$td['search_id'],'fully_drained'=>$td['fully_drained'],'continue_rounds'=>$td['rounds'],'unique_hotels'=>count($tv),'hotels'=>array_values($tv)],'andromeda'=>['pages_count'=>$pagesCount,'pages_drained'=>$pagesCount===0?0:count($pagesMeta),'page_meta'=>$pagesMeta,'unique_hotels'=>count($allSamo),'hotels'=>array_values($allSamo)],'unique_exact_name_matches'=>count($matches),'matches'=>$matches];
    }
    return ['operation'=>HM_OP,'state'=>'completed_read_only','route'=>['departure'=>'Moscow','country'=>'Turkey','resort'=>'Antalya','nights'=>HM_NIGHTS,'adults'=>HM_ADULTS,'tv_operator'=>$tvOperatorId,'tv_operator_name'=>$op[0]['name'],'samo_operator'=>(int)$samoOperatorId,'samo_operator_name'=>$sop[0]['name'],'historical_tv_operator'=>HM_HISTORICAL_TV_OPERATOR,'historical_samo_operator'=>HM_HISTORICAL_SAMO_OPERATOR],'selected_date'=>$date,'common_date_count'=>count($common),'common_dates_sample'=>array_slice($common,0,15),'tv_region'=>$region[0],'samo_townto'=>$town[0],'samo_star_ids'=>$starIds,'tv_calls'=>$tvCounter['calls'],'andromeda_calls'=>$andromedaCalls,'detail_calls'=>$detailBudget,'stars'=>$stars,'database_writes'=>0,'mapping_writes'=>0,'booking_calls'=>0,'lead_writes'=>0,'no_replay'=>true];
}
...
if(PHP_SAPI==='cli'&&realpath($_SERVER['SCRIPT_FILENAME']??'')===__FILE__){
    ...
    catch(Throwable$e){$reason=preg_match('/^[A-Za-z0-9_.:-]{1,120}$/D',$e->getMessage())?$e->getMessage():'sanitized_failure';$fail=['operation'=>HM_OP,'state'=>'terminal_failed_no_replay','reason'=>$reason,'database_writes'=>0,'mapping_writes'=>0,'booking_calls'=>0,'lead_writes'=>0,'no_replay'=>true];$sha=hm_write($opDir.'/result.json',$fail);hm_write($opDir.'/receipt.json',['operation'=>HM_OP,'state'=>$fail['state'],'result_sha256'=>$sha,'readback_verified'=>true,'provider_accessed'=>true,'database_writes'=>0,'mapping_writes'=>0,'no_replay'=>true]);fwrite(STDERR,$reason."\n");exit(2);}
}
