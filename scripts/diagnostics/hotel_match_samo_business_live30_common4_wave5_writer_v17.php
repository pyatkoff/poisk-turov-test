...
const SBW5_AUDIT_OP='hotel-match-samo-business-live30-common4-wave5-current-1971-20260925-v16c';
...
function sbw5_manifest(array $audit,int $offset,int $expected):array{
    ...
    sbw5_need((int)($audit['captured_single_native_edge_count']??0)===508&&(int)($audit['writer_ready_total']??0)===499,'audit_wave1_counts');
    sbw5_need(($audit['writer_ready_counts']??null)===['operator_342'=>499],'audit_wave1_namespace');
    ...
    sbw5_need($total===499&&(($offset===0&&$expected===300)||($offset===300&&$expected===199)),'wave1_exact_slice');
    $all=[];$sources=[];$targets=[];
...
function sbw5_self_test():void{
    ...
    $row=['supplier_namespace'=>'operator_342','operator_id'=>342,'external_hotel_id'=>'900','local_hotel_id'=>100,'source_operation'=>'hotel-match-samo-business-live30-common4-wave5-recovery-acquire-1971-20260925-v15','source_result_sha256'=>str_repeat('c',64),'source_edge_sha256'=>str_repeat('d',64),'source_edge_sha256s'=>[str_repeat('d',64)],'input_catalog_ids'=>['10'],'catalog_hotel'=>['id'=>100,'name'=>'X','country_id'=>1,'country_name'=>'Turkey','region_id'=>2,'region_name'=>'R','subregion_id'=>3,'subregion_name'=>'S','category'=>'5','is_active'=>1],'unanimous_catalog_sha256'=>str_repeat('a',64),'anchors'=>[$anchor],'status'=>'writer_ready','anchor_state'=>'canonical_anchor_ok','writer_ready'=>true,'safe_to_write_now'=>false];
    $rows=array_fill(0,499,$row);for($i=0;$i<499;$i++){$rows[$i]['external_hotel_id']=(string)(900+$i);$rows[$i]['local_hotel_id']=100+$i;$rows[$i]['catalog_hotel']['id']=100+$i;$rows[$i]['anchors'][0]['local_hotel_id']=100+$i;}
    $audit=['operation'=>SBW5_AUDIT_OP,'state'=>'completed_read_only_samo_business_common4_current','source_operation'=>$row['source_operation'],'source_result_sha256'=>$row['source_result_sha256'],'captured_single_native_edge_count'=>508,'writer_ready_counts'=>['operator_342'=>499],'writer_ready_total'=>499,'rows'=>$rows,'provider_http_calls'=>0,'tourvisor_calls'=>0,'samo_calls'=>0,'anex_calls'=>0,'andromeda_calls'=>0,'database_writes'=>0,'mapping_writes'=>0,'safe_to_write_now'=>false];
    $m1=sbw5_manifest($audit,0,300);$m2=sbw5_manifest($audit,300,199);sbw5_need($m1['total']===499&&count($m1['rows'])===300&&$m2['total']===499&&count($m2['rows'])===199,'self_manifest');
}
if(PHP_SAPI==='cli'&&realpath($_SERVER['SCRIPT_FILENAME']??'')===__FILE__){
    if(in_array('--self-test',$argv??[],true)){sbw5_self_test();echo "MATCH_SAMO_BUSINESS_LIVE30_WAVE5_WRITER_V17_SELFTEST_OK\n";exit;}
    ...
    sbw5_need(is_dir($root)&&basename($root)==='anytoour.ru'&&is_dir($dir)&&basename($dir)===$writerOp&&preg_match('/^hotel-match-samo-business-live30-common4-wave5-writer-1971-20260925-v17-w[12]b$/D',$writerOp)===1&&is_file($auditPath)&&sbw5_sha($auditSha)&&hash_file('sha256',$auditPath)===$auditSha&&preg_match('/^[0-9a-f]{40}$/D',$sourceSha)===1,'runtime_scope');
