...
OP='hotel-match-reverse110-live-anex-continuation-1971-20260921-v2';DAY='2026-09-21';LIMIT=3000;CALL_CAP=600;BASE='https://api.tourvisor.ru/search/api/v1';BODY_LIMIT=16*1024*1024
...
class P:
 def __init__(self,root,opdir):
  q=pathlib.Path(os.environ['HOME'])/'.anytour-match/provider-quotas';q.mkdir(parents=True,exist_ok=True);self.day=q/('tourvisor-test-'+DAY+'.json');self.op=q/('tourvisor-'+OP+'.json');self.dir=opdir;self.used=0;self.last=None;self.counts=collections.Counter()
...
def selftest():print('REVERSE110_LIVE_V2_SELFTEST_OK')
