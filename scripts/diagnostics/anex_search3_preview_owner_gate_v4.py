#!/usr/bin/env python3
"""Pure authorization gate for one no-replay ANEX/Andromeda Search3 preview v4 publication/canary."""
...
COMMAND = "/publish-anex-search3-preview-2530-v4"
...
def main() -> int:
    ...
    try:
        ...
        print("ANEX_PREVIEW_V4_CONTROL_DENIED invalid_event", file=sys.stderr)
        return 1
    allowed, reason = authorize(event, dict(os.environ))
    if not allowed:
        print("ANEX_PREVIEW_V4_CONTROL_DENIED " + reason, file=sys.stderr)
        return 1
    print("ANEX_PREVIEW_V4_CONTROL_AUTHORIZED")
    return 0
