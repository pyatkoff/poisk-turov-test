...
try {
    ...
    $phase='preflight';$config=require $target.'/.andromeda-private.php';
    ...
    $countries=[];foreach($catalogPaths as $path){
        $saved=json_decode(file_get_contents($path),true,32,JSON_THROW_ON_ERROR);$state=(string)($saved['all']['params']['STATEINC']??'');
        $saved['local_country_id']=(int)($saved['local_country_id']??1);$countryName->execute([$saved['local_country_id']]);$saved['local_country_name']=(string)$countryName->fetchColumn();
        if($state===''||$saved['local_country_name']===''||isset($countries[$state]))throw new RuntimeException('COUNTRY_CONTEXT_INVALID');$countries[$state]=$saved;
    }
    ...
    $pages=0;$unique=0;$inserted=0;$mappedFiltered=0;$skipped=0;$planned=[];
    ...
    foreach($paths as $path){
        ...
        $rows=AnyTourAndromedaHotelObservations::rows($page,$country);$unique+=count($rows);$planned[]=[$page,$country];++$pages;
    }
    $phase='schema';AnyTourAndromedaHotelObservations::install($pdo);
    $existing=(int)$pdo->query('SELECT COUNT(*) FROM andromeda_search_hotel_observations')->fetchColumn();
    $resolvedBefore=(int)$pdo->query("SELECT COUNT(*) FROM andromeda_search_hotel_observations o JOIN andromeda_hotel_identities i ON i.supplier_namespace=o.supplier_namespace AND i.external_hotel_id=o.external_hotel_id AND i.decision_status='accepted'")->fetchColumn();
    if($resolvedBefore!==0)throw new RuntimeException('OBSERVATION_IDENTITY_CONFLICT');
    ...
    $phase='backfill';foreach($planned as [$page,$country]){$stored=AnyTourAndromedaHotelObservations::record($pdo,$page,$country);$inserted+=$stored['inserted'];}
    ...
    if($rows!==$existing+$inserted||$resolved!==0)throw new RuntimeException('OBSERVATION_READBACK_INVALID');
    $result=['status'=>'installed','source_sha'=>$request['source_sha'],'table'=>'andromeda_search_hotel_observations',
        'saved_pages_scanned'=>$pages,'unresolved_hotel_events'=>$rows,'preexisting_events'=>$existing,'duplicate_events'=>$unique-$inserted,
    ...
    $result=['status'=>'failed','phase'=>$phase,'retry'=>false,'rollback_attempted'=>count($written)>0,
        'failure_class'=>get_class($e),'failure_sha256'=>hash('sha256',get_class($e).':'.$e->getMessage())];
}
