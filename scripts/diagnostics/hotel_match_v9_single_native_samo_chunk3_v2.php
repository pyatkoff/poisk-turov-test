...
const OP='hotel-match-v9-single-native-samo-chunk3-1971-20260921-v2';
...
if(($argv[1]??'')==='--self-test'){
    ...
    echo "V9_SAMO_CHUNK3_V2_SELFTEST_OK\n"; exit;
}
...
foreach([[$vp,V9_SHA],[$p1,CH1_SHA],[$p2,CH2_SHA]] as [$p,$sha]){
...
$skip=[];
foreach(array_merge($c1['selected_hotel_ids']??[],$c2['selected_hotel_ids']??[]) as $id) $skip[(int)$id]=true;
need(count($skip)===166,'skip166');
$country=[];
...
need((int)array_key_first($countries)===4,'next_country4');
$hotelSet=[]; foreach($remaining as $e) if($e['country_id']===4) $hotelSet[$e['tv_hotel_id']]=true;
$ids=array_keys($hotelSet); sort($ids,SORT_NUMERIC); need(count($ids)===49,'remaining_country4_49');
$selectedSet=array_fill_keys($ids,true);
$edges=array_values(array_filter($remaining,fn($e)=>$e['country_id']===4 && isset($selectedSet[$e['tv_hotel_id']])));
...
try {
    ...
    $result=[
        ...
        'prior_selected_unique_hotels'=>166,'selected_country_id'=>4,'selected_unique_hotels'=>count($ids),
        'selected_edges'=>count($edges),'selected_hotel_ids'=>$ids,
