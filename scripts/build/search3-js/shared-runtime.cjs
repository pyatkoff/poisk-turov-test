...
const { compactBindings, compactBindingTokens } = require('./compact.cjs');
const root = path.resolve(__dirname, '../../..');
const sha256 = value => crypto.createHash('sha256').update(value).digest('hex');

// Explicit reviewed files. Legacy endpoint stays unchanged; the two core modules
// retain original syntax tokens instead of passing through the JS printer.
const tokenFiles = new Set(['tour-controller-v4.js', 'catalogs-v2.js']);
const files = [
  ...
  'search-lifecycle-v6.js', 'passive-price-observer-v1.js', ...tokenFiles
];
...
(async () => {
  ...
  for (const name of files) {
    ...
    const code = await (tokenFiles.has(name) ? compactBindingTokens : compactBindings)(source);
    // A failed syntax proof aborts the build.
