...
const { normalizeSharedSource } = require('./shared-source-normalization.cjs');
const root = path.resolve(__dirname, '../../..');
const sha256 = value => crypto.createHash('sha256').update(value).digest('hex');

// Explicit reviewed files. Protected controller/catalog sources remain byte-identical;
// their two hash-locked printer normalizations affect only the Search3 representation.
// The legacy endpoint stays unchanged.
const files = [
  ...
  'tour-controller-v4.js', 'catalogs-v2.js'
];
...
(async () => {
  ...
  for (const name of files) {
    ...
    const normalized = normalizeSharedSource(name, source);
    const code = await compactBindings(normalized); // A failed syntax proof aborts the build.
