"""Create-only transaction for the owner-authorized v17 preview.
...
def verify(root, q):
    ...
    need(m.get('schema_version') == 1 and m.get('target') == 'search3-v17-preview'
    ...
    need(m.get('derived_from') == {'artifact_id': q['artifact_id'], 'build_run': q['build_run'],
                                 'ZIP_sha256': q['source_ZIP_sha256'], 'transform': 'v17-route-v1'}, 'derivation_identity')
    expected = {}
    for f in m['files']:
    ...
    need('#^(/_preview/search3-v17-candidate)(?:/|$)#' in (root / 'payload/site-path-v1.php').read_text(), 'wrong_route_helper')
    return expected
...
class Site:
    def __init__(self, root):
        ...
        self.owner = self.parent / '.search3-v17-owner'
        need(not self.target.is_symlink() and not self.owner.is_symlink(), 'target_or_owner_link')

    @contextmanager
    def lock(self):
        p = self.parent / '.search3-v17-lock'
        need(not (self.parent / '.search3-site-lock').exists() and not (self.parent / '.search3-local-lock').exists(), 'existing_preview_busy')
        p.mkdir(mode=0o700)
        try:
            yield
        finally:
    ...
    def protected(self):
        ...
        for name in ('search3-site-candidate', 'search3-candidate', 'search3-local-candidate'):
            p = self.parent / name
    ...
    def binding(self, q, remove=False):
        ...
        need(re.fullmatch(r'search3-v17-bind-[1-9][0-9]*-[0-9a-f]{24}\.txt', name)
    ...
    def receipt(self, q):
        return self.parent / ('.search3-v17-receipt-' + str(q['deploy_run']))

    def activate(self, q):
        validate(q); archive = Path(q['archive'])
        need(re.fullmatch(r'/tmp/search3-v17\.[A-Za-z0-9_-]+\.tar\.gz', str(archive)), 'upload_path')
        need(archive.is_file() and not archive.is_symlink() and digest(archive.read_bytes()) == q['archive_sha256'], 'upload_digest')
        stage = self.parent / ('.search3-v17-stage-' + str(q['deploy_run']))
        with self.lock():
...
def main():
    ...
    if action == 'snapshot':
        ...
        fd, name = tempfile.mkstemp(prefix='search3-v17.', suffix='.tar.gz', dir='/tmp'); os.close(fd)
        result = {'archive': name}
