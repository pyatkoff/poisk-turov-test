...
INSTALL_CMD=re.compile(r'^/install-search3-root-gateway ([a-f0-9]{40}) ([a-f0-9]{40}) ([a-f0-9]{40}) ([a-f0-9]{64})$')
INSPECT_CMD='/inspect-search3-root-gateway'
...
def event():
 e=json.loads(pathlib.Path(os.environ['GITHUB_EVENT_PATH']).read_text());body=e['comment']['body'].strip();need(e['issue']['number']==ISSUE and e['comment']['user']['id']==226193297 and e['comment']['author_association']=='OWNER','owner')
 if body==INSPECT_CMD:return 'inspect',()
 m=INSTALL_CMD.fullmatch(body);need(m,'command');return 'install',m.groups()
def api(path):
...
def ssh_context(work):
 ...
 opts=['-i',str(key),'-o','StrictHostKeyChecking=yes','-o','UserKnownHostsFile='+str(known),'-o','GlobalKnownHostsFile=/dev/null','-o','BatchMode=yes','-o','ConnectTimeout=15'];return key,known,opts,user+'@'+host
def prepare():
 mode,args=event();need(mode=='install','install_command');src,rel,bl,prev=args;need(api('/git/ref/heads/release/search3-production-ready-v1')['object']['sha']==rel,'release_moved');b,got=source(src);need(got==bl,'source_blob');need(b'search_results_limit' in b and b'strict_int($value, 1, 5000' in b,'limit_contract')
...
def inspect():
 mode,args=event();need(mode=='inspect' and not args,'inspect_command');work=pathlib.Path(os.environ['RUNNER_TEMP'])/'search3-root-gateway-inspect';work.mkdir(parents=True);key=known=None
 try:
  key,known,opts,remote=ssh_context(work);out=run(['ssh',*opts,remote,'set -e; f="$HOME/www/anytoour.ru/api-v2.php"; test -f "$f" && ! test -L "$f"; php -l "$f" >/dev/null; sha256sum "$f" | cut -d" " -f1']).decode().strip();need(re.fullmatch(r'[a-f0-9]{64}',out),'live_hash');e={'status':'inspected_read_only','sha256':out,'supplier_calls':0,'server_writes':0};(work/'evidence.json').write_text(json.dumps(e,sort_keys=True));print(json.dumps(e))
 finally:
  if key:key.unlink(missing_ok=True)
  if known:known.unlink(missing_ok=True)
def install():
 mode,args=event();need(mode=='install','install_command');src,rel,bl,prev=args;need(api('/git/ref/heads/release/search3-production-ready-v1')['object']['sha']==rel,'release_moved');meta=api('/actions/artifacts/'+os.environ['READY_ARTIFACT_ID']);need(not meta['expired'] and meta['workflow_run']['id']==int(os.environ['GITHUB_RUN_ID']),'artifact_identity')
 ...
 work=pathlib.Path(os.environ['RUNNER_TEMP'])/'search3-root-gateway-install';work.mkdir();f=work/'api-v2.php';f.write_bytes(b);key=known=None;backup='$HOME/www/anytoour.ru/.api-v2.php.search3-'+os.environ['GITHUB_RUN_ID']+'.bak';e={'source':src,'release':rel,'status':'checked_not_installed','supplier_calls':0,'real_leads':0};attempt=False
 try:
  key,known,opts,remote=ssh_context(work);before=run(['ssh',*opts,remote,'sha256sum "$HOME/www/anytoour.ru/api-v2.php" | cut -d" " -f1']).decode().strip();need(before==prev,'predecessor_drift');run(['scp',*opts,str(f),remote+':/tmp/search3-api-v2-'+os.environ['GITHUB_RUN_ID']+'.php']);script='set -euo pipefail; t="$HOME/www/anytoour.ru/api-v2.php"; n="/tmp/search3-api-v2-'+os.environ['GITHUB_RUN_ID']+'.php"; b="'+backup+'"; php -l "$n" >/dev/null; cp "$t" "$b"; chmod 600 "$b"; mv "$n" "$t"; chmod 644 "$t"; php -l "$t" >/dev/null; sha256sum "$t" | cut -d" " -f1';attempt=True;after=run(['ssh',*opts,remote,script]).decode().strip();need(after==sha(b),'installed_hash')
  ...
  (work/'evidence.json').write_text(json.dumps(e,sort_keys=True))
  if key:key.unlink(missing_ok=True)
  if known:known.unlink(missing_ok=True)
 print(json.dumps(e))
