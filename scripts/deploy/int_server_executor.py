...
def parse_command(body: str) -> dict:
    ...
    if mode == 'match-common4-continuation-remainder':
        ...
        return {'source_sha': source, 'mode': mode, 'operation_id': operation, 'limit': limit}
...
def match_common4_continuation_attempted():
    match_root=home/'.anytoour-match/operations'
    plan_operation='hotel-match-live30-common4-continuation-plan-1971-20260923-v9'
    plan_path=match_root/plan_operation/'result.json'
    receipt_path=match_root/plan_operation/'receipt.json'
    if not safe_file(plan_path,32*1024*1024) or not safe_file(receipt_path,1024*1024):
        fail('match_common4_remainder_plan_missing')
    plan=safe_json(plan_path,32*1024*1024);receipt=safe_json(receipt_path,1024*1024)
    digest=hashlib.sha256(plan_path.read_bytes()).hexdigest()
    if receipt.get('result_sha256')!=digest: fail('match_common4_remainder_plan_hash')
    rows=plan.get('rows')
    if (plan.get('state')!='live30_common4_continuation_ready' or plan.get('acquisition_target_count')!=1349
            or not isinstance(rows,list) or len(rows)!=1349
            or plan.get('acquisition_target_id_sha256')!='ce464a7b71dc72cf425197c73c1b8a4770adaf586ee05d169f4c67f8fc43ccca'
...
def execute(command: dict, source_root: Path) -> dict:
    ...
    try:
        run = subprocess.run(
            ...
            capture_output=True, timeout=(1400 if command['mode'] in ('match-common4-continuation-resume-day','match-common4-continuation-remainder') else 1000)
        )
