"""Two-phase trusted-main provisioning of the fixed, private v17 draft preview.

The existing public publishers and their release-equality requirements are unchanged.
Only PR3214 with its bounded presentation diff is eligible for this private target.
"""
...
import tarfile
import zipfile
from search3_private_preview_remote import ROUTE, INVARIANTS, digest, inventory, json_bytes, need, verify, PUBLIC_INDEX, PUBLIC_HTACCESS

PREFIX = '/create-search3-private-preview '
...
def remote_program():
    """Only checked-out trusted-main modules go to SSH; never artifact code."""
    base = Path(__file__).parent
    sources = {name: (base / (name + '.py')).read_text() for name in
        ('search3_local_preview_remote', 'search3_private_preview_remote')}
    return ("import sys,types,json\nmodules=json.loads(" + repr(json.dumps(sources)) + ")\n"
        "for name,source in modules.items():\n"
        " module=types.ModuleType(name);sys.modules[name]=module\n"
        " exec(compile(source,name+'.py','exec'),module.__dict__)\n"
        "sys.modules['search3_private_preview_remote'].main()\n").encode()


def verify_private_provenance(api, q, control_sha):
    """A separate, fixed draft target; no changes to public release authorization."""
    from search3_preview_publish import BUILD
    ...
    pr = api.get('/pulls/3214')
    need(pr.get('state') == 'open' and not pr.get('merged') and pr.get('user', {}).get('id') == 226193297
         and pr.get('head', {}).get('sha') == q['source_sha']
         and pr['head'].get('ref') == 'feat/search3-v17-entry-migration-20260920'
         and pr['head'].get('repo', {}).get('full_name') == REPO
         and pr.get('base', {}).get('ref') == RELEASE, 'private_source_PR')
    comparison = api.get('/compare/' + q['release_sha'] + '...' + q['source_sha'])
    allowed = {'v2/index.php', 'src/search3/behavior/search-form.js', 'src/search3/styles/entry-native-controls.css',
        'tests/search3-entry-owner-browser.cjs', 'tests/search3-entry-geometry-browser.cjs',
        'tests/search3_production_presentation_test.py', 'tests/search3-current-results-browser.cjs',
        'v2/search3-entry-v1.css', 'v2/search3-results-filters-v1.js', 'docs/project/search3-production-import.json'}
    changed = comparison.get('files', [])
    need(comparison.get('status') == 'ahead' and 0 < len(changed) <= len(allowed)
         and all(f.get('filename') in allowed and f.get('status') == 'modified' for f in changed), 'private_bounded_source_diff')
    source_tree = api.get('/git/commits/' + q['source_sha'])['tree']['sha']
...
def derive(source, output, request, zip_digest):
    ...
    new = '#^(/_preview/search3-v17-candidate)(?:/|$)#'
    ...
    gate = Path(__file__).with_name('search3_private_preview_gate.php').read_bytes()
    (output / 'control/gate.php').write_bytes(gate)
    (output / 'control/public-index.php').write_bytes(PUBLIC_INDEX)
    (output / 'control/public.htaccess').write_bytes(PUBLIC_HTACCESS)
    ...
    m.update(target='search3-private-preview', route=ROUTE, invariants=INVARIANTS,
        derived_from={'artifact_id': request['artifact_id'], 'build_run': request['build_run'],
                      'ZIP_sha256': zip_digest, 'transform': 'private-route-v1'})
    ...
    with tarfile.open(fileobj=raw, mode='w', format=tarfile.USTAR_FORMAT) as t:
        for name in ('control/manifest.json', 'control/payload.sha256', 'control/gate.php', 'control/public-index.php', 'control/public.htaccess', *('payload/' + n for n in files)):
    ...
    q = {**request, 'source_ZIP_sha256': zip_digest, 'archive_sha256': digest(archive),
         ...
         'payload_sha256': digest((output / 'control/payload.sha256').read_bytes()), 'file_count': len(files), 'gate_sha256': digest(gate)}
    verify(output, q)
    return q, archive
...
def authorized():
    from search3_preview_publish import Github
    ...
    tree, zip_hash = verify_private_provenance(api, q, env['GITHUB_SHA'])
...
def prepare():
    ...
    work = Path(os.environ['RUNNER_TEMP']) / 'search3-private-prepare'; work.mkdir(mode=0o700)
    ...
    (retained / 'private-preview.tar.gz').write_bytes(archive)
...
def live_checks(files):
    ...
    paths = ('', 'poisk-turov/', 'country/', 'bundle-v1.php?name=search3',
        'search3-entry-v1.css', 'search3-results-filters-v1.js', 'search-page-v2.php',
        'data/search3-local-results-read-v1.php', 'config.php')
    for route in paths:
        status, body = http(ROUTE + route)
        need(status == 401 and 'Закрытый просмотр Search3' in body
             and 'id="tourSearch"' not in body, 'anonymous_route_not_private:' + route)
    status, body = http(ROUTE + 'preview-lead-disabled.php', b'{"synthetic":true}')
    need(status == 401 and 'id="tourSearch"' not in body, 'anonymous_lead_not_private')
    need(http('/')[0] == 200 and http('/poisk-turov/')[0] == 200, 'production_pages_unhealthy')
    return {'anonymous_routes_denied': len(paths) + 1, 'payload_public': False,
            'owner_login_required': True, 'supplier_searches': 0, 'real_leads': 0}


def publish():
    from search3_preview_publish import prepare_zip, command, http
    q, api, tree, zip_hash = authorized()
    work = Path(os.environ['RUNNER_TEMP']) / 'search3-private-publish'; work.mkdir(mode=0o700)
    original = work / 'original'
    prepare_zip(api.download(q['artifact_id']), q, tree, zip_hash, original)
    ...
    need(not meta.get('expired') and meta.get('workflow_run', {}).get('id') == q['deploy_run']
         ...
         and meta.get('name') == 'search3-private-ready-' + str(q['deploy_run']), 'ready_artifact_identity')
    ...
    with zipfile.ZipFile(io.BytesIO(packed)) as z:
        need(len(z.infolist()) == 2 and set(z.namelist()) == {'private-preview.tar.gz', 'request.json'}, 'ready_ZIP_inventory')
        need(sum(x.file_size for x in z.infolist()) <= 64 * 1024 * 1024, 'ready_ZIP_limit')
        need(z.read('private-preview.tar.gz') == archive and json.loads(z.read('request.json')) == expected_q, 'derived_artifact_drift')
    q = expected_q; (work / 'private-preview.tar.gz').write_bytes(archive)
    ...
    try:
        ...
        need(all(before[k] is None for k in ('target', 'private', 'owner')), 'create_only_target_exists')
        verify_private_provenance(api, q, os.environ['GITHUB_SHA'])
        ...
        need(re.fullmatch(r'/tmp/search3-private\.[A-Za-z0-9_-]+\.tar\.gz', q['archive']), 'remote_upload_path')
        command(['scp', *opts, str(work / 'private-preview.tar.gz'), user + '@' + host + ':' + q['archive']])
        ...
        evidence.update(status='published_private', production_unchanged=True, existing_preview_unchanged=True)
