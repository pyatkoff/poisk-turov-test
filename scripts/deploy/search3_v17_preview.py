"""Two-phase, trusted-main provisioning of the separate v17 preview.
...
PREFIX = '/create-search3-v17-preview '
...
def derive(source, output, request, zip_digest):
    ...
    m.update(target='search3-v17-preview', route=ROUTE, invariants=INVARIANTS,
        derived_from={'artifact_id': request['artifact_id'], 'build_run': request['build_run'],
                      'ZIP_sha256': zip_digest, 'transform': 'v17-route-v1'})
...
def prepare():
    ...
    work = Path(os.environ['RUNNER_TEMP']) / 'search3-v17-prepare'; work.mkdir(mode=0o700)
    ...
    (retained / 'v17-preview.tar.gz').write_bytes(archive)
...
def publish():
    ...
    q, api, tree, zip_hash = authorized()
    work = Path(os.environ['RUNNER_TEMP']) / 'search3-v17-publish'; work.mkdir(mode=0o700)
    original = work / 'original'
    prepare_zip(api.download(q['artifact_id']), q, tree, zip_hash, original)
    ...
    need(not meta.get('expired') and meta.get('workflow_run', {}).get('id') == q['deploy_run']
         ...
         and meta.get('name') == 'search3-v17-ready-' + str(q['deploy_run']), 'ready_artifact_identity')
    ...
    with zipfile.ZipFile(io.BytesIO(packed)) as z:
        need(len(z.infolist()) == 2 and set(z.namelist()) == {'v17-preview.tar.gz', 'request.json'}, 'ready_ZIP_inventory')
        need(sum(x.file_size for x in z.infolist()) <= 64 * 1024 * 1024, 'ready_ZIP_limit')
        need(z.read('v17-preview.tar.gz') == archive and json.loads(z.read('request.json')) == expected_q, 'derived_artifact_drift')
    q = expected_q; (work / 'v17-preview.tar.gz').write_bytes(archive)
    ...
    try:
        ...
        source = Path(__file__).with_name('search3_v17_preview_remote.py').read_bytes()
        ...
        binding = {'name': f"search3-v17-bind-{q['deploy_run']}-{secrets.token_hex(12)}.txt", 'nonce': secrets.token_hex(32)}
        remote('bind', binding)
        try:
        ...
        need(re.fullmatch(r'/tmp/search3-v17\.[A-Za-z0-9_-]+\.tar\.gz', q['archive']), 'remote_upload_path')
        command(['scp', *opts, str(work / 'v17-preview.tar.gz'), user + '@' + host + ':' + q['archive']])
