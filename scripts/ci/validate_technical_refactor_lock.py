...
if owner.get('active_mode')!='ux_visual' or owner.get('active_phase')!='ANYTOUR_DESIGN_SYSTEM_2_SITE_UNIFICATION' or owner.get('current_stage')!='CROSS_PAGE_JOURNEY_AUDIT': errors.append('owner visual stage drift')
if state.get('mode')!='ux_visual' or state.get('phase')!='ANYTOUR_DESIGN_SYSTEM_2_SITE_UNIFICATION' or (state.get('current_stage') or {}).get('id')!='CROSS_PAGE_JOURNEY_AUDIT': errors.append('state visual stage drift')
if errors:
 print('TECHNICAL_REFACTOR_LOCK_FAIL'); [print('-',e) for e in errors]; raise SystemExit(1)
print('TECHNICAL_REFACTOR_LOCK_OK status=inactive owner_mode=ux_visual design_system=2.0')
