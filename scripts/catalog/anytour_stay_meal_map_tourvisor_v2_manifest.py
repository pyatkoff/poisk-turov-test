#!/usr/bin/env python3
"""Build the exact reviewed Tourvisor meal mapping v2 manifest from immutable CURRENT v4 evidence."""
...
OPERATION = "local-stay-meal-map-tourvisor-2690-20260917-v2"
CURRENT_RESULT_SHA256 = "b4a03a16cac10139760abb3b1c9c22d281cebbcb4a78bde32c5a3691a8a1c06a"
CURRENT_ARTIFACT_ID = 10486206713
CURRENT_OPERATION = "local-stay-evidence-current-2690-20260917-v4"
...
EXPECTED_ALL = {"2": 38, "3": 79, "4": 19, "5": 1, "7": 100, "9": 49}
EXPECTED_SELECTED = {"3": 79, "4": 19, "7": 100, "9": 49}
EXPECTED_ROWS = 247
...
def main() -> int:
    if len(sys.argv) != 3:
        raise SystemExit("usage: anytour_stay_meal_map_tourvisor_v2_manifest.py CURRENT_V4_JSON NEW_MANIFEST_JSON")
    source = pathlib.Path(sys.argv[1])
    target = pathlib.Path(sys.argv[2])
    if source.is_symlink() or not source.is_file() or target.exists() or target.is_symlink():
        raise SystemExit("expected regular source and exclusive NEW manifest path")
    raw = source.read_bytes()
    if hashlib.sha256(raw).hexdigest() != CURRENT_RESULT_SHA256:
        raise SystemExit("CURRENT v4 evidence digest mismatch")
    data = json.loads(raw)
    if not isinstance(data, dict) or data.get("status") != "current_read_only_evidence_verified" or data.get("operation") != CURRENT_OPERATION:
        raise SystemExit("wrong CURRENT v4 operation/status")
    if any(data.get(k) != 0 for k in ("writes", "supplierCalls", "automaticAccepts", "publicFileWrites")):
        raise SystemExit("CURRENT v4 evidence is not side-effect-free")
    inventory = data.get("inventory")
    if not isinstance(inventory, dict) or inventory.get("source") != "saved_db_only" or inventory.get("namespace") != "legacy_catalog":
        raise SystemExit("wrong CURRENT inventory source")
    if any(inventory.get(k) != 0 for k in ("writes", "supplierCalls", "automaticAccepts")):
        raise SystemExit("CURRENT inventory is not side-effect-free")
    meals = inventory.get("mealCandidates")
    if not isinstance(meals, list) or len(meals) != sum(EXPECTED_ALL.values()):
        raise SystemExit("unexpected CURRENT meal candidate count")
    ...
    for row in meals:
        if not isinstance(row, dict) or row.get("decisionState") != "unmapped":
            raise SystemExit("CURRENT meal candidates must all remain unmapped")
        scope = row.get("scope")
        ref = row.get("reference")
        if not isinstance(scope, dict) or not isinstance(ref, dict):
            raise SystemExit("invalid CURRENT meal scope/reference")
        if scope.get("namespace") != "legacy_catalog" or ref.get("kind") != "meal" or ref.get("keyKind") != "code":
            raise SystemExit("unexpected CURRENT meal scope/reference kind")
        ...
        evidence_ref = f"stay-v4:{CURRENT_RESULT_SHA256};tv-meal:{external}->{target_code}"
        if len(evidence_ref.encode("utf-8")) > 255:
            raise SystemExit("evidence reference too long")
        rows.append({
            "scope": scope,
            "reference": ref,
            "hotelId": hotel_id,
            ...
            "evidence": {"ref": evidence_ref, "sha256": evidence_sha, "reviewedBy": "pyatkoff"},
        })

    if dict(sorted(all_counts.items())) != EXPECTED_ALL:
        raise SystemExit(f"unexpected CURRENT meal-id distribution: {dict(sorted(all_counts.items()))}")
    if dict(sorted(selected_counts.items())) != EXPECTED_SELECTED or len(rows) != EXPECTED_ROWS:
        raise SystemExit("reviewed selected distribution mismatch")
