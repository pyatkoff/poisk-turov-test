...
final class AnyTourStayCandidates
{
    ...
    private function mealCandidates(int $limit): array
    {
        $sql = "SELECT s.external_key AS hotel_key,s.anytour_hotel_id,s.source_sha256,
                CAST(h.operator_id AS CHAR) AS operator_key,CAST(h.meal_id AS CHAR) AS external_key,
        ...
        return array_map(static fn(array $r): array => self::row($r, 'meal', 'code', (string)$r['external_key']), $rows);
    }
    ...
    private function roomCandidates(int $limit): array
    {
        $sql = "SELECT s.external_key AS hotel_key,s.anytour_hotel_id,s.source_sha256,
                ...
                CASE WHEN o.room_id IS NOT NULL AND o.room_id>0 THEN 'code' ELSE 'label' END AS key_kind,
                CASE WHEN o.room_id IS NOT NULL AND o.room_id>0 THEN CAST(o.room_id AS CHAR) ELSE o.room_type END AS external_key,
            ...
            LEFT JOIN anytour_stay_mappings m ON m.namespace='legacy_catalog' AND m.external_hotel_key=s.external_key
                ...
                AND m.key_kind=CASE WHEN o.room_id IS NOT NULL AND o.room_id>0 THEN 'code' ELSE 'label' END
                AND m.external_key=CASE WHEN o.room_id IS NOT NULL AND o.room_id>0 THEN CAST(o.room_id AS CHAR) ELSE o.room_type END
            ...
            GROUP BY s.external_key,s.anytour_hotel_id,s.source_sha256,o.operator_id,key_kind,external_key
            ORDER BY observed_count DESC,last_seen_at DESC,s.anytour_hotel_id ASC,o.operator_id ASC,external_key ASC
            LIMIT " . $limit;
        $rows = $this->pdo->query($sql)->fetchAll(PDO::FETCH_ASSOC);
        return array_map(static fn(array $r): array => self::row($r, 'room', (string)$r['key_kind'], (string)$r['external_key']), $rows);
    }
