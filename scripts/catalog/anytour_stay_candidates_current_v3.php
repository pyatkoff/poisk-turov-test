<?php
/** One-shot CURRENT read-only stay evidence capture v3. No supplier calls or DB writes. */
declare(strict_types=1);
if (PHP_SAPI !== 'cli') { http_response_code(404); exit; }

const ANYTOUR_STAY_EVIDENCE_OPERATION = 'local-stay-evidence-current-2690-20260917-v3';
const ANYTOUR_STAY_EVIDENCE_SOURCE = '26e2de291f20bfca6aff365d3e9c77b48a12f3cf';
const ANYTOUR_STAY_EVIDENCE_LIMIT = 1000;

function anytour_stay_schema_metadata_v3(PDO $pdo): array
{
...
function anytour_stay_write_result_v3(string $dir, array $result): string
{
...
try {
    ...
    $metadata = anytour_stay_schema_metadata_v3($pdo);
    ...
    $sha = anytour_stay_write_result_v3($operationDir, $result);
    ...
    if ($operationDir !== '' && is_dir($operationDir) && !is_file($operationDir . '/result.json')) {
        ...
        try {
            anytour_stay_write_result_v3($operationDir, ['status'=>'current_read_only_evidence_failed','operation'=>ANYTOUR_STAY_EVIDENCE_OPERATION,
